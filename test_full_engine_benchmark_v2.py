# -*- coding: utf-8 -*-
"""
test_full_engine_benchmark_v2.py
=================================
Second-generation benchmark: all four fixes applied to the failing components.

CHANGES vs benchmark_ricci (previous generation):
───────────────────────────────────────────────────
  C2  REDEFINED   — drop raw Edot drift (always negative, no pre-onset contrast).
                    New signal: Edot_accel = d(Edot)/dt — the ACCELERATION of
                    energy dissipation.  Predict < 0 pre-onset (braking
                    intensifies as collapse approaches).

  C3  RETIRED     — three-phase AND-precursor removed.  The AND of three
                    individually-weak binary signals has no path to significance
                    at N_onset ≤ 12.  C3b (OR disjunction) retained and
                    updated to use Edot_accel.

  C5  REBUILT     — replace TSB Morse proxy with 0-dimensional persistent
                    homology via sublevel filtration on the Forman-Ricci
                    vertex curvature.  Union-find on sorted vertices; a β₀
                    death marks a topological pinch in the k-NN graph.
                    Applied to ALL tiers (same Ricci graph, domain-level).

  C6  FIXED       — z-score E by quiescent statistics before Kramers barrier
                    computation.  Direction-aware e_star (placed on the crisis
                    side of the quiescent mean, ±2σ) so the hazard is
                    interpretable regardless of whether E rises or falls into
                    crisis.  EEG e_star was 16M (degenerate) → now ≈ +2.

  cgs1_morse RETIRED  — existed solely for C5; C5 now runs on Ricci graph.

Tiers:   v4 / cgs1 / cgs1_stoch / cgs1_curv   (4 tiers)
Claims:  C1 / C2(new) / C3b / C4 / C5(rebuilt) / C6(fixed) / C7 / C8-C10
Domains: banks / eeg / protein
Output:  benchmark_v2_results.json
...
def compute_forman_betti_transitions(forman: np.ndarray,
                                      nn_idx: np.ndarray) -> np.ndarray:
    """0-dim persistent homology via sublevel filtration on vertex Forman curvature.

    Vertices are added in ascending Forman order.  When an added vertex
    connects to an already-added component, a β₀ death occurs (topological
    pinch in the k-NN graph).  Both endpoints of the merging edge are marked.

    F < 0 vertices (bottleneck / collapse corridor) are added first — exactly
    the regime where pre-onset states are expected to live.
    """
    T      = len(forman)
    ...
    order  = np.argsort(forman)    # ascending: most-negative (bottleneck) first
...
def kramers_hazard(E: np.ndarray, Edot: np.ndarray,
    ...
    sigma_n = max(float(np.std(np.diff(E_norm))), EPS)
...
def main() -> None:
    ...
    print("=" * 100)
    print("DYNAMICAL + RICCI VALIDATION v2")
    print("Fixes: C2=accel | C3AND=retired | C5=Forman_Betti | C6=normalized_Kramers")
    ...
    if not rows:
        ...
        return
    print_report(rows)
    out_path = os.path.join(ROOT, "benchmark_v2_results.json")
    payload  = {
