...
(combinatorial), and \textbf{[supply chain results TBD]}.
...
\begin{itemize}[nosep]
  \item \textbf{Data:} World Bank trade data, Bureau of Transportation
        Statistics, semiconductor industry association reports,
        Bloomberg supply chain maps
  \item \textbf{Simulation:} DPD model with supply chain topology
        (adapt from SIAM paper's financial DPD model)
  \item \textbf{Metrics:} Lead time for disruption warning, cascade
        prevention rate under adaptive vs constant buffers, AUROC for
        disrupted-supplier prediction
\end{itemize}
...
Supply chain          & \checkmark & \checkmark & \checkmark & TBD \\
\bottomrule
\end{tabular}
\end{table}
