# -*- coding: utf-8 -*-
"""
domain_trading_stack_fdic.py
=============================
Mode A: apply trading-stack brakes to the canonical-v4 engine on the FDIC
sector-level Call Report panel (1990-Q1 onward) augmented with the FRED
10y-2y slope as macro context.

Pipeline:
  1. Load FRED slope_10y2y via fred_loader.fetch_all (cached).
  2. Load FDIC SPECGRP quarterly panel via fetch_fdic_specgrp (cached).
  3. compute_sector_features -> build_panel  =>  X (T, N=7 sectors, d=6).
  4. Aggregate to (T, d) by cross-sectional mean across sectors.
  5. Reference window = quarters chronologically before 2008-01-01 (pre-GFC).
  6. Crisis truth windows:
       GFC      2008-Q3 → 2009-Q1
       Euro     2011-Q3 → 2012-Q1
       COVID    2020-Q1 → 2020-Q2
       SVB      2023-Q1 → 2023-Q2
  7. Sweep ~100 brake configs; record AUROC / hit-rate / FAR / lead-time.

Outputs:
  results/trading_stack_fdic/sweep_results.csv
  results/trading_stack_fdic/summary.json
  figures/trading_stack_fdic_top3.png
"""
...
OUTDIR = os.path.join(THIS, "results", "trading_stack_fdic")
...
def main() -> None:
    t0 = time.time()
    print("=" * 100)
    print("  trading-stack brakes on FDIC SPECGRP (sector-level + FRED slope) — Mode A")
    print("=" * 100)

    print("\n[load] FRED slope_10y2y (cache reuse) ...")
    raw_fred = fetch_fred_all(use_cache=True, verbose=False)
    fred_slope = raw_fred["slope_10y2y"]
    print(f"  fred_slope: {len(fred_slope)} obs, "
          f"{fred_slope.index[0].date()}..{fred_slope.index[-1].date()}")

    print("\n[load] FDIC SPECGRP quarterly panel (cache reuse) ...")
    ...
    for j in range(Xagg.shape[1]):
        ...
        if np.isnan(col).any():
            ...
            Xagg[:, j] = col
    print(f"  aggregated (T,d) = {Xagg.shape}")
    ...
    b = best.iloc[0]
    summary = dict(
        domain="fdic", T=int(T), N=int(N), d=int(d),
        date_start=str(dates[0].date()), date_end=str(dates[-1].date()),
        sectors=list(map(str, sectors)),
    ...
    fig.suptitle("FDIC SPECGRP — Top-3 brake configs by AUROC")
    for ax, (_, r) in zip(axes, top3.iterrows()):
        ...
        ax.set_yticks([])
    fig.tight_layout()
    png = os.path.join(FIGDIR, "trading_stack_fdic_top3.png")
