...
PRE-TRADE volatility cap on K_normal — the actual fix for the v55 MaxDD ceiling.

Diagnosis from v56:
  The adaptive-Y schedule is locally optimal at (0.05, 0.20, 0.25, 'aty').
  Every alternative tested in v56 either traps the engine in a death spiral
  (y_floor=0 + dd_aty source) or destroys the moonshot path (dd_source='cb').
  Conclusion: Y throttle is reactive — it cannot stop a fast crash that fires
  one big bar at full leverage.

The real lever is PRE-TRADE: scale K_normal by realised volatility before
the bar even runs.

For each bar i:
  sigma_real = std of last `vol_win` bars of unit returns (annualised proxy)
  k_vol = min(1, sigma_target / max(sigma_real, eps))
  effective_K = K_normal * k_vol * y * k_mult_stack

If realised vol exceeds target, K is shrunk proportionally; if below, K stays
at K_normal (no boost).  This is the correct inverse-vol leverage rule and
is independent of the equity drawdown trajectory.

Stacks held fixed at v55 winners; sweep is over (vol_win, sigma_target).
"""
...
# vol windows (in hourly bars) and per-bar sigma targets.
# unit-return std is per-bar; annualised mult = sqrt(24*365) ≈ 93.6.
# For 1h bars, daily σ ≈ σ_bar · sqrt(24).  Target 1% daily ≈ 0.00204/bar.
VOL_WINS = [24, 72, 168, 24 * 14]   # 1d, 3d, 1w, 2w
SIGMA_TARGETS = [
    ("off",    None),
    ("d_2.0",  0.020 / np.sqrt(24)),  # ~0.408%/bar  (very loose)
    ("d_1.5",  0.015 / np.sqrt(24)),  # ~0.306%/bar
    ("d_1.0",  0.010 / np.sqrt(24)),  # ~0.204%/bar
    ("d_0.7",  0.007 / np.sqrt(24)),  # ~0.143%/bar  (tight)
    ("d_0.5",  0.005 / np.sqrt(24)),  # ~0.102%/bar  (very tight)
]


def simulate_volkcap(
    ...
    *,
    ...
    release_ext: int,
    vol_win: int = 24,
    sigma_target: float | None = None,   # per-bar; None = vol cap off
):
    ...
    # Pre-compute rolling std of unit returns
    if sigma_target is not None:
        s = pd.Series(unit, index=unit_series.index)
        sigma_roll = s.rolling(vol_win, min_periods=max(8, vol_win // 4)).std().shift(1).fillna(0.0).values
    else:
        sigma_roll = None
    ...
    for i in range(len(unit)):
        ...
        # Pre-trade vol cap (the new mechanism)
        if sigma_target is not None and sigma_roll is not None:
            sr = float(sigma_roll[i])
            if sr > 1e-12:
                k_vol = min(1.0, sigma_target / sr)
            else:
                k_vol = 1.0
        else:
            k_vol = 1.0

        r = max(K_normal * y * k_mult * k_vol * ur, -0.95)
        eq *= (1.0 + r)
        peak_alltime = max(peak_alltime, eq)
        eq_vals.append(eq)
        kvol_vals.append(k_vol)

    return dict(
        eq=pd.Series(eq_vals, index=unit_series.index),
        n_blocked=n_blocked,
        n_g24_releases=n_g24_releases,
        mean_kvol=float(np.mean(kvol_vals)),
    )
...
def main():
    ...
    print(BAR)
    print("  v57 — PRE-TRADE VOL CAP on K_normal (real fix for v55 MaxDD ceiling)")
    print(f"  Stacks tested        : {[s[0] for s in STACKS]}")
    print(f"  Vol windows (bars)   : {VOL_WINS}")
    print(f"  Sigma targets (bar)  : {[(l, t) for l, t in SIGMA_TARGETS]}")
    print(BAR)
    ...
    print("\n[Sweep] stack x vol_win x sigma_target")
    print(f"\n  {'stack':<11} {'win':>4} {'sig':<7} {'mean_kv':>7} {'final':>11} {'maxdd':>8} {'calmar':>7} {'sharpe':>7}")
    print("  " + "-" * 85)
    ...
    for (stk_lbl, kp, omega_a, curv_b, admiss_c, g24_eta, g24_lm, rel_thr, rel_d) in STACKS:
        ...
        for vw in VOL_WINS:
            for (sig_lbl, sig_t) in SIGMA_TARGETS:
                # When sig=off, vol_win is irrelevant — skip duplicates
                if sig_t is None and vw != VOL_WINS[0]:
                    continue
                sim = simulate_volkcap(
                    ...
                    vol_win=vw, sigma_target=sig_t,
                )
                eq_test = sim["eq"][sim["eq"].index >= test_ts]
                m = equity_metrics(eq_test, "v57")
                row = dict(
                    stack=stk_lbl, kappa_max=kp_lbl,
                    omega_a=omega_a, curv_b=curv_b, admiss_c=admiss_c,
                    g24_eta=g24_eta, g24_lock_min=g24_lm,
                    release_thr=rel_thr, release_ext_days=rel_d,
                    vol_win=vw, sigma_label=sig_lbl,
                    sigma_target=sig_t if sig_t is not None else 0.0,
                    mean_kvol=sim["mean_kvol"],
                ...
                if m["final"] > 900_000 and m["maxdd"] > -0.25:
                    ...
                    mark = "  · beats v54 DD"
                print(f"  {stk_lbl:<11} {vw:>4} {sig_lbl:<7} {sim['mean_kvol']:>7.3f} "
    ...
    def show_top(label, df_sub, sort_by, ascending):
        ...
        for _, r in top.iterrows():
            print(f"    [{r['stack']:<11}] vw={int(r['vol_win']):>4} sig={r['sigma_label']:<6} "
                  f"mean_kv={r['mean_kvol']:.3f}  Final=${r['final']:>10,.0f}  "
                  f"MaxDD={r['maxdd']:.2%}  Cal={r['calmar']:.2f}  Sh={r['sharpe']:.3f}")
    ...
    json.dump({
        "version": "v57",
        "focus": "pre_trade_vol_cap_on_K_normal",
        "stacks": [list(s) for s in STACKS],
        "vol_wins": VOL_WINS,
        "sigma_targets": [(l, (t if t is not None else None)) for l, t in SIGMA_TARGETS],
        "results": res.to_dict(orient="records"),
    }, open(OUT / "v57_results.json", "w"), default=str, indent=2)

    print(f"\n  -> saved: {OUT / 'sweep_results.csv'}")
    print(f"  -> saved: {OUT / 'v57_results.json'}")
