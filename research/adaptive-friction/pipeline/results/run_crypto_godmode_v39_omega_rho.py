# -*- coding: utf-8 -*-
"""
run_crypto_godmode_v39_omega_rho.py
=====================================
...
Expected outcome
────────────────
  All-fixes-on (omega=0.02, rho_α=0.5–0.6, saturating budget):
    MaxDD: −22% to −28%  (vs v38 −35%)
    Calmar: ≥ 20         (preserving v38 upside)
    r2026: > 0
...
BAR      = "=" * 120
OUT_DIR_ = Path(OUT_DIR) / "v39_omega_rho"
...
# ── v39 CB simulation with all three re-entry filters ────────────────────────

def simulate_combined_lockmin_v39(
    ...
    """CB simulation with three v39 re-entry filters applied.
    ...
    Step 3  Fix 3 — ρ_eff gate:
            If rho_alpha > 0 AND ρ_eff[bar] < rho_alpha × ρ_typ
            → stay halted (extend lock)

    Both gates apply to *all* release types (standard and G2.4 override), since
    ODE health at release time is what matters, regardless of what triggered the
    release condition.
    """
        ...
        # ── Resume evaluation ─────────────────────────────────────────────────
            ...
            if candidate:
                ...
                if candidate and rho_alpha > 0.0:
                    if rho_eff_arr[bar_i] < rho_alpha * rho_typ:
                        gate_blocks_rho += 1
                        candidate = False
...
def main() -> None:
    ...
    print(BAR)
    print("  CRYPTO GODMODE v39 — Ω gate + saturating Itô budget + ρ_eff gate")
    ...
    print("  Fix 3: ρ_eff < rho_alpha × rho_typ → extend lock (Mode 2 crash)")
                ...
                for lock_min in LOCK_MIN_GRID:
                    ...
                    for omega_thresh in omega_loop:
                        for rho_alpha in rho_loop:
                            ...
                            sim = simulate_combined_lockmin_v39(
                                unit_normal=unit,
    ...
    out_json = OUT_DIR_ / "crypto_godmode_v39_omega_rho.json"
    rows_clean = [
        ...
        json.dump(
            {
                ...
                "fixes": {
                    ...
                    "Fix3_rho_eff_gate":      "rho_eff < rho_alpha*rho_typ → stay halted",
                },
                "results": rows_clean,
            },
    ...
    print(f"\n{BAR}")
    print(f"  v39 COMPLETE  |  elapsed={elapsed:.1f}s  ({elapsed/60:.1f} min)")
    ...
    print(f"    Fix 3 (ρ_eff gate): ρ_eff < rho_alpha × ρ_typ → extend lock")
    print(BAR)
