...
def main():
    print("=" * 80)
    print("  CRYPTO BSDT v11 — Fast γ + Weight Tilt")
    print("  Layer 6: w_tilt = rolling_Sharpe × γ_fast_tilt (no position scaling)")
    ...
    sims_v11 = dict(sims)   # individual strategies same as v9 (positions unchanged)
    ...
    results = {
        ...
        "v11_results": _clean(sims_v11),
        "v11_gamma_stats": {
            "gamma_clip_low":  GAMMA_CLIP_LOW,
            "gamma_clip_high": GAMMA_CLIP_HIGH,
            "gamma_score_win": GAMMA_SCORE_WIN,
            "gamma_fast_mean_test":  float(round(gamma_fast.clip(GAMMA_CLIP_LOW, GAMMA_CLIP_HIGH)[test_mask].mean(), 4)),
            "gamma_fast_std_test":   float(round(gamma_fast.clip(GAMMA_CLIP_LOW, GAMMA_CLIP_HIGH)[test_mask].std(), 4)),
            "gamma_slow_mean_test":  float(round(gamma_eth.clip(GAMMA_CLIP_LOW, GAMMA_CLIP_HIGH)[test_mask].mean(), 4)),
            "gamma_slow_std_test":   float(round(gamma_eth.clip(GAMMA_CLIP_LOW, GAMMA_CLIP_HIGH)[test_mask].std(), 4)),
            "pct_hi_gamma_fast":     float(round((gamma_fast.clip(GAMMA_CLIP_LOW, GAMMA_CLIP_HIGH)[test_mask] > 0.6).mean() * 100, 1)),
            "pct_lo_gamma_fast":     float(round((gamma_fast.clip(GAMMA_CLIP_LOW, GAMMA_CLIP_HIGH)[test_mask] < 0.4).mean() * 100, 1)),
        },
        "version_history": {
            ...
            "v8":  {"P1_ETH_BTC": -5.664, "P2_ETH_SOL": 0.546, "P3_ETH_BNB": -16.946,
                    "M1_ALT_macro": 1.387},
            "v10": {"DYNAMIC_COMBINED": 0.597},
        },
    }

    import os
    out_path = os.path.join(OUT_DIR, "crypto_bsdt_v11_results.json")
    ...
    # ── Summary ───────────────────────────────────────────────────────────
    print("\n" + "=" * 80)
    print("  SUMMARY: v4 → v6 → v7 → v8 → v9 → v10 → v11 (fast γ weight tilt)")
    print("=" * 80)
    v10_ref = {'A_directional': 0.535, 'P1_ETH_BTC': -1.248,
               'P2_ETH_SOL': 1.357, 'P3_ETH_BNB': -0.463,
               'M1_ALT_macro': 1.387, 'M1_BTC_macro': 0.156,
               'DYNAMIC_COMBINED': 0.597}
    for name in ['A_directional', 'P1_ETH_BTC', 'P2_ETH_SOL', 'P3_ETH_BNB',
                 'M1_ALT_macro', 'DYNAMIC_v10', 'DYNAMIC_v11']:
        v4  = v4_ref.get(name, None)
        v10 = v10_ref.get(name.replace('DYNAMIC_v10','DYNAMIC_COMBINED').replace('DYNAMIC_v11','DYNAMIC_COMBINED'), None)
        v11 = sims_v11[name]['sharpe'] if name in sims_v11 else None
        v4s  = f"{v4:>+7.3f}"  if v4  is not None else "       "
        v10s = f"{v10:>+7.3f}" if v10 is not None else "       "
        v11s = f"{v11:>+7.3f}" if v11 is not None else "       "
        if name in sims_v11:
            r  = sims_v11[name]
            ...
            dd = ""
        print(f"  {name:<22}  v4:{v4s}  v10:{v10s}  v11:{v11s}{dd}")
    print()
