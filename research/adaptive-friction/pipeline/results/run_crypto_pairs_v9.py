...
v8 lesson:
  Training-anchored z-score correctly raised VALID% from 13–17% → 28–40%.
  Drift diagnosis worked. But rev_eff_med was negative for ALL three pairs
  because the rev_eff gate evaluates sign(-z_{t-1})×sret_t relative to the
  training anchor — when z has a persistent positive/negative bias (drifted
  pair), sign(-z) is consistently directional and competes with the actual
  reversion signal, making rev_eff systematically negative even on days where
  local reversion is happening. Trading collapsed to 0.6–1.6% active (n=7–19).

  The ablation (v6-router, no validity, anchored z) showed:
    P2 ETH/SOL: +1.288  P3 ETH/BNB: +0.772
  — anchored z already provides genuine drift-detection signal. The rev_eff
  gate added noise rather than signal for all three pairs.

v9 fix — Drift-Only Validity Gate:
  Valid = |rolling_mean(z_t, 60d)| < DRIFT_THRESH
  (rev_eff gate removed)

  The drift gate alone is sufficient: if z_t is anchored to training μ/σ,
  |z̄_60d| directly measures displacement from the training equilibrium level.
  Reversion effectiveness is captured implicitly by UDL geometry (EQUIL/DEGEN)
  and the L2 A-rank intensity gate — no need for an additional rolling PnL gate.

Full 5-layer architecture (v9):
  Layer 1: Ω — timing gate
  Layer 2: A — intensity gate
  Layer 3: UDL MDN — geometry (EQUIL / DEGEN / STRUCT)
  Layer 4: Drift gate only  |z̄_60d| < DRIFT_THRESH (anchored z)
...
def main():
    print("=" * 80)
    print("  CRYPTO BSDT v9 — Drift-Only Validity Gate (rev_eff gate removed)")
    print("  Anchored z: z = (S − μ_train) / σ_train")
    ...
    import os
    out_path = os.path.join(OUT_DIR, "crypto_bsdt_v9_results.json")
    ...
    # ── Summary ───────────────────────────────────────────────────────────
    print("\n" + "=" * 80)
    print("  SUMMARY: v4 → v6 → v7 → v8 → v9 (drift-only validity gate)")
    print("=" * 80)
    v8_ref = {'A_directional': 0.535, 'P1_ETH_BTC': -5.664,
    ...
    for name in ['A_directional', 'P1_ETH_BTC', 'P2_ETH_SOL', 'P3_ETH_BNB',
        ...
        if name in sims:
            ...
            dd = ""
        print(f"  {name:<22}  v4:{v4s}  v8:{v8s}  v9:{v9s}{dd}")
    print()
