"""
Crypto BSDT v17 — Circular statistics + true MFLS curvature on top of v16
=========================================================================

v16 (BSDT vector geometry, +0.969 Sharpe) is the production champion.

v17 — add the four signals from the math reference (§VII / §XIV):
  1. R_t   = mean resultant length of strategy direction vectors      "crowding"
  2. θ̄_t   = mean pairwise angle between strategy direction vectors
  3. tan(θ̄_t)  — singular near π/2, amplifies regime-flip risk
  4. κ_t   = |MFLS_t − 2 MFLS_{t-1} + MFLS_{t-2}|  (true 2nd-order curvature
             vs. v11/12's |ΔMFLS| proxy)

Geometry: at each t, build a (W,N_s) matrix of the last W days of each
strategy's daily pnl. Unit-normalise each strategy's W-vector. Across the
N_s = 6 strategies:
    R_t       = || mean of N_s unit vectors ||                 ∈ [0, 1]
    cosθ_ij_t = u_i · u_j      → θ̄_t = mean over i<j of arccos(·)

Falsifiable 5-way ablation (all on v14b 6-strat baseline; v16 is the float):
  v14b                      v10 × linear γ dial                     +0.913
  v16  (PROD)              v14b + φ + collapse alignment + EMA       +0.969
  v17_R         layered "crowding" penalty: lev_v16 / (1 + R_PEN · R)
  v17_tan       layered tan(θ̄) penalty: lev_v16 / (1 + TAN_PEN · tan_clip)
  v17_kappa     layered curvature penalty: lev_v16 / (1 + KAPPA_PEN · κ_z)
  v17  (PROD?)  all three layered + EMA smoothing

All four new signals are causal (rolling/expanding windows only).
"""
...
# ─── v17: circular statistics + true MFLS curvature ──────────────────
CIRC_WIN          = 21          # rolling window for unit-vector construction
R_PEN             = 0.5         # crowding penalty: lev /= (1 + R_PEN * R_t)
TAN_PEN           = 0.3         # tan(θ̄) penalty after clip
TAN_CLIP          = 5.0         # cap tan to avoid runaway near π/2
KAPPA_WIN         = 60          # expanding-z window for κ
KAPPA_PEN         = 0.2         # curvature penalty: lev /= (1 + KAPPA_PEN * max(κ_z, 0))
...
# ─────────────────────────────────────────────────────────────────────────────
#  v17: circular statistics over strategy direction-vectors  +  true MFLS κ
# ─────────────────────────────────────────────────────────────────────────────
def compute_circular_stats(pnl_dict, window=21):
    """
    v17: rolling circular statistics over the cross-section of strategy
    direction vectors.

    For each time t, take the last `window` days of each strategy's daily
    pnl as a W-vector u_i ∈ R^W, then unit-normalise:  û_i = u_i / ‖u_i‖.

    Across the N_s strategies:
      R_t        = ‖ (1/N) Σ_i û_i ‖           (mean resultant length, ∈ [0,1])
      cosθ̄_t     = (2/N(N-1)) Σ_{i<j} (û_i · û_j)        (mean pairwise cos)
      θ̄_t        = arccos( clip(cosθ̄_t, -1, 1) )         ∈ [0, π]
      tanθ̄_t    = tan(θ̄_t)        (singular near π/2 — use TAN_CLIP)

    Geometry:
      R_t → 1 : all strategies move in the same direction → CROWDED
      R_t → 0 : strategies orthogonal/cancel → DIVERSIFIED
      θ̄_t → 0  ⇔ R_t → 1
      θ̄_t → π/2: tan blows up — extreme regime-flip / orthogonality threshold

    Returns three pd.Series indexed like the pnl frame.
    """
    # Stack pnl into (T, N_s) frame
    P = pd.DataFrame(pnl_dict).fillna(0.0)
    T_, N_ = P.shape
    R_arr   = np.full(T_, np.nan)
    cos_arr = np.full(T_, np.nan)
    M = P.values
    for t in range(window, T_):
        # Each column = one strategy's last `window` daily pnls
        seg = M[t - window:t]                       # (W, N_s)
        norms = np.linalg.norm(seg, axis=0) + 1e-9  # (N_s,)
        U = seg / norms                              # (W, N_s) unit columns
        # Mean resultant: norm of mean across strategies
        # u_i is the column. Mean vector = U @ (1/N) ones
        mean_u = U.mean(axis=1)                      # (W,)
        R_arr[t] = np.linalg.norm(mean_u)
        # Mean pairwise cosine = (||sum||^2 - N) / (N(N-1))
        # since ||sum||^2 = Σ_i ||û_i||^2 + 2 Σ_{i<j} û_i·û_j = N + 2 Σ cos
        s = U.sum(axis=1)                            # (W,)
        sum_sq = float(np.dot(s, s))
        cos_arr[t] = (sum_sq - N_) / (N_ * (N_ - 1)) if N_ > 1 else 1.0

    R       = pd.Series(R_arr,   index=P.index, name='R')
    cos_bar = pd.Series(cos_arr, index=P.index, name='cos_theta_bar')
    theta_bar = np.arccos(cos_bar.clip(-1.0, 1.0))
    tan_bar = np.tan(theta_bar)
    return R, theta_bar.rename('theta_bar'), tan_bar.rename('tan_theta_bar')


def compute_mfls_curvature(mfls, win=60):
    """
    v17: TRUE 2nd-order MFLS curvature (causal central-diff is non-causal,
    so use the trailing 2nd-difference instead):
        κ_t = | MFLS_t − 2·MFLS_{t-1} + MFLS_{t-2} |

    This is the discrete second derivative — what v11/12 called λ via |ΔMFLS|
    is only the FIRST derivative. The math reference (§XIV) calls for the
    full curvature operator.

    Returns:
        kappa     : raw |2nd diff|
        kappa_z   : expanding-z normalised
    """
    s = mfls
    kappa = (s - 2.0 * s.shift(1) + s.shift(2)).abs()
    mu = kappa.expanding(win).mean()
    sd = kappa.expanding(win).std()
    kappa_z = (kappa - mu) / (sd + 1e-9)
    return kappa.rename('kappa'), kappa_z.rename('kappa_z')


def apply_circ_penalties(lev_base, R=None, tan_bar=None, kappa_z=None,
                          R_pen=0.5, tan_pen=0.3, tan_clip=5.0,
                          kappa_pen=0.2, ema_span=5, lo=0.3, hi=1.2):
    """
    v17: layer crowding (R), regime-flip (tan θ̄), curvature (κ_z) penalties
    on top of any base leverage scalar.

      lev = lev_base / (1 + R_pen     · R_t)              (crowding guard)
      lev = lev      / (1 + tan_pen   · clip(tan θ̄, 0, C)) (regime-flip guard)
      lev = lev      / (1 + kappa_pen · max(κ_z, 0))      (curvature guard)
      lev = ema(lev, ema_span).clip(lo, hi)
    Any of R / tan_bar / kappa_z passed as None is skipped (penalty=1).
    """
    lev = lev_base.copy()
    if R is not None:
        lev = lev / (1.0 + R_pen * R.fillna(0.0).clip(0.0, 1.0))
    if tan_bar is not None:
        # Per math reference (§XIV.1): tan(θ)→0 = trajectory aligned with
        # collapse direction = DANGEROUS; tan(θ)→∞ = orthogonal = SAFE.
        # θ̄ here is the mean pairwise angle BETWEEN strategies, so:
        #   small |tan(θ̄)|  ⇔  strategies near-aligned (or near-anti-aligned)
        #                       ⇔  collapse-directed cross-section → PENALISE.
        #   large |tan(θ̄)|  ⇔  θ̄ ≈ π/2 ⇔ orthogonal book → SAFE.
        # Build a "collapse-aligned indicator" ∈ [0,1]: 1 at tan=0, 0 at |tan|≥tan_clip.
        tan_abs = tan_bar.fillna(0.0).abs().clip(upper=tan_clip)
        collapse_aligned = (1.0 - tan_abs / tan_clip).clip(lower=0.0)
        lev = lev / (1.0 + tan_pen * collapse_aligned)
    if kappa_z is not None:
        kz = kappa_z.fillna(0.0).clip(lower=0.0)
        lev = lev / (1.0 + kappa_pen * kz)
    lev = lev.ewm(span=ema_span, adjust=False).mean()
    return lev.clip(lower=lo, upper=hi)
...
def main():
    ...
    # ── v17: circular statistics + true MFLS curvature ───────────────────
    # All ablations layered on TOP of v16 (current production champion).
    R_t, theta_bar_t, tan_bar_t = compute_circular_stats(
        pnl_v10_dict_orig, window=CIRC_WIN)
    kappa_t, kappa_z_t          = compute_mfls_curvature(mfls_eth, win=KAPPA_WIN)

    # 17a: R only (crowding penalty)
    lev_v17_R = apply_circ_penalties(lev_v16, R=R_t,
                                      tan_bar=None, kappa_z=None,
                                      R_pen=R_PEN, ema_span=GEOM_EMA_SPAN,
                                      lo=GEOM_LEV_LO, hi=GEOM_LEV_HI)
    pnl_dyn_v17_R = pnl_dyn_v10_orig * lev_v17_R.shift(1).fillna(1.0)
    sims_v17_R    = simulate_from_pnl(pnl_dyn_v17_R[test_mask], 'DYNAMIC_v17_R')

    # 17b: tan(θ̄) only (regime-flip penalty)
    lev_v17_tan = apply_circ_penalties(lev_v16, R=None,
                                        tan_bar=tan_bar_t, kappa_z=None,
                                        tan_pen=TAN_PEN, tan_clip=TAN_CLIP,
                                        ema_span=GEOM_EMA_SPAN,
                                        lo=GEOM_LEV_LO, hi=GEOM_LEV_HI)
    pnl_dyn_v17_tan = pnl_dyn_v10_orig * lev_v17_tan.shift(1).fillna(1.0)
    sims_v17_tan    = simulate_from_pnl(pnl_dyn_v17_tan[test_mask], 'DYNAMIC_v17_tan')

    # 17c: κ only (true 2nd-order MFLS curvature penalty)
    lev_v17_kap = apply_circ_penalties(lev_v16, R=None, tan_bar=None,
                                        kappa_z=kappa_z_t,
                                        kappa_pen=KAPPA_PEN,
                                        ema_span=GEOM_EMA_SPAN,
                                        lo=GEOM_LEV_LO, hi=GEOM_LEV_HI)
    pnl_dyn_v17_kap = pnl_dyn_v10_orig * lev_v17_kap.shift(1).fillna(1.0)
    sims_v17_kap    = simulate_from_pnl(pnl_dyn_v17_kap[test_mask], 'DYNAMIC_v17_kappa')

    # 17: full circular layer  (R + tan + κ + EMA)   PRODUCTION CANDIDATE
    lev_v17 = apply_circ_penalties(lev_v16, R=R_t,
                                    tan_bar=tan_bar_t, kappa_z=kappa_z_t,
                                    R_pen=R_PEN, tan_pen=TAN_PEN,
                                    tan_clip=TAN_CLIP, kappa_pen=KAPPA_PEN,
                                    ema_span=GEOM_EMA_SPAN,
                                    lo=GEOM_LEV_LO, hi=GEOM_LEV_HI)
    pnl_dyn_v17 = pnl_dyn_v10_orig * lev_v17.shift(1).fillna(1.0)
    sims_v17_comb = simulate_from_pnl(pnl_dyn_v17[test_mask], 'DYNAMIC_v17')

    # ── v17 diagnostics ───────────────────────────────────────────────────
    print(f"\n── v17 circular statistics + MFLS curvature [test] ──────────────────")
    R_t_test       = R_t[test_mask].dropna()
    theta_t_test   = theta_bar_t[test_mask].dropna()
    tan_t_test     = tan_bar_t[test_mask].dropna()
    kappa_z_test   = kappa_z_t[test_mask].dropna()
    lev_v16_t      = lev_v16[test_mask].dropna()
    lev_v14b_t     = lev_mult[test_mask].dropna()
    phi_t_test_pre = phi_t[test_mask].dropna()
    print(f"  R   (mean resultant): mean={R_t_test.mean():.3f}  "
          f"std={R_t_test.std():.3f}  "
          f"p10={R_t_test.quantile(.1):.3f}  p90={R_t_test.quantile(.9):.3f}  "
          f"max={R_t_test.max():.3f}")
    print(f"  θ̄  (mean pair angle): mean={theta_t_test.mean():.3f} rad  "
          f"({np.degrees(theta_t_test.mean()):.1f}°)  "
          f"std={theta_t_test.std():.3f}")
    tan_clip_arr = tan_t_test.clip(-TAN_CLIP, TAN_CLIP)
    print(f"  tan(θ̄) [clip ±{TAN_CLIP}]: mean={tan_clip_arr.mean():.3f}  "
          f"std={tan_clip_arr.std():.3f}  "
          f"max={tan_clip_arr.max():.3f}  "
          f"%>1: {(tan_t_test>1.0).mean()*100:.1f}%")
    print(f"  κ_z (MFLS 2nd-diff z): mean={kappa_z_test.mean():+.3f}  "
          f"std={kappa_z_test.std():.3f}  "
          f"p90={kappa_z_test.quantile(.9):+.3f}  "
          f"p99={kappa_z_test.quantile(.99):+.3f}")
    lev_v17_t = lev_v17[test_mask].dropna()
    print(f"  lev_v16:  mean={lev_v16_t.mean():.3f}  std={lev_v16_t.std():.3f}")
    print(f"  lev_v17:  mean={lev_v17_t.mean():.3f}  std={lev_v17_t.std():.3f}  "
          f"min={lev_v17_t.min():.3f}  max={lev_v17_t.max():.3f}")
    print(f"  v17 vs v16: defensive (<0.7) {(lev_v17_t<0.7).mean()*100:.1f}% "
          f"vs {(lev_v16_t<0.7).mean()*100:.1f}%")
    # Orthogonality check vs existing signals
    common = R_t_test.index.intersection(phi_t_test_pre.index)
    corr_R_phi    = np.corrcoef(R_t_test.loc[common].values,
                                 phi_t_test_pre.loc[common].values)[0,1]
    corr_R_gamma  = np.corrcoef(R_t_test.loc[common].values,
                                 gamma_rank.loc[common].fillna(0.5).values)[0,1]
    corr_kap_phi  = np.corrcoef(kappa_z_test.reindex(common).fillna(0.0).values,
                                 phi_t_test_pre.loc[common].values)[0,1]
    print(f"  corr(R, φ)={corr_R_phi:+.3f}   "
          f"corr(R, γ_rank)={corr_R_gamma:+.3f}   "
          f"corr(κ_z, φ)={corr_kap_phi:+.3f}    (orthogonality check)")
    ...
    sims_all['DYNAMIC_v17_R']     = sims_v17_R
    sims_all['DYNAMIC_v17_tan']   = sims_v17_tan
    sims_all['DYNAMIC_v17_kappa'] = sims_v17_kap
    sims_all['DYNAMIC_v17']       = sims_v17_comb
    sims_all['F_FUND_ETH']   = sim_fund_eth
    sims_all['F_FUND_BTC']   = sim_fund_btc

    results = {
        "model":   "Crypto BSDT v17 — circular statistics + true MFLS curvature",
        "version": "v17",
        "gamma_stats": {
        ...
        "version_history": {
            ...
            "v17_R_dynamic":     sims_v17_R['sharpe'],
            "v17_tan_dynamic":   sims_v17_tan['sharpe'],
            "v17_kappa_dynamic": sims_v17_kap['sharpe'],
            "v17_dynamic":       sims_v17_comb['sharpe'],
        },
        "v14_diagnostics": {
        ...
        "v17_diagnostics": {
            "R_mean":        float(round(R_t_test.mean(), 4)),
            "R_std":         float(round(R_t_test.std(), 4)),
            "R_p90":         float(round(R_t_test.quantile(0.90), 4)),
            "theta_bar_mean_rad": float(round(theta_t_test.mean(), 4)),
            "theta_bar_mean_deg": float(round(np.degrees(theta_t_test.mean()), 2)),
            "tan_theta_mean_clip": float(round(tan_clip_arr.mean(), 4)),
            "tan_theta_pct_gt1": float(round((tan_t_test>1.0).mean()*100, 1)),
            "kappa_z_mean":  float(round(kappa_z_test.mean(), 4)),
            "kappa_z_p99":   float(round(kappa_z_test.quantile(0.99), 4)),
            "corr_R_phi":    float(round(corr_R_phi, 4)),
            "corr_R_gamma_rank":  float(round(corr_R_gamma, 4)),
            "corr_kappa_phi":     float(round(corr_kap_phi, 4)),
            "lev_v17_mean":  float(round(lev_v17_t.mean(), 3)),
            "lev_v17_std":   float(round(lev_v17_t.std(), 3)),
            "lev_v17_min":   float(round(lev_v17_t.min(), 3)),
            "lev_v17_max":   float(round(lev_v17_t.max(), 3)),
            "pct_defensive_v17": float(round((lev_v17_t<0.7).mean()*100, 1)),
        },
    }

    import os
    out_path = os.path.join(OUT_DIR, "crypto_bsdt_v17_results.json")
    ...
    # ── Summary ───────────────────────────────────────────────────────────
    print("\n" + "=" * 96)
    print("  SUMMARY: v9 → v10 → v11 → v12 → v13 → v14 → v15 → v16 → v17")
    print("=" * 96)
    ...
    summary_rows = [
        ...
        ('v16 (φ+align+EMA) prevPROD','v14b + φ + collapse alignment + EMA',           sims_v16_comb['sharpe']),
        ('v17_R (crowding)',         'v16 / (1 + R)',                                  sims_v17_R['sharpe']),
        ('v17_tan (regime-flip)',    'v16 / (1 + tanθ̄)',                              sims_v17_tan['sharpe']),
        ('v17_kappa (curvature)',    'v16 / (1 + κ_z⁺)  (true MFLS 2nd-diff)',         sims_v17_kap['sharpe']),
        ('v17  (PRODUCTION)',        'v16 + R + tan(θ̄) + κ + EMA',                   sims_v17_comb['sharpe']),
    ]
