...
BAR      = "=" * 120
OUT_DIR_ = Path(OUT_DIR) / "v42_cb_sweep"
...
# ── v42 CB parameter sweep grids ─────────────────────────────────────────────
# Motivation: 86.5% halted time with cb_halt=0.08 / cb_window=90d staircase.
# Looser halt -> fewer trips; shorter window -> rolling peak resets faster.
CB_HALT_GRID   = [0.06, 0.08, 0.10, 0.12, 0.15]
CB_WINDOW_GRID = [14, 30, 60, 90]  # days
# cb_resume = cb_halt / 2  (recomputed per halt level in loop)

# Ramp: disabled (v41 showed no benefit)
RAMP_INIT_GRID = [1.0]
RAMP_BARS_GRID = [168]
...
def main() -> None:
    ...
    n_total = len(TOP3) * len(CB_HALT_GRID) * len(CB_WINDOW_GRID)
    print(BAR)
    print("  CRYPTO GODMODE v42 — CB parameter sweep")
    print(f"  CB_HALT grid:    {CB_HALT_GRID}")
    print(f"  CB_WINDOW grid:  {CB_WINDOW_GRID}")
    print(f"  cb_resume = cb_halt / 2 per level")
    ...
    for d, q in TOP3:
        for kappa_max in KAPPA_MAX_GRID:
            ...
            for cb_halt_v in CB_HALT_GRID:
                cb_resume_v = round(cb_halt_v / 2.0, 4)
                for cb_window_v in CB_WINDOW_GRID:
                    label = (f"d{str(d).replace('.','p')}"
                             f"_q{str(q).replace('.','p')}"
                             f"_halt{int(cb_halt_v*100)}_win{cb_window_v}"
                             f"_res{int(cb_resume_v*100)}"
                    ...
                    # tag CB params
                    row["cb_halt"]   = cb_halt_v
                    row["cb_resume"] = cb_resume_v
                    row["cb_window"] = cb_window_v
                    results.append(row)

                    print(f"  halt={cb_halt_v:.0%} win={cb_window_v:>3}d  "
                          f"{row['calmar']:>7.2f}  ${row['final']:>11,.0f}  "
                          f"{row['maxdd']:>7.1%}  {row['r2026']:>+7.3f}  "
                          f"pct_h={sim.get('pct_halted',0):.1%}  "
                          f"n_trips={sim.get('n_trips',0)}")
    ...
    out_json = OUT_DIR_ / "crypto_godmode_v42_cb_sweep.json"
    rows_clean = [
