"""
Crypto BSDT v7 — UDL MDN + Manifold Validity Layer
===================================================

v6 lesson:
  ✓ UDL MDN correctly classifies spread geometry (EQUIL / DEGEN / STRUCT)
  ✗ But geometry ≠ economic validity:
        ETH/BNB: z-score looked statistically normal (EQUIL)
        yet spread was TRENDING (BNB structural repricing 2023+)
        → UDL told truth about structure; trading assumption was wrong

  Root cause: z-score predicts reversion ONLY when the spread manifold
  is stationary around its training-period mean. When the mean SHIFTS,
  UDL detects a normal-looking deviation from an *outdated* centroid.

v7 design — Manifold Validity Layer (Layer 4):
  ✓ Test 1 — Drift gate:
        drift = |rolling_mean(z, 60d)|
        if drift > DRIFT_THRESH (0.5σ): spread NOT centered → no trade
        Rationale: if z̄ ≠ 0 over 60d, the equilibrium level has shifted

  ✓ Test 2 — Reversion effectiveness gate:
        rev_eff_t = rolling_mean(sign(-z_{t-1}) × sret_t, 60d)
        if rev_eff_t < EFF_THRESH (0.0): reversion not paying → no trade
        Rationale: realized test — did past reversion calls earn money?

  ✓ Both tests are backward-looking only (no lookahead bias)
  ✓ Both auto-disable a pair dynamically (not manual pruning)
  ✓ Pair re-enables when validity recovers

Full 5-layer architecture:
  Layer 1: Ω (BSDT) — is the system active?
  Layer 2: A (MFLS/γ) — is the move strong enough?
  Layer 3: UDL MDN — what is the local spread geometry?
             DEGENERATE  → skip (flat window)
             EQUILIBRIUM → revert × POA_scale
             STRUCTURAL  → revert × 0.5 (cautious)
  Layer 4: VALIDITY FILTER (new)
             drift_ok   = |z̄_60d| < 0.5
             rev_eff_ok = rev_eff_60d > 0.0
             if not (drift_ok AND rev_eff_ok): skip
  Layer 5: sizing = min(|z|, 2) / 2 × POA_scale

Pairs:
  P1. ETH/BTC — moderate edge; validity layer stabilises it
  P2. ETH/SOL — strong edge; validity should confirm consistently
  P3. ETH/BNB — broken pair; validity should auto-suppress
  M1. BTC/ALT — macro rotation (unchanged)
"""
...
DRIFT_THRESH  = 0.50  # |z̄_60d| above this → manifold not centered
...
def compute_manifold_validity(z_spread, sret):
    """
    Two backward-looking tests — no lookahead bias.
    ...
    Test 2 — Reversion effectiveness gate:
        rev_eff = rolling_mean(sign(-z_{t-1}) × sret_t, EFF_WINDOW)
        If < EFF_THRESH: reversion calls have been losing → invalid

    Returns
    -------
    valid     : pd.Series int  (1 = both tests pass, 0 = either fails)
    z_drift   : pd.Series float  absolute rolling mean of z (Test 1 raw)
    rev_eff   : pd.Series float  rolling reversion PnL (Test 2 raw)
    """
    ...
    eff_ok    = rev_eff > EFF_THRESH

    valid = (drift_ok & eff_ok).astype(int)
    return (
...
def route_pair_v7(omega, A_rank, z_spread, udl_state, poa,
    ...
    5 gates (all must pass):
      ...
      5. manifold_valid (drift OK and reversion paying)
...
def main():
    print("=" * 80)
    print("  CRYPTO BSDT v7 — UDL MDN + Manifold Validity Layer")
    print("  Layer 3: UDL MDN  (EQUIL / DEGEN / STRUCT)")
    print("  Layer 4: Validity (|z̄_60d| < 0.5  AND  rev_eff_60d > 0)")
    print("=" * 80)
    ...
    print()
    print("─" * 95)
    print(f"── PnL results (test {TEST_START}–2026) — v4 / v5 / v6 / v7 ──────────────────────────")
    print("─" * 95)
    ...
    results = {
        "model": "Crypto BSDT v7 — UDL MDN + Manifold Validity",
        "version": "v7",
        "architecture": {
            ...
            "layer_4_validity": {
                "drift_test":       f"|rolling_mean(z, {DRIFT_WINDOW}d)| < {DRIFT_THRESH}",
                "effectiveness_test": f"rolling_mean(sign(-z_prev)*sret, {EFF_WINDOW}d) > {EFF_THRESH}",
                "action_if_invalid": "skip (no trade regardless of UDL state)",
            },
        ...
        "version_history": {
            ...
            "v5": {"P1_ETH_BTC": 0.443, "P2_ETH_SOL": 2.099, "P3_ETH_BNB": 0.167,
                   "M1_ALT_macro": 1.387},
    ...
    import os
    out_path = os.path.join(OUT_DIR, "crypto_bsdt_v7_results.json")
    ...
    # ── Summary ───────────────────────────────────────────────────────────
    print("\n" + "=" * 80)
    print("  SUMMARY: v4 → v6 → v7 (with manifold validity layer)")
    print("=" * 80)
    for name in ['A_directional', 'P1_ETH_BTC', 'P2_ETH_SOL', 'P3_ETH_BNB',
                 'M1_ALT_macro']:
        v4 = v4_ref.get(name, None)
        v6 = v6_ref.get(name, None)
        v7 = sims[name]['sharpe'] if name in sims else None
        ...
        if name in sims:
            ...
            dd = ""
        print(f"  {name:<22}  v4:{v4s}  v6:{v6s}  v7:{v7s}{dd}")
    print()
