"""
Crypto BSDT v13 — Regime-conditional rolling Sharpe weights
===================================================================

v12 lesson:
  γ_rank now has structural dynamic range (std=0.264, 34% high / 44% low),
  but the manual tilt_modes assumption ('directional': 1-γ, 'reversion': γ,
  'macro': 1) hurts performance because the empirical regime→strategy
  relationship doesn't match those a priori labels:
    - A_directional Sharpe didn't actually drop on high-γ days
    - P1_ETH_BTC and P3_ETH_BNB lose money in BOTH regimes
    - Tilt redirected weight from +0.535 (A_dir) to +0.156 (BTC_macro)
  Result: v12 +0.355 vs v10 +0.597.

v13 — Regime-conditional rolling Sharpe (no tilt_modes assumption):
  Don't impose which strategy 'should' work in which regime.
  Let the data decide. For each day t with γ_rank_t = r:
    For each strategy:
      Sharpe^high_t = rolling Sharpe over last N high-γ days  (γ_rank > 0.5)
      Sharpe^low_t  = rolling Sharpe over last N low-γ  days  (γ_rank ≤ 0.5)
      Sharpe_t      = Sharpe^high_t  if  r > 0.5  else  Sharpe^low_t
    Weights = normalised Sharpe_t (negative → 0, like v10).

  This is v10's killer feature (rolling Sharpe zeroes losers) PLUS
  regime-conditioning. A strategy that works only in stress regimes will
  get high weight on stress days and zero on calm days — automatically,
  with no a priori 'reversion' label.

  Window = 252 (double v10's 126) because only ≈50% of days are in each
  regime; 252 calendar days → ≈126 same-regime days, matching v10's signal.

v13 architecture:
  Layer 1–5: unchanged from v9/v10/v11/v12 (Ω_strat for strategy gates)
  Layer 6: w = regime_conditional_rolling_Sharpe(pnl_base, γ_rank, window=252)

Reports v10 / v11 / v12 / v13 dynamic Sharpes side-by-side for comparison.
"""
...
GAMMA_REGIME_THRESHOLD = 0.5    # γ_rank > 0.5 → 'high stress' regime
...
def main():
    ...
    W = 110
    print("\n" + "─" * W)
    print("── v9 / v10 / v11 / v12 / v13: dynamic portfolio comparison [test] ──")
    print("─" * W)
    print(f"  {'Strategy':<22}  {'v9 Sh':>7}  {'v10 Sh':>8}  {'v11 Sh':>8}  "
          f"{'v12 Sh':>8}  {'v13 Sh':>8}  {'v12→v13':>8}  {'MaxDD_v13':>10}")
    print("─" * W)
    ...
    # v11/v12/v13 individual = v9 (positions unchanged)
    ...
    for name in indiv_labels + ['DYNAMIC_COMBINED']:
        ...
        if name == 'DYNAMIC_COMBINED':
            ...
            v13s = sims_v13_comb.get('sharpe')
            dd   = sims_v13_comb.get('max_dd')
        else:
        ...
        v13ss = f"{v13s:>+8.3f}" if v13s is not None else "        "
        dlt   = (f"{v13s - v12s:>+8.3f}"
                 if (v12s is not None and v13s is not None) else "        ")
        dds   = f"{dd:>10.3f}" if dd is not None else "          "
        label = name if name != 'DYNAMIC_COMBINED' else '─' * 4 + ' DYNAMIC_COMBINED'
        print(f"  {label:<26}  {v9ss}  {v10ss}  {v11ss}  {v12ss}  {v13ss}  {dlt}  {dds}")

    # ── Mean weights: v10 vs v11 vs v12 vs v13 ────────────────────────────
    print(f"\n── Mean weights [test]: v10 / v11 / v12 / v13 ──────────────────────────")
    print(f"  {'Strategy':<22}  {'v10 wt':>8}  {'v11 wt':>8}  {'v12 wt':>8}  "
          f"{'v13 wt':>8}  {'v12→v13':>8}  {'γ mode':>12}")
    print("─" * 88)
    for k, mode in tilt_modes.items():
        ...
        wt13 = cs_wts_v13[k][test_mask].mean()
        print(f"  {k:<22}  {wt10:>+8.3f}  {wt11:>+8.3f}  {wt12:>+8.3f}  "
              f"{wt13:>+8.3f}  {wt13-wt12:>+8.3f}  {mode:>12}")
    ...
    results = {
        "model":   "Crypto BSDT v13 — regime-conditional rolling Sharpe weights (γ_rank gate)",
        "version": "v13",
        "gamma_stats": {
    ...
    import os
    out_path = os.path.join(OUT_DIR, "crypto_bsdt_v13_results.json")
    ...
    # ── Summary ───────────────────────────────────────────────────────────
    print("\n" + "=" * 88)
    print("  SUMMARY: v9 → v10 → v11 → v12 → v13")
    print("=" * 88)
    print(f"  {'Version':<24}  {'Source':<46}  {'DYNAMIC Sharpe':>14}")
    print("─" * 88)
    summary_rows = [
        ...
        ('v12 (γ_rank tilt)',    'γ_rank=pct_rank(Ω_price+lev+cross), tilt_modes',      sims_v12_comb['sharpe']),
