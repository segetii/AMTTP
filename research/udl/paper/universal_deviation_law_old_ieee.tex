...
%  Title
% ============================================================
\title{Universal Deviation Principle: A Multi-Operator Framework for Anomaly Detection and Classification with Sensitivity Guarantees}
...
% ============================================================
\begin{abstract}
We introduce the \textbf{Universal Deviation Principle (UDP)}, a multi-operator framework for anomaly detection grounded in the thesis that an anomaly cannot simultaneously match normality under all structural perspectives.  We formalise this via \emph{deviation-separating operator families}---differentiable maps whose stacked Jacobian has full column rank---and prove six results under explicit assumptions (A1--A3): local/global sensitivity bounds, finite-sample guarantees, SDP-optimal fusion, greedy $(1{-}1/e)$-approximate operator selection, and operator-space separation.

The framework instantiates 14~composable spectrum operators across five mathematical families and introduces (i)~a \textbf{boundary-centred dimension magnifier} with adaptive $\tanh$ saturation paired with QDA, and (ii)~\textbf{SubspaceScan}, a random-projection ensemble with gated stretching for boundary anomalies.  We propose \textbf{top-$k$ coverage} as a complementary metric revealing that methods with comparable AUC exhibit markedly different retrieval profiles.

Across seven ODDS benchmarks with a \emph{single} hyperparameter set, the pipeline achieves mAUC~$0.910$ (five-fold CV: $0.925 \pm 0.020$ on Mammography), with the largest gain on Arrhythmia ($+6.4$~pp).  A correlation audit confirms only one redundant pair ($\rho > 0.85$) among 91~operator pairs.  We report prospective case studies on G-SIB banks, FRED indicators, and ERCOT grid data.

The framework extends to \textbf{multi-class classification} via per-class MDN decomposition, achieving mean accuracy~$0.936$ across six benchmarks---competitive with LightGBM ($0.934$)---using no gradient optimisation.  Code: \url{https://github.com/segetii/udp}.
\end{abstract}

\begin{IEEEkeywords}
Anomaly detection, multi-class classification, multi-spectrum representation, subspace boundary rescue, multi-operator fusion, adaptive gating, operator redundancy analysis, coverage metric, interpretable scoring, systemic anomaly monitoring
\end{IEEEkeywords}
...
We answer this with the \textbf{Universal Deviation Principle}: the hypothesis that anomalies are structured disturbances across multiple mathematical perspectives, and that multi-operator representation can improve sensitivity to such disturbances. This paper makes five contributions:

\begin{enumerate}
  \item It formalises a \textbf{deviation-separating operator family} and proves six conditional results on local/global sensitivity, finite-sample estimation, fusion-weight design, greedy operator selection, and operator-space separation.
  \item It defines a \textbf{multi-operator representation stack} built from five spectrum projection operators ($\Phi_{\text{stat}}$, $\Phi_{\text{chaos}}$, $\Phi_{\text{freq}}$, $\Phi_{\text{geom}}$, $\Phi_{\text{exp}}$), together with an anomaly tensor, centroid selection schemes, and a geometrically interpretable projection stage.
  \item It introduces two practical detection modules: a \textbf{Boundary-Centred Dimension Magnifier} that saturates informative dimensions while silencing blind ones, and \textbf{SubspaceScan} with adaptive gated stretching for weak distributed boundary anomalies.
  \item It evaluates the framework on public ODDS benchmarks, operator-level ablations, and coverage analysis, where the full boundary-rescue pipeline reaches \textbf{mAUC~$0.910$} across seven ODDS datasets and the operator audit shows low redundancy across 14 operators.
  \item It reports \textbf{prospective case studies} on banking, macroeconomic, and energy-infrastructure monitoring to illustrate how the framework behaves on structured multi-agent anomaly-detection problems.
\end{enumerate}

The entire pipeline uses a \textit{single set of hyperparameters} across the benchmark suite, with the same parameterisation carried into the prospective case studies.

\textbf{Intuition.} The core idea is simple: if someone is sick, a single blood test may miss the illness, but running blood work, an X-ray, and a stress test together makes evasion far harder.  Similarly, UDP projects every observation through $K$ mathematically distinct ``lenses''---statistics, dynamics, frequency, geometry, amplification---and flags it as anomalous whenever \emph{any} lens registers abnormality.  The theorems below make this intuition precise: under mild regularity (A1--A3), the stacked operator map is locally injective, so any displacement from the normal reference must produce a measurable response in at least one operator, with a quantitative lower bound proportional to the displacement magnitude.
...
\item[A2 (Operator-family richness)] The stacked Jacobian has full column rank (Definition~1).  This fails if operators are collectively redundant or insensitive on certain subspaces; we verify it empirically via Monte Carlo rank checks on the numerical Jacobian.
...
\textbf{Theorem 1 (Operator Sensitivity --- Local Form).} \textit{Let $\{\Phi_k\}_{k=1}^K$ be a deviation-separating family at $\boldsymbol{\mu}$ with each $\Phi_k \in C^1$. Then the stacked map $\Phi = (\Phi_1, \ldots, \Phi_K)$ is locally injective at $\boldsymbol{\mu}$: there exists an open neighbourhood $U \ni \boldsymbol{\mu}$ such that for every $\mathbf{x} \in U \setminus \{\boldsymbol{\mu}\}$, there exists $k$ with $\Phi_k(\mathbf{x}) \neq \Phi_k(\boldsymbol{\mu})$. A quantitative lower bound holds:}
...
\textit{where $\sigma_{\min} = \sigma_{\min}(J(\boldsymbol{\mu})) > 0$. (Proof via the Inverse Function Theorem; see Appendix~\ref{app:proofs}.)}

\vspace{0.5em}

\textbf{Theorem 2 (Operator Sensitivity --- Global Form).} \textit{Let $\mathcal{K} \subset \R^m$ be compact with $\text{rank}(J(\mathbf{x})) = m$ for all $\mathbf{x} \in \mathcal{K}$, and each $\Phi_k \in C^2$. Define $\underline{\sigma} := \min_{\mathbf{x} \in \mathcal{K}} \sigma_{\min}(J(\mathbf{x})) > 0$ (positive by the Extreme Value Theorem). Then for any set $\mathcal{A} \subset \mathcal{K}$:}
...
\textit{where $C = \frac{1}{2}\sup_{\mathbf{z} \in \mathcal{K}} \norm{D^2 \Phi(\mathbf{z})}$ is finite by continuity on the compact set. (Proof in Appendix~\ref{app:proofs}.)}
...
\textit{(Proof via Hoeffding concentration with union bound; see Appendix~\ref{app:proofs}.)}

\textbf{Corollary 1 (Sample complexity).} The bound~\eqref{eq:finite_sample} is non-trivial whenever $n > 2 L^2 K \cdot \text{tr}(\Sigma) \cdot \ln(2m/\delta) / (\underline{\sigma}^2 \cdot \norm{\mathbf{x} - \boldsymbol{\mu}}^2)$.
...
\textit{is concave on the simplex $\Delta_K$ (as an infimum of linear functions). Maximising $\eta$ over $\mathbf{w} \in \Delta_K$ yields the weight vector that makes the weakest detection direction as strong as possible, solvable as a standard SDP. (Proof in Appendix~\ref{app:proofs}.)}
...
\textit{(Submodularity follows from the identity $\log\det(A+B) - \log\det(A) = \log\det(I + A^{-1/2}BA^{-1/2})$, which yields diminishing returns as $A$ grows. The $(1-1/e)$ bound follows from Nemhauser et al.\ (1978). Proof in Appendix~\ref{app:proofs}.)}

\vspace{0.5em}

\textbf{Operational form.} Let $\epsilon_k$ be the $(1-\alpha)$ quantile of $\norm{\Phi_k(\mathbf{x}) - \Phi_k(\boldsymbol{\mu})}$ under $P_{\mathcal{N}}$. Then an observation is flagged anomalous if $\exists\, k$ such that $\norm{\Phi_k(\mathbf{x}) - \Phi_k(\boldsymbol{\mu})} > \epsilon_k$.  Note that Theorems~1--2 guarantee that any displaced point produces an operator response, but this response must \emph{exceed} the normal-data baseline $\epsilon_k$ for detection.  This gap is non-trivial only when the anomaly's displacement is large enough relative to the normal-data spread---a condition that A1 (observability) ensures in principle, and that the quantitative bounds (Eqs.~\eqref{eq:quantitative}--\eqref{eq:finite_sample}) make precise.  All proofs are deferred to Appendix~\ref{app:proofs}.

\subsection{From Sensitivity to Detection}
\label{ssec:detection}

Theorems~1--3 are \emph{sensitivity} (injectivity) statements: they guarantee that the stacked operator map responds to any displacement from $\boldsymbol{\mu}$, including displacements of normal points.  To bridge from sensitivity to detection, we need a score that is \emph{large for anomalies and small for normals}.  We define:
...
where $\Phi(\mathbf{x}) = (\Phi_1(\mathbf{x}), \ldots, \Phi_K(\mathbf{x}))$ is the joint operator map and $\mathcal{N}$ is the normal training set.  For normal points $\mathbf{x} \in \mathcal{N}$, $s(\mathbf{x}) = 0$ by construction.  The theorem below gives conditions under which anomalies receive $s(\mathbf{x}) > 0$.

\textbf{Theorem 6 (Operator-Space Separation).} \textit{Let $\mathcal{N} \subset \R^m$ be compact and each $\Phi_k \in C^1$.  Suppose the family $\{\Phi_k\}$ is deviation-separating (Definition~1) at every $\boldsymbol{\mu} \in \mathcal{N}$, and let $\underline{\sigma} := \min_{\boldsymbol{\mu} \in \mathcal{N}} \sigma_{\min}(J(\boldsymbol{\mu})) > 0$.  If $\mathbf{x} \notin \mathcal{N}$ with $d := \inf_{\mathbf{z} \in \mathcal{N}}\norm{\mathbf{x} - \mathbf{z}} > 0$ and $d$ is sufficiently small that the first-order Taylor remainder is dominated, then:}

\begin{equation}
s(\mathbf{x}) \geq \underline{\sigma} \cdot d - O(d^2)
\label{eq:separation}
\end{equation}

\textit{In particular, for any $\mathbf{x}$ sufficiently close to $\mathcal{N}$ but not in $\mathcal{N}$, $s(\mathbf{x}) > 0$, and there exists $k$ such that $s_k(\mathbf{x}) \geq \underline{\sigma} \cdot d / \sqrt{K} - O(d^2)$.  (Proof: apply Theorem~1 at the nearest point $\mathbf{z}^* \in \mathcal{N}$.  See Appendix~\ref{app:proofs}.)}

\textbf{Remark.} Theorem~6 converts the injectivity guarantee (Theorem~1) into a detection statement: if $\mathbf{x}$ is geometrically separated from $\mathcal{N}$ in input space and the operator family has full-rank Jacobian everywhere on $\mathcal{N}$, then $\mathbf{x}$ is also separated in operator space.  The assumption that $\mathbf{x} \notin \mathcal{N}$ is non-trivial---it is precisely A1 (observability).  Without A1, no operator family can distinguish $\mathbf{x}$ from normal data.
...
where $d_C$ is Cohen's $d$ (capped at $3.0$, corresponding to the ``huge effect'' threshold in Cohen's convention, thereby normalising to $[0,1]$), overlap is the distributional overlap fraction, and $AUC_d$ is the per-dimension AUC.  The component weights ($0.4$, $0.3$, $0.3$) give slight precedence to effect size as the most direct measure of class separation; cross-dataset sweeps confirm that any convex combination in the range $[0.25, 0.50]$ per component yields $<0.005$ AUC variation on Mammography.  The global steepness multiplier $\gamma = 5.0$ was selected via cross-validated grid search over $\gamma \in \{2, 3, 4, 5, 6, 8\}$ with 3-fold inner stratified CV across five ODDS datasets; the search consistently selects values in $[4, 6]$, confirming that $5.0$ sits on a stable plateau.  The overall scoring architecture---discriminative gating, persistence-based signal selection, and the decomposition into effect-size, overlap, and AUC components---is designed to anchor scoring to empirically persistent signals: components with stable discriminative power across cross-validation folds receive higher weight, ensuring robustness to perturbations of the training data.  The specific numerical values are calibrated empirically within this framework.
...
where $z_j^{+} = \max(0, z_j)$ is the positive-only standardised score, $\gamma_s = 3.0$ controls steepness, and $\alpha_j$ is a normalising constant. The stretch steepness $\gamma_s = 3.0$ was selected from dedicated A/B benchmarks comparing $\gamma_s \in \{3, 5\}$ across seven ODDS datasets: $\gamma_s{=}3$ consistently outperforms $\gamma_s{=}5$ (e.g., Pendigits: $0.939$ vs $0.935$; Arrhythmia: $0.699$ vs $0.689$).  The gate threshold ($0.4$) is positioned at medium discriminability---the midpoint between clearly separable ($d_j > 0.5$, raw Mahalanobis preserved) and weakly separable ($d_j < 0.3$, full stretching applied)---and the steepness ($20$) produces a sharp but smooth sigmoid transition between these regimes.  This gating mechanism preserves subspaces whose discriminative signal is empirically robust (stable across resampling) while stretching subspaces with weak signal.  The gate constants themselves are calibrated empirically: stability analysis across all seven ODDS benchmarks confirms that thresholds in $[0.3, 0.5]$ and steepness in $[15, 30]$ yield $<0.005$~pp AUC variation.

\textbf{BoundaryRescue and SubMF-CombA pipeline.} The \textbf{BoundaryRescue} strategy wraps SubspaceScan with adaptive gated stretching as a dedicated scorer for TYPE-1 BOUNDARY anomalies. The full \textbf{SubMF-CombA} pipeline fuses MetaFusion (the existing multi-strategy QDA/Fisher/RankFusion ensemble) with BoundaryRescue via CombA (Combined Average) rank fusion~\cite{fox1994combination}: ranks from both pipelines are averaged, creating a combined ranking that benefits from both per-dimension magnification and cross-subspace signal aggregation.

\subsection{System Mode Aggregation}
\label{ssec:system_mode}

The operators defined above produce a per-observation anomaly score $s_i \in \R$.  For multi-agent, time-indexed domains (e.g., a panel of banks observed quarterly), we aggregate these scores into a \textbf{System Mode} signal.  Given $A$ agents observed at time~$t$, the system-level score is

\begin{equation}
\bar{s}(t) = \frac{1}{A}\sum_{a=1}^{A} s_a(t)
\label{eq:system_score}
\end{equation}

where $s_a(t)$ is the UDP anomaly score for agent~$a$ at time~$t$.  To convert this aggregate score into an alarm signal without look-ahead, we apply a Basel-style rolling z-score:

\begin{equation}
z(t) = \frac{\bar{s}(t) - \hat{\mu}_{0:t}}{\hat{\sigma}_{0:t}}
\label{eq:zscore}
\end{equation}

where $\hat{\mu}_{0:t}$ and $\hat{\sigma}_{0:t}$ are the expanding-window mean and standard deviation of $\bar{s}$ computed from the calibration origin up to~$t$ (no future information).  An alarm fires when $z(t) > z_{\text{thresh}}$ (default $z_{\text{thresh}} = 2.0$).  This strictly prospective protocol ensures that the system could be deployed in real-time surveillance without any look-ahead bias.  Section~\ref{ssec:real_world} evaluates System Mode on G-SIB banks, FRED macroeconomic indicators, and ERCOT grid data.

\subsection{Connection to Blind Spot Decomposition Theory}
\label{sec:bsdt}

BSDT~\cite{odeyemi2025bsdt} decomposed detection failures into four orthogonal components ($C, G, A, T$) and demonstrated cross-domain correction.  UDP generalises BSDT from a post-model scalar correction to a model-independent multi-dimensional tensor framework with $K$ composable operators and full MDN decomposition.  BSDT can be integrated as an additional operator $\Phi_{\text{BSDT}}(\mathbf{x}) = [C(\mathbf{x}),\; G(\mathbf{x}),\; A(\mathbf{x}),\; T(\mathbf{x})] \in \R^4$, yielding a 6-operator stack.  Empirical evaluation (Section~\ref{ssec:bsdt_integration}) shows this achieves the highest average AUC when combined with the standard 5-operator representation.
...
\textbf{Datasets.} We evaluate on thirteen datasets spanning six application domains, comprising ten tabular benchmarks and three real-world sensitive-sector datasets~\cite{han2022adbench}:

\begin{table}[!t]
\centering
\caption{Benchmark and Real-World Datasets}
\label{tab:datasets}
\begin{tabular}{@{}lrrrl@{}}
\toprule
\textbf{Dataset} & $N$ & $m$ & \textbf{Anom.\%} & \textbf{Domain} \\
\midrule
\multicolumn{5}{@{}l}{\textit{ODDS Benchmarks}} \\
...
\midrule
\multicolumn{5}{@{}l}{\textit{Real-World Sensitive Sectors}} \\
G-SIB Panel & 1,900 & 5 & 6.8\% & Banking (25 banks) \\
Economy (FRED) & 948 & 6 & 9.5\% & Macroeconomic \\
ERCOT Grid & 15,665 & 5 & 37.4\% & Energy infrastructure \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Protocol.} All ODDS experiments use a single hyperparameter configuration: exponential amplification $\alpha = 1.0$, score weights $(w_r, w_\theta) = (0.7, 0.3)$, centroid method = auto, magnifier steepness $\gamma = 5.0$, SubspaceScan stretch $\gamma_s = 3.0$. Data is split 70/30 with stratified sampling and fixed random seed (42). Five-fold cross-validation is reported for generalisation assessment.  The real-world evaluation (Section~\ref{ssec:real_world}) uses an entirely separate prospective protocol with no future information.
...
\midrule
\textbf{Mean (incl.~sanity checks)} & 0.943$^\dagger$ & --- & --- & --- & --- \\
\textbf{Mean (real-world only)} & \textbf{0.904} & --- & --- & --- & --- \\
\bottomrule
\end{tabular}
\begin{tablenotes}\small
  \item[$\dagger$] Synthetic and Mimic are author-constructed sanity checks where AUC$\,{=}1.000$ by design.
  Real-world mean covers Mammography ($0.947$) + Shuttle ($0.989$) + Pendigits ($0.777$) only.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\begin{itemize}
  \item \textbf{Synthetic camouflage sanity check}: Both standard and mimic anomalies are separated with AUC$=1.000$. We treat these synthetic tasks primarily as sanity checks showing that the representation can recover partially camouflaged signal, rather than as substitutes for public benchmark evidence.
  \item \textbf{Strong performance on public tabular benchmarks}: QDA-Magnified achieves 0.947 on Mammography (exceeding the established baselines in Fig.~\ref{fig:comparison}) and 0.989 on Shuttle, using the same hyperparameters and no per-domain tuning.
...
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Operator} & \textbf{Synth.} & \textbf{Mimic} & \textbf{Pendg.} & \textbf{Mamm.} \\
\midrule
...
\textbf{No single operator dominates.} Geometric distance is the strongest individual operator in 3/4 real-world domains, yet:
\begin{itemize}
  ...
  \item Spectral analysis perfectly separates synthetic offsets (1.000) but is weak on real data (0.541--0.668), illustrating that performance can vary substantially across operator families and motivating complementary multi-operator aggregation.
  \item On this benchmark slice, the combined score matches or exceeds the best individual operator in every dataset, supporting the view that aggregating complementary operators can improve robustness over any single mathematical perspective.
\end{itemize}

Taken together, these results support the motivating hypothesis behind the Universal Deviation Principle: different operators exhibit different blind spots, and combining them can improve detection robustness.
...
We compare four input modes to evaluate how BSDT components interact with the UDP framework:
...
All operators preserve per-feature information through PCA compression ($d_{\text{pca}} = 12$) and are composable with the base stack.
...
Among 91~unique operator pairs, \textbf{only one pair exceeds the $\rho > 0.85$ redundancy threshold}: Wavelet~$\leftrightarrow$~Phase ($\rho = 0.924$). All other pairs exhibit $\rho \leq 0.78$, with most below $0.50$. This pattern is consistent with the Universal Deviation Principle's motivating hypothesis: different mathematical perspectives capture partially distinct aspects of deviation, and the multi-spectrum representation is not dominated by redundant views.
...
To position UDP against the state of the art, we compare against methods at three supervision levels.  Data is split 70/30 with stratified sampling (seed~42). StandardScaler is fit on normal training data only.

\textbf{Label usage clarification.} UDP's centroid $\boldsymbol{\mu}$ and scaler are computed from normal training data only (no anomaly labels needed). However, the Dimension Magnifier (Eq.~\eqref{eq:disc_score}) requires per-class means $\mu_d^{(0)}, \mu_d^{(1)}$ and the QDA classifier (Eq.~\eqref{eq:qda}) requires per-class covariances---both computed from the labelled training split. This makes QDA-Magnified a \textbf{supervised} method. The Fisher LDA baseline (Eq.~\eqref{eq:fisher}) similarly requires class labels for the between-class scatter matrix $S_B$. The unsupervised UDP score (Eq.~\eqref{eq:score}) based on magnitude and novelty does not require labels and can serve as a fully unsupervised detector, though with lower AUC than the supervised variants.
...
\begin{table*}[t]
\centering
\caption{Comparison across supervision levels (AUC-ROC).  \textbf{S}=supervised (uses anomaly labels in training), \textbf{U}=unsupervised (normal data only).  UDP-RankFusion is the label-free variant; UDP-QDA-Magnified and UDP-Hybrid use both-class labels. XGBoost and LightGBM are gradient-boosted tree baselines trained on the identical labelled splits.}
\label{tab:sota}
\begin{threeparttable}
...
\textbf{UDP-Hybrid (4-op)} & S & 0.999 & 0.999 & 0.911 & \textbf{0.991} & 0.960 & \textbf{0.972} \\
\textbf{UDP-QDA-Magnified} & S & \textbf{1.000} & \textbf{1.000} & 0.925 & 0.989 & 0.777 & 0.938 \\
\midrule
\multicolumn{8}{l}{\textit{Unsupervised (normal data only)}} \\
\textbf{UDP-RankFusion} & U & 0.998 & 0.997 & 0.890 & 0.985 & 0.953 & 0.963 \\
...
UDP-Fisher (5-op) & S & 1.000 & 1.000 & 0.901 & 0.889 & --- & 0.947 \\
...
COPOD~\cite{li2020copod} & U & 1.000 & 1.000 & 0.918 & 0.850 & 0.709 & 0.895 \\
ECOD~\cite{li2022ecod} & U & 1.000 & 0.994 & 0.922 & 0.750 & 0.718 & 0.877 \\
HBOS~\cite{goldstein2012histogram} & U & 1.000 & 0.998 & 0.834 & 0.768 & 0.488 & 0.818 \\
\bottomrule
\end{tabular}
\begin{tablenotes}\small
  \item[$^\star$] LightGBM on Pendigits reported as 0.9997; near ceiling.  XGBoost/LightGBM 5-fold CV.
\end{tablenotes}
\end{threeparttable}
\end{table*}

\textbf{Head-to-head with supervised tabular classifiers.}  We benchmarked UDP against XGBoost~\cite{chen2016xgboost} (200~trees, depth~6) and LightGBM~\cite{ke2017lightgbm} (same) under identical 5-fold stratified CV on five ODDS datasets.  Results are reported in Table~\ref{tab:supervised}.

\begin{table}[!t]
\centering
\caption{UDP standalone vs supervised baselines: AUC-ROC (5-fold CV). UDP methods use \emph{only} UDP components (Stack, Magnifier, QDA); XGBoost and LightGBM are external baselines on raw features.}
\label{tab:supervised}
\begin{tabular}{@{}lccccc@{}}
\toprule
\textbf{Method} & \textbf{Mamm.} & \textbf{AnnThy.} & \textbf{PenDig.} & \textbf{Satell.} & \textbf{Arrhyth.} \\
\midrule
\multicolumn{6}{@{}l}{\textit{UDP (ours)}} \\
UDP-Opt (QDA)     & 0.925 & 0.863 & 0.983 & 0.952 & 0.767 \\
\midrule
...
Three findings emerge.  \textbf{(i)}~UDP-Opt (closed-form QDA head, no gradient optimisation) achieves $0.925$ on Mammography, within $0.2$~pp of LightGBM ($0.927$); this indicates that the multi-spectrum deviation features carry competitive discriminative signal on low-dimensional data even with a simple closed-form head.  \textbf{(ii)}~UDP-Opt reaches $0.983$ on PenDigits and $0.952$ on Satellite, confirming strong performance across diverse data geometries.  \textbf{(iii)}~On Arrhythmia ($D{=}274$), the $274{\to}32$ dimensional compression that UDP operators impose causes information loss: UDP-Opt drops to $0.767$ while LGB-raw retains $0.880$.  This confirms UDP's high-dimensional scaling limitation (Section~\ref{ssec:limits}).

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{figures/udl_vs_baselines_bar.pdf}
\caption{UDP standalone methods vs supervised baselines across four ODDS benchmarks (5-fold CV). UDP-QDA-Opt (dark blue) matches or approaches tree baselines on Mammography and Pendigits while using only closed-form QDA with a single hyperparameter. XGBoost and LightGBM (warm colours) run independently on raw features---they are \emph{not} mixed into the UDP pipeline.}
\label{fig:udl_vs_baselines}
\end{figure}

\begin{figure}[!t]
\centering
\includegraphics[width=0.85\columnwidth]{figures/udl_mAUC_summary.pdf}
\caption{Mean AUC-ROC summary across four ODDS datasets. UDP-QDA-Opt achieves mAUC~$0.931$, the strongest UDP variant, vs LightGBM's~$0.968$. The $3.7$~pp gap is attributable to trees' capacity for non-linear feature interaction on raw inputs---a capability UDP trades for interpretable per-operator attribution and theoretical guarantees.}
\label{fig:udl_mauc}
\end{figure}

Summary: on moderate-dimensional tabular data with abundant labels, gradient-boosted trees on raw features are hard to beat.  UDP's value proposition is \textbf{not} raw AUC supremacy but rather: (a)~per-operator attribution that decomposes \emph{why} an observation is anomalous, (b)~graceful degradation to a competitive unsupervised detector (RankFusion, mAUC~0.963) when labels vanish, (c)~domain-agnostic composability requiring no gradient optimisation, and (d)~theoretical guarantees (Theorems~1--4) that no other tabular method provides.

Table~\ref{tab:sota} reports the results. Key findings:

\begin{itemize}
  \item \textbf{UDP-Hybrid achieves the highest mean AUC (0.972)} across all five datasets, including Pendigits where it reaches 0.960---a substantial improvement over QDA-Magnified's 0.777 on the same dataset. Hybrid auto-selection adapts the projection strategy per dataset, avoiding Fisher's failure mode on high-dimensional data.
  \item \textbf{Competitive with gradient-boosted trees.} On Mammography, UDP-QDA-Magnified ($0.925$) matches LightGBM ($0.927$) using only a closed-form QDA head (Table~\ref{tab:supervised}).
  \item \textbf{Magnification transforms UDP from competitive to dominant.} Without the magnifier, Fisher-based UDP scores 0.901 on Mammography and 0.889 on Shuttle. QDA-Magnified lifts these to 0.925 and 0.989 respectively---demonstrating that the representation contains untapped discriminative structure that non-linear projection unlocks.
  \item \textbf{Cross-validation confirms robustness.} Five-fold CV on Mammography yields $0.925 \pm 0.020$ for QDA-Magnified vs\ $0.896 \pm 0.014$ for Fisher (Table~\ref{tab:cv}).
...
\caption{AUC-ROC comparison on Mammography: QDA-Magnified vs established baselines under identical semi-supervised protocol. UDP-QDA-Magnified (red) achieves the highest AUC (0.947), exceeding the next best established method ($k$-NN Distance, 0.876) by $+0.071$.}
...
\caption{5-fold cross-validation on Mammography: QDA-Magnified vs Fisher LDA. QDA-Magnified achieves higher mean AUC with lower variance, confirming robust generalisation ($0.925 \pm 0.020$).}
\label{fig:cv}
\end{figure}
...
\subsection{Real-World Sensitive-Sector Evaluation}
\label{ssec:real_world}

Beyond standard ODDS benchmarks, we evaluate UDP's System Mode on three real-world datasets spanning finance and energy infrastructure, using protocols designed for actual deployment.

\subsubsection{G-SIB Bank-Level Crisis Detection}
\label{sssec:gsib}

Twenty-five systemically important banks are tracked quarterly from 2005\,Q1 to 2023\,Q4 ($T{=}76$~quarters). Six~US banks use individual call-report filings from the FDIC Surveillance and Data~Infrastructure; 19~non-US banks use country-level banking-sector aggregates from the World~Bank Global Financial Development Database (GFDD)~\cite{worldbank2024gfdd}. The feature space is $d{=}5$: loan-to-asset ratio, equity ratio, NPL ratio, return on assets, and funding cost. Crisis labels (used \emph{only} for post-hoc evaluation) comprise 13~quarters from objective published sources: seven~GFC quarters (2007\,Q4--2009\,Q2, per the NBER Business~Cycle Dating Committee), two~COVID quarters (2020\,Q1--Q2), and four European sovereign-debt stress quarters (2011\,Q3--2012\,Q2).

\textbf{Prospective protocol.} The experiment simulates real-time supervisory deployment with no look-ahead: (i)~the detection engine is calibrated on 275~bank-quarter observations from 2005\,Q1--2007\,Q3 (the Great Moderation period); (ii)~at each new quarter~$t$, the engine is re-fitted on an expanding window up to~$t$ with \emph{no crisis labels}; (iii)~a Basel-style rolling z-score $z(t) = (s(t) - \bar{s}_{0:t})/\sigma_{s,\,0:t}$ fires an alarm when $z > 2.0$.  We present this study as a prospective case study rather than a substitute for standard public benchmarks.

The UDP Hybrid engine first fires at 2007\,Q1 ($z{=}+2.65$), flagging the Bear~Stearns hedge-fund collapse as anomalous---a full \textbf{6~quarters before Lehman Brothers failed} and 3~quarters before the NBER recession started. It peaks at $z{=}+8.65$ at the BNP~Paribas fund freeze quarter, the single largest deviation in the entire 19-year panel. No alarm fires during the COVID shock (2020\,Q1--Q2), suggesting that the engine is more sensitive to endogenous balance-sheet stress than to exogenous macro shocks within this feature space. FAR remains at $1.6\%$ on this case study.

We note that the UDP architecture supports several alternative engine configurations---including \textbf{Gravity} (inter-agent gravitational coupling) and \textbf{Molecular} (kinetic energy and thermal equilibrium) formulations---which provide complementary detection profiles. A detailed comparative study of these engine variants, including per-bank ablations and multi-engine fusion strategies, will be presented in a forthcoming companion paper.

At the Lehman quarter (2008\,Q3), per-bank scores are geographically coherent: Goldman~Sachs ranks highest (0.567), BNY~Mellon second (0.219), UK banks cluster at 0.142, while Chinese state banks are least stressed (0.046)---all consistent with known GFC loss geography. Goldman was already flagged at 0.772 in 2007\,Q3, \textbf{twelve months before Lehman}, without any information about Goldman's trading positions.

\subsubsection{Economy and Energy Grid}
\label{sssec:economy_ercot}

\begin{table}[!t]
\centering
\caption{Economy (FRED) and ERCOT Grid Failure Detection}
\label{tab:economy_ercot}
\begin{tabular}{@{}lccccc@{}}
\toprule
\textbf{Dataset} & \textbf{AUC} & \textbf{FAR} & \textbf{F1} & \textbf{Prec.} & \textbf{Recall} \\
\midrule
Economy (FRED) & \textbf{0.9999} & \textbf{0.1\%} & 0.995 & 0.991 & 1.000 \\
ERCOT Grid & \textbf{0.9940} & 3.9\% & 0.950 & 0.938 & 0.963 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Economy (FRED).} Eleven quarterly macroeconomic series projected through a $12{\times}6$-dimensional sector-weight matrix to create 12~synthetic bank-sector agents ($N{=}948$ samples). UDP achieves AUC~$= 0.9999$ and FAR~$= 0.1\%$ on this constructed case study: all nine crisis quarters are detected with one false alarm.

\textbf{ERCOT Texas Grid (Feb 2021).} Sixty-five heterogeneous agents (gas, wind, thermal, consumer, supply) simulated over 241~hourly steps using real ERCOT capacity-loss, temperature, and demand data from Winter Storm~Uri. The system provides \textbf{37~hours of lead time} before peak crisis (first alarm at hour~71, peak at hour~108), with no false alarms during the normal-operation segment of this simulation and 100\% alarm rate during all four crisis phases. Together, these case studies illustrate how the framework behaves on structured multi-agent monitoring problems, but they should not be interpreted as replacements for standard benchmark evaluation.
...
% ============================================================
%  Section 6: Classification via Per-Class MDN Decomposition
% ============================================================
\section{Classification via Per-Class MDN Decomposition}
\label{sec:classification}

The Universal Deviation Principle extends naturally from anomaly detection to multi-class classification.  In anomaly detection, UDP asks ``how does $x$ deviate from the normal reference across $K$ operator perspectives?''  For classification, we ask ``how does $x$ deviate from \emph{each class} across $K$ perspectives?''  The Magnitude-Direction-Novelty~(MDN) decomposition provides the machinery.

For each class $c \in \{1, \ldots, C\}$, a dedicated \texttt{RepresentationStack}~$\Phi^{(c)}$ is fitted on class~$c$'s training data, calibrating the $K$~operators to that class's distribution.  An \texttt{AnomalyTensor}~$\mathcal{T}^{(c)}$ is then fitted on the class-specific representations, capturing centroid~$\mu_c$ and reference directions.  At prediction time, every sample is decomposed through each class's tensor, yielding a compact feature vector:
\begin{equation}
  F(x) = \bigoplus_{c=1}^{C} \bigl[\, r_c(x),\; m_{c,1}(x), \ldots, m_{c,K}(x),\; \theta_c(x) \,\bigr]
         \;\in\; \mathbb{R}^{C(K{+}2)}
  \label{eq:classification}
\end{equation}
where $r_c$ is the overall deviation magnitude from class~$c$, $m_{c,k}$ is the per-law magnitude for operator~$k$, and $\theta_c$ is the directional novelty.  With $K{=}6$ core operators, each class contributes 8~features: 24 for a 3-class problem or 80 for 10~classes---far more compact than concatenating the full $C \times D$ deviation profiles.

Theorem~1 provides the theoretical basis: if the stacked operator map has full-rank Jacobian, distinct inputs map to distinct deviation profiles.  When operators are calibrated \emph{per class}, samples from class~$c$ produce near-zero MDN features for class~$c$ (small deviation from own centroid) but large features for other classes, creating separable clusters in MDN space.  A lightweight classifier head (QDA, Logistic Regression, or Random Forest) is trained on these features with no gradient optimisation.

\begin{table*}[t]
\centering
\caption{Classification accuracy (5-fold stratified CV, mean $\pm$ std).  Best per dataset in \textbf{bold}; best UDP variant \underline{underlined}.  UDP uses per-class MDN features ($C{\times}8$ dimensions); baselines operate on standardised raw features.}
\label{tab:classification}
\begin{tabular}{@{}l ccc ccccc@{}}
\toprule
\textbf{Dataset} ($n$, $d$, $C$)
  & \textbf{UDP-QDA} & \textbf{UDP-Log.} & \textbf{UDP-RF}
  & \textbf{RF-200} & \textbf{SVM} & \textbf{$k$NN-5}
  & \textbf{Logistic} & \textbf{LightGBM} \\
\midrule
Iris (150, 4, 3)
  & \underline{.960{\scriptsize$\pm$.033}} & .960{\scriptsize$\pm$.033} & .960{\scriptsize$\pm$.025}
  & .960{\scriptsize$\pm$.039} & .960{\scriptsize$\pm$.039} & \textbf{.973{\scriptsize$\pm$.025}}
  & .953{\scriptsize$\pm$.045} & .953{\scriptsize$\pm$.034} \\
Wine (178, 13, 3)
  & .972{\scriptsize$\pm$.018} & \underline{\textbf{.989{\scriptsize$\pm$.014}}} & .983{\scriptsize$\pm$.014}
  & .978{\scriptsize$\pm$.021} & .983{\scriptsize$\pm$.022} & .972{\scriptsize$\pm$.018}
  & .983{\scriptsize$\pm$.014} & .972{\scriptsize$\pm$.031} \\
BreastCancer (569, 30, 2)
  & .951{\scriptsize$\pm$.012} & \underline{.958{\scriptsize$\pm$.009}} & .868{\scriptsize$\pm$.045}
  & .954{\scriptsize$\pm$.010} & \textbf{.977{\scriptsize$\pm$.016}} & .963{\scriptsize$\pm$.018}
  & .970{\scriptsize$\pm$.020} & .967{\scriptsize$\pm$.018} \\
Digits (1797, 64, 10)
  & .951{\scriptsize$\pm$.007} & .646{\scriptsize$\pm$.053} & \underline{.960{\scriptsize$\pm$.007}}
  & .980{\scriptsize$\pm$.005} & \textbf{.984{\scriptsize$\pm$.006}} & .979{\scriptsize$\pm$.006}
  & .971{\scriptsize$\pm$.004} & .974{\scriptsize$\pm$.004} \\
Vehicle (846, 18, 4)
  & .765{\scriptsize$\pm$.015} & \underline{\textbf{.794{\scriptsize$\pm$.021}}} & .771{\scriptsize$\pm$.030}
  & .746{\scriptsize$\pm$.016} & .745{\scriptsize$\pm$.023} & .718{\scriptsize$\pm$.018}
  & .788{\scriptsize$\pm$.016} & .754{\scriptsize$\pm$.018} \\
Segment (2310, 19, 7)
  & .936{\scriptsize$\pm$.004} & .917{\scriptsize$\pm$.058} & \underline{.952{\scriptsize$\pm$.009}}
  & .977{\scriptsize$\pm$.005} & .940{\scriptsize$\pm$.009} & .936{\scriptsize$\pm$.008}
  & .932{\scriptsize$\pm$.013} & \textbf{.984{\scriptsize$\pm$.006}} \\
\midrule
\textbf{mAcc (per-method)}
  & .923 & .877 & .916
  & .933 & .932 & .923
  & .933 & .934 \\
\textbf{mAcc (best UDP)}
  & \multicolumn{3}{c}{\underline{.936}}
  & --- & --- & ---
  & --- & --- \\
\bottomrule
\end{tabular}
...
\textbf{(i)}~UDP achieves the highest accuracy on \textbf{Wine} ($0.989$, UDP-Logistic) and \textbf{Vehicle} ($0.794$, UDP-Logistic), demonstrating that per-class MDN features can surpass strong baselines on moderate-dimensional data.  On Wine, the $+0.6$~pp margin over the best baseline (SVM, $0.983$) is notable given the compact 24-feature MDN representation.
\textbf{(ii)}~On high-dimensional data (Digits, $d{=}64$, $C{=}10$), UDP-RF reaches $0.960$ using 80~MDN features, competitive with SVM ($0.984$) and RF-200 ($0.980$).  No single head dominates: QDA excels on low-dimensional problems (Iris, BreastCancer), Logistic on moderate-dimensional (Wine, Vehicle), and RF on high-dimensional or many-class data (Digits, Segment).
\textbf{(iii)}~Selecting the best head per dataset, UDP achieves mean accuracy \textbf{0.936}---competitive with LightGBM ($0.934$), RF-200 ($0.933$), and Logistic ($0.933$), and above SVM ($0.932$) and $k$NN ($0.923$)---using only $C \times 8$ MDN features with no gradient optimisation.  This dual-use capability---competitive anomaly detection \emph{and} classification from the same multi-spectrum operators---validates the representation quality predicted by Theorem~1.
...
The Universal Deviation Principle formalises an observation that practitioners have long exploited intuitively: combining perspectives can improve detection. Our contribution is to make this precise:

\begin{enumerate}
  \item The \textbf{Universal Deviation Principle} provides conditional guarantees (Theorems~1--6) under explicit assumptions (A1--A3). Unlike OOD detection~\cite{hendrycks2017baseline,liang2018odin,liu2020energy}, which produces a single ``in vs.\ out'' score, UDP decomposes \textit{which operator} makes an observation anomalous.
  \item The \textbf{Representation Stack} (Eq.~\eqref{eq:stack}) provides a composable, extensible architecture where new operators can be added without modifying existing ones. This modularity distinguishes UDP from ensemble methods~\cite{lazarevic2005feature} that combine detectors of the same type.
  ...
  \item The \textbf{real-world sensitive-sector evaluation} (Section~\ref{ssec:real_world}) provides prospective case studies on banking, macroeconomic, and energy-infrastructure monitoring.  These studies illustrate how the framework behaves under expanding-window supervision and multi-agent aggregation, complementing the public benchmark evaluation.
  \item The \textbf{classification extension} (Section~\ref{sec:classification}) demonstrates that the same multi-spectrum representation extends to supervised classification via per-class MDN decomposition, achieving mean accuracy $0.936$ across six standard benchmarks---competitive with gradient-boosted trees---using the compact $C{\times}(K{+}2)$ MDN feature representation.
\end{enumerate}

\subsection{Practical Implications}

\textbf{Cross-domain portability.} The same hyperparameters work across medical imaging, aerospace, handwriting recognition, financial crisis detection, macroeconomic monitoring, and energy grid resilience. This reduces---though does not eliminate---the amount of domain-specific retuning needed to configure the detector.

\textbf{Interpretability.} Unlike black-box autoencoders, every component of the UDP pipeline produces interpretable quantities: which operator is most violated, how the deviation decomposes along the hyperplane, and which operators co-activate.  In the G-SIB evaluation (Section~\ref{sssec:gsib}), per-bank scores at the Lehman quarter produce a geographically coherent stress ranking that a supervisor can inspect and compare against known stress geography.
...
\textbf{Coverage over AUC.} The coverage analysis (Section~\ref{sssec:coverage}) reveals that AUC alone is an insufficient evaluation metric. A method with strong AUC can still miss the majority of anomalies at practical operating thresholds. We therefore recommend reporting both AUC (ranking quality) and top-$k$ coverage (actionable detection rate), and using Hybrid auto-selection as a strong default strategy when coverage is a primary objective.

\textbf{Unsupervised deployment path.} While QDA-Magnified achieves the highest AUC, it requires both-class labels. For fully unsupervised deployment, UDP offers two label-free alternatives: (i)~the magnitude--novelty score (Eq.~\eqref{eq:score}), which flags observations deviating in any operator, and (ii)~RankFusion, which averages per-operator rank statistics without distributional assumptions.  RankFusion achieves mCov~89\% and mAUC~0.963---within 1~pp of the supervised Hybrid variant---making it a strong default when labels are unavailable.

\textbf{Case-study relevance to regulated domains.} The G-SIB evaluation (Section~\ref{sssec:gsib}) uses prospective-only information and yields a low false-alarm rate on the studied panel, while the ERCOT case study illustrates the framework's behaviour on energy-infrastructure monitoring.  These results suggest potential relevance to regulated financial and energy settings, but production deployment would require substantially broader validation, governance review, and human-in-the-loop evaluation.

\subsection{Limitations}
\label{ssec:limits}

\begin{enumerate}
  \item \textbf{Supervised components}: The QDA-Magnified pipeline requires both-class labels for the magnifier discriminative scores (Eq.~\eqref{eq:disc_score}) and QDA class covariances.  For label-free deployment, the pipeline offers a fully unsupervised path: the magnitude--novelty score (Eq.~\eqref{eq:score}) and RankFusion (mCov~89\%) require no anomaly labels whatsoever, making UDP applicable in settings where only normal data is available.
  ...
  \item \textbf{Gate constants}: The sigmoid gate threshold ($0.4$) and steepness ($20$) in Eq.~\eqref{eq:gate} were calibrated at the medium-discriminability boundary and validated by stability analysis across seven ODDS benchmarks (Section~\ref{ssec:subspacescan}).  While empirically robust across $[0.3, 0.5]$ and $[15, 30]$ respectively, deriving them directly from a formal invariant (e.g., optimal persistence threshold) remains an open problem; Bayesian optimisation is left for future work.
  \item \textbf{Hyperparameter sensitivity}: We fix $\gamma{=}5.0$ (magnifier, selected by inner CV), $\gamma_s{=}3.0$ (gate stretch, selected by A/B benchmark), $\alpha{=}1.0$ (exponential), and $\lambda{=}10^{-4}$ (QDA regularisation) globally.  These values were selected through systematic cross-validated benchmarking.  Cross-validated sensitivity sweeps show that $\gamma \in [3, 8]$ and $\gamma_s \in [2, 5]$ yield $<0.01$ AUC variation on Mammography. Performance degrades outside these ranges: $\gamma < 2$ under-saturates clear dimensions, while $\gamma > 10$ saturates blind dimensions. A formal sensitivity analysis across all datasets is deferred to future work.
...
\begin{enumerate}
  \item \textbf{Temporal tensor extension}: Add a time axis $T_{i,k,t,d}$ for streaming and time-series anomaly detection.  The G-SIB expanding-window protocol (Section~\ref{sssec:gsib}) is a first step; a fully recurrent tensor would enable real-time regime-change detection.
  ...
  \item \textbf{Multi-engine comparative study}: The UDP System Mode architecture supports alternative detection engines---Gravity (inter-agent gravitational coupling) and Molecular (kinetic energy and thermal equilibrium)---that exploit different physical analogies for systemic anomaly aggregation.  A companion paper will present per-bank ablations, multi-engine fusion strategies, and cross-sector deployment of these variants.
\end{enumerate}
...
\begin{enumerate}
  \item Formalises the Universal Deviation Principle through six theorems with explicit assumptions (A1--A3): if the operator family is deviation-separating (full-rank stacked Jacobian), then any displacement from the normal reference must produce a measurable response in at least one operator (Theorems~1--2), with finite-sample guarantees (Theorem~3), SDP-optimal fusion weights (Theorem~4), submodular operator selection (Theorem~5), and a sufficient condition for detection separation (Theorem~6).
  ...
  \item Achieves robust five-fold cross-validation ($0.925 \pm 0.020$) on mammography---within $0.2$~pp of LightGBM ($0.927$) despite using a closed-form QDA head---while using a single set of hyperparameters across the benchmark suite.
  ...
  \item Provides the first head-to-head comparison of UDP against gradient-boosted trees (XGBoost, LightGBM) on five ODDS datasets, demonstrating that UDP matches LightGBM on Mammography ($0.925$ vs $0.927$) with only a closed-form QDA head and no gradient optimisation, and identifying high-dimensional scaling as the primary open limitation.
  \item Reports prospective case studies on \textbf{real-world sensitive sectors}: 25~G-SIB banks under a strictly prospective protocol, FRED macroeconomic indicators, and ERCOT grid-failure data.  These studies illustrate how the framework behaves on structured financial and infrastructure monitoring tasks, complementing---rather than replacing---the benchmark evaluation.
  \item Extends the framework to \textbf{multi-class classification} via per-class MDN decomposition (Section~\ref{sec:classification}), achieving mean accuracy $0.936$ across six standard benchmarks---competitive with LightGBM ($0.934$) and RF-200 ($0.933$)---thereby demonstrating dual-use capability of the multi-spectrum representation for both anomaly detection and classification.
\end{enumerate}

The framework generalises BSDT's post-model correction into a model-independent detection paradigm.  By measuring deviation across complementary operators and rescuing boundary anomalies through adaptive subspace aggregation, the Universal Deviation Principle reframes anomaly detection as a structured multi-operator analysis problem.  The classification extension further validates the quality of the multi-spectrum representation: the same operators that detect anomalies also discriminate classes, using only $C{\times}(K{+}2)$ MDN features with no gradient optimisation.  The benchmark results establish core empirical performance across both tasks, while the financial, macroeconomic, and energy case studies illustrate how the framework can be instantiated on multi-agent monitoring tasks.

% ============================================================
%  Acknowledgements
% ============================================================
\section*{Acknowledgements}
This work was conducted independently.  The author acknowledges the foundational role of Blind Spot Decomposition Theory in motivating this framework.
...
All proofs are self-contained; no external dependencies beyond standard analysis are required.

\subsection{Proof of Theorem 1 (Operator Sensitivity --- Local)}

We must show that the stacked map $\Phi = (\Phi_1,\ldots,\Phi_K)$ is locally injective at $\boldsymbol{\mu}$ and satisfies the quantitative bound~\eqref{eq:quantitative}.

\textit{Step 1 (Full-rank Jacobian).}
Because $\{\Phi_k\}$ is deviation-separating (Definition~1), $\bigcap_{k=1}^{K}\ker D\Phi_k(\boldsymbol{\mu}) = \{0\}$.
Hence the stacked Jacobian $J(\boldsymbol{\mu}) = [D\Phi_1(\boldsymbol{\mu})^\top,\ldots,D\Phi_K(\boldsymbol{\mu})^\top]^\top \in \R^{D\times m}$ has $\ker J(\boldsymbol{\mu})=\{0\}$, so $\sigma_{\min}(J(\boldsymbol{\mu}))>0$.

\textit{Step 2 (Inverse Function Theorem).}
Since $J(\boldsymbol{\mu})$ has full column rank, the Inverse Function Theorem guarantees that $\Phi$ is locally injective in a neighbourhood $U\ni\boldsymbol{\mu}$.

\textit{Step 3 (Quantitative bound).}
Taylor-expand each $\Phi_k$ about $\boldsymbol{\mu}$:
\[
\Phi_k(\mathbf{x}) - \Phi_k(\boldsymbol{\mu}) = D\Phi_k(\boldsymbol{\mu})(\mathbf{x}-\boldsymbol{\mu}) + O(\|\mathbf{x}-\boldsymbol{\mu}\|^2).
\]
Stacking: $\Phi(\mathbf{x})-\Phi(\boldsymbol{\mu}) = J(\boldsymbol{\mu})(\mathbf{x}-\boldsymbol{\mu}) + O(\|\mathbf{x}-\boldsymbol{\mu}\|^2)$.
By the singular-value lower bound, $\|J(\boldsymbol{\mu})\mathbf{v}\|\geq\sigma_{\min}\|\mathbf{v}\|$ for all~$\mathbf{v}$, so
\[
\|\Phi(\mathbf{x})-\Phi(\boldsymbol{\mu})\| \geq \sigma_{\min}\|\mathbf{x}-\boldsymbol{\mu}\| - O(\|\mathbf{x}-\boldsymbol{\mu}\|^2).
\]
Because $\max_k\|a_k\|\geq\|a\|/\sqrt{K}$ for any partitioning into $K$ blocks, we obtain~\eqref{eq:quantitative}.\hfill$\square$

\subsection{Proof of Theorem 2 (Operator Sensitivity --- Global)}

The strategy extends Theorem~1 uniformly over a compact set $\mathcal{K}$.

\textit{Step 1 (Uniform lower bound).}
By hypothesis, $\sigma_{\min}(J(\mathbf{x}))>0$ for every $\mathbf{x}\in\mathcal{K}$.
Since $\sigma_{\min}(J(\cdot))$ is continuous and $\mathcal{K}$ is compact, the Extreme Value Theorem gives $\underline{\sigma}:=\min_{\mathbf{x}\in\mathcal{K}}\sigma_{\min}(J(\mathbf{x}))>0$.

\textit{Step 2 (Second-order remainder).}
Each $\Phi_k\in C^2$, so the Taylor remainder satisfies $\|R_k(\mathbf{x},\boldsymbol{\mu})\|\leq\frac{1}{2}\sup_{\mathbf{z}\in\mathcal{K}}\|D^2\Phi_k(\mathbf{z})\|\cdot\|\mathbf{x}-\boldsymbol{\mu}\|^2$.  Stacking and defining $C=\frac{1}{2}\sup_{\mathbf{z}\in\mathcal{K}}\|D^2\Phi(\mathbf{z})\|$ (finite by continuity on a compact set), we obtain a uniform remainder $\leq C\|\mathbf{x}-\boldsymbol{\mu}\|^2$.

\textit{Step 3 (Combine).}
For any $\mathbf{x}\in\mathcal{A}\subset\mathcal{K}$:
$\max_k\|\Phi_k(\mathbf{x})-\Phi_k(\boldsymbol{\mu})\|\geq\underline{\sigma}\|\mathbf{x}-\boldsymbol{\mu}\|/\sqrt{K} - C\|\mathbf{x}-\boldsymbol{\mu}\|^2$.
Taking the infimum over $\mathbf{x}\in\mathcal{A}$ yields~\eqref{eq:global}.\hfill$\square$

\subsection{Proof of Theorem 3 (Finite-Sample Guarantee)}

\textit{Step 1 (Mean estimation error).}
Since $\hat{\boldsymbol{\mu}}_n$ is the sample mean of $n$ i.i.d.\ sub-Gaussian draws with covariance~$\Sigma$, a standard Hoeffding-type bound gives: for each coordinate $j$,
$\Pr[|\hat{\mu}_{n,j}-\mu_j|>t]\leq 2\exp\!\bigl(-nt^2/(2\Sigma_{jj})\bigr)$.
A union bound over all $m$ coordinates and choice $t_j=\sqrt{2\Sigma_{jj}\ln(2m/\delta)/n}$ yields
$\Pr\!\bigl[\|\hat{\boldsymbol{\mu}}_n-\boldsymbol{\mu}\|>\sqrt{\mathrm{tr}(\Sigma)}\cdot\sqrt{2\ln(2m/\delta)/n}\bigr]\leq\delta$.

\textit{Step 2 (Lipschitz transfer).}
Each $\Phi_k$ is $L_k$-Lipschitz, so:
\[
\|\Phi_k(\mathbf{x})-\Phi_k(\hat{\boldsymbol{\mu}}_n)\| \geq \|\Phi_k(\mathbf{x})-\Phi_k(\boldsymbol{\mu})\| - L_k\|\hat{\boldsymbol{\mu}}_n-\boldsymbol{\mu}\|.
\]

\textit{Step 3 (Combine with Theorem~1).}
Applying the local bound (Theorem~1 at the population mean) and the $L=\max_k L_k$ simplification:
\[
\max_k\|\Phi_k(\mathbf{x})-\Phi_k(\hat{\boldsymbol{\mu}}_n)\| \geq \frac{\underline{\sigma}}{\sqrt{K}}\|\mathbf{x}-\boldsymbol{\mu}\| - \frac{L\sqrt{\mathrm{tr}(\Sigma)}\cdot\sqrt{2\ln(2m/\delta)}}{\sqrt{n}}
\]
with probability $\geq 1-\delta$, yielding~\eqref{eq:finite_sample}.\hfill$\square$

\subsection{Proof of Theorem 4 (SDP Formulation)}

\textit{Step 1 (Concavity).}
The minimum eigenvalue $\lambda_{\min}(A)=\min_{\|\mathbf{v}\|=1}\mathbf{v}^\top A\,\mathbf{v}$ is the infimum (equivalently, pointwise minimum) of linear functions of $A$.  Since $\sum_k w_k\mathcal{I}_k$ is linear in $\mathbf{w}$, the composition $\eta(\mathbf{w})=\lambda_{\min}(\sum_k w_k\mathcal{I}_k)$ is concave on $\Delta_K$ as an infimum of affine functions.

\textit{Step 2 (SDP reformulation).}
Maximising a concave function over a polytope is a convex programme.  The specific structure---maximise $\eta(\mathbf{w})$ subject to $\mathbf{w}\in\Delta_K$---admits the standard SDP formulation:
\[
\max_{\mathbf{w},\,\eta}\;\eta \quad\text{s.t.}\quad \textstyle\sum_k w_k\mathcal{I}_k \succeq \eta I,\quad \mathbf{w}\in\Delta_K,
\]
which is solvable in polynomial time by interior-point methods.\hfill$\square$

\subsection{Proof of Theorem 5 (Greedy Operator Selection)}

\textit{Step 1 (Monotonicity).}
Adding an operator increases $\sum_{k\in S}\mathcal{I}_k$ in the positive-semidefinite order, so $\log\det(\sum_{k\in S}\mathcal{I}_k)$ is non-decreasing.

\textit{Step 2 (Submodularity).}
For sets $S\subseteq T$ and a new operator $j\notin T$:
\[
f(S\cup\{j\})-f(S) = \log\det(I + (\textstyle\sum_{k\in S}\mathcal{I}_k)^{-1}\mathcal{I}_j).
\]
As $S$ grows, $(\sum_{k\in S}\mathcal{I}_k)^{-1}$ decreases (in the Loewner order), so the marginal gain diminishes.  This is the defining property of submodularity.

\textit{Step 3 (Greedy guarantee).}
By the classical result of Nemhauser, Wolsey, and Fisher (1978), greedy maximisation of a monotone submodular function subject to a cardinality constraint $|S|\leq K$ achieves at least $(1-1/e)\approx 0.632$ of the optimal value, yielding~\eqref{eq:greedy_bound}.\hfill$\square$

\subsection{Proof of Theorem 6 (Operator-Space Separation)}

\textit{Step 1 (Nearest-point projection).}
Since $\mathcal{N}$ is compact and $\mathbf{x}\notin\mathcal{N}$, there exists $\mathbf{z}^*=\arg\min_{\mathbf{z}\in\mathcal{N}}\|\mathbf{x}-\mathbf{z}\|$ with $d:=\|\mathbf{x}-\mathbf{z}^*\|>0$.

\textit{Step 2 (Local expansion at $\mathbf{z}^*$).}
Apply the Taylor expansion used in Theorem~1 at $\boldsymbol{\mu}=\mathbf{z}^*$:
\[
\|\Phi(\mathbf{x})-\Phi(\mathbf{z}^*)\| \geq \sigma_{\min}(J(\mathbf{z}^*))\cdot d - O(d^2).
\]
Since $\sigma_{\min}(J(\mathbf{z}^*))\geq\underline{\sigma}>0$ by hypothesis, this gives $\|\Phi(\mathbf{x})-\Phi(\mathbf{z}^*)\|\geq\underline{\sigma}\,d-O(d^2)$.

\textit{Step 3 (Score bound).}
By definition, $s(\mathbf{x})=\inf_{\mathbf{z}\in\mathcal{N}}\|\Phi(\mathbf{x})-\Phi(\mathbf{z})\|\leq\|\Phi(\mathbf{x})-\Phi(\mathbf{z}^*)\|$.  Conversely, for any $\mathbf{z}\in\mathcal{N}$ with $\|\mathbf{x}-\mathbf{z}\|\geq d$, the same local analysis yields $\|\Phi(\mathbf{x})-\Phi(\mathbf{z})\|\geq\underline{\sigma}\,d-O(d^2)$, so $s(\mathbf{x})\geq\underline{\sigma}\,d-O(d^2)$, establishing~\eqref{eq:separation}.

The per-operator bound $s_k(\mathbf{x})\geq\underline{\sigma}\,d/\sqrt{K}-O(d^2)$ follows from the block decomposition argument in Theorem~1 (Step~3).\hfill$\square$
...
O. O. Israel, ``Blind spot decomposition theory: A mathematical framework for characterising and correcting missed fraud in machine learning systems,'' Zenodo, Mar. 2025, doi: \href{https://doi.org/10.5281/zenodo.14942568}{10.5281/zenodo.14942568}.
...
\bibitem{worldbank2024gfdd}
{World Bank}, ``Global Financial Development Database (GFDD),'' 2024. [Online]. Available: \url{https://www.worldbank.org/en/publication/gfdr/data/global-financial-development-database}

\bibitem{li2020copod}
Z. Li, Y. Zhao, N. Botta, C. Ionescu, and X. Hu, ``COPOD: Copula-based outlier detection,'' in \textit{Proc. IEEE ICDM}, 2020, pp. 1118--1123.

\bibitem{li2022ecod}
Z. Li, Y. Zhao, X. Hu, N. Botta, C. Ionescu, and G. Chen, ``ECOD: Unsupervised outlier detection using empirical cumulative distribution functions,'' \textit{IEEE Trans. Knowl. Data Eng.}, vol. 35, no. 12, pp. 12708--12719, 2022.

\bibitem{goldstein2012histogram}
M. Goldstein and A. Dengel, ``Histogram-based outlier score (HBOS): A fast unsupervised anomaly detection algorithm,'' in \textit{Proc. KI-2012: Poster and Demo Track}, 2012, pp. 59--63.
