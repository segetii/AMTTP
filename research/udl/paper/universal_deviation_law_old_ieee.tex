...
%  Title
% ============================================================
\title{Universal Deviation Principle: A Multi-Operator Framework for Anomaly Detection with Sensitivity Guarantees and Boundary-Anomaly Recovery}
...
\begin{tablenotes}\small
  ...
  \item[$\dagger$] Wilcoxon signed-rank test ($n{=}15$, 3 real datasets~$\times$~5 folds, ReducedTensorDescriptor vs.\ baselines): UDL is statistically significant vs.\ LOF ($p{=}0.028$); differences vs.\ IForest, COPOD, HBOS are not significant at $\alpha{=}0.05$.
\end{tablenotes}
\end{threeparttable}
\end{table*}
...
The framework generalises BSDT's post-model correction into a model-independent detection paradigm.  By measuring deviation across complementary operators and rescuing boundary anomalies through adaptive subspace aggregation, the Universal Deviation Principle reframes anomaly detection as a structured multi-operator analysis problem.  The benchmark results establish its core empirical performance, while the financial, macroeconomic, and energy case studies illustrate how the framework can be instantiated on multi-agent monitoring tasks.
