...
% ============================================================
\begin{abstract}
We introduce the \textbf{Universal Deviation Principle (UDL)}, a theoretically grounded multi-operator framework for anomaly detection.  The core thesis is that a point meaningfully different from the normal set cannot simultaneously match normality under all structural perspectives.  We formalise this via \emph{deviation-separating operator families}---collections of differentiable maps $\{\Phi_k\}_{k=1}^K$ whose stacked Jacobian has full column rank---and prove six results under explicit assumptions (A1--A3): local and global sensitivity bounds, finite-sample guarantees, SDP-optimal fusion weights, greedy operator selection with $(1{-}1/e)$-approximation, and a sufficient condition for operator-space separation.

We instantiate the framework with 14~composable spectrum operators spanning five mathematical families (statistical, geometric, frequency, dynamical, topological) and introduce two practical contributions: (i)~a \textbf{boundary-centred dimension magnifier} that selectively amplifies discriminative dimensions while suppressing noise through adaptive $\tanh$ saturation, paired with QDA for supervised projection; and (ii)~\textbf{SubspaceScan}, a random-projection ensemble with adaptive gated stretching that recovers boundary anomalies invisible to per-dimension detectors.  We additionally propose \textbf{top-$k$ coverage}---the fraction of true anomalies surfaced among the highest-ranked predictions---as a complementary evaluation metric, revealing that methods with comparable AUC exhibit markedly different retrieval profiles.

Across ten ODDS benchmarks spanning six domains with a \emph{single} hyperparameter set, the full pipeline achieves a mean AUC of $0.910$ (five-fold CV: $0.925 \pm 0.020$ on Mammography), improving upon the base fusion model by $+0.9$~pp with the largest gain on arrhythmia ($+6.4$~pp).  A Spearman correlation audit finds only one redundant operator pair ($\rho > 0.85$) among 91, empirically confirming that each mathematical perspective contributes largely independent signal.  We further report prospective case studies on 25~Global Systemically Important Banks, FRED macroeconomic indicators, and ERCOT grid-failure data to illustrate how the framework behaves in financial, macroeconomic, and energy-infrastructure settings.

We additionally demonstrate that the framework extends naturally to \textbf{multi-class classification}: per-class operator stacks combined with the MDN decomposition yield a compact $C{\times}(K{+}2)$ feature representation that achieves mean accuracy $0.936$ across six standard benchmarks (Iris, Wine, Breast~Cancer, Digits, Vehicle, Segment), competitive with LightGBM ($0.934$) and RF-200 ($0.933$) while using no gradient optimisation.  Code is available at \url{https://github.com/segetii/fintech}.
\end{abstract}

\begin{IEEEkeywords}
Anomaly detection, multi-class classification, multi-spectrum representation, subspace boundary rescue, tensor decomposition, adaptive gating, operator redundancy analysis, coverage metric, interpretable scoring, systemic anomaly monitoring
\end{IEEEkeywords}
...
\textit{In particular, for any $\mathbf{x}$ sufficiently close to $\mathcal{N}$ but not in $\mathcal{N}$, $s(\mathbf{x}) > 0$, and there exists $k$ such that $s_k(\mathbf{x}) \geq \underline{\sigma} \cdot d / \sqrt{K} - O(d^2)$.  (Proof: apply Theorem~1 at the nearest point $\mathbf{z}^* \in \mathcal{N}$.)}
...
\textbf{Operational form.} Let $\epsilon_k$ be the $(1-\alpha)$ quantile of $\norm{\Phi_k(\mathbf{x}) - \Phi_k(\boldsymbol{\mu})}$ under $P_{\mathcal{N}}$. Then an observation is flagged anomalous if $\exists\, k$ such that $\norm{\Phi_k(\mathbf{x}) - \Phi_k(\boldsymbol{\mu})} > \epsilon_k$.  Note that Theorems~1--2 guarantee that any displaced point produces operator response, but this response must \emph{exceed} the normal-data baseline $\epsilon_k$ for detection.  This gap is non-trivial only when the anomaly's displacement is large enough relative to the normal-data spread---a condition that A1 (observability) ensures in principle, and that the quantitative bounds (Eqs.~\eqref{eq:quantitative}--\eqref{eq:finite_sample}) make precise.  All proofs are deferred to the Appendix.
...
where $s_a(t)$ is the UDL anomaly score for agent~$a$ at time~$t$.  To convert this aggregate score into an alarm signal without look-ahead, we apply a Basel-style rolling z-score:
...
Three findings emerge.  \textbf{(i)}~UDL-Opt (closed-form QDA head, no gradient optimisation) achieves $0.925$ on Mammography, within $0.2$~pp of LightGBM ($0.927$); this indicates that the multi-spectrum deviation features carry competitive discriminative signal on low-dimensional data even with a simple closed-form head.  \textbf{(ii)}~UDL-Opt reaches $0.983$ on PenDigits and $0.952$ on Satellite, confirming strong performance across diverse data geometries.  \textbf{(iii)}~On Arrhythmia ($D{=}274$), the $274{\to}32$ dimensional compression that UDL operators impose causes information loss: UDL-Opt drops to $0.767$ while LGB-raw retains $0.880$.  This confirms UDL's high-dimensional scaling limitation (Section~\ref{ssec:limits}).

\begin{figure}[h]
\centering
\includegraphics[width=\columnwidth]{figures/udl_vs_baselines_bar.pdf}
\caption{UDL standalone methods vs supervised baselines across four ODDS benchmarks (5-fold CV). UDL-QDA-Opt (dark blue) matches or approaches tree baselines on Mammography and Pendigits while using only closed-form QDA with a single hyperparameter. XGBoost and LightGBM (warm colours) run independently on raw features---they are \emph{not} mixed into the UDL pipeline.}
...
\caption{Mean AUC-ROC summary across four ODDS datasets. UDL-QDA-Opt achieves mAUC~$0.931$, the strongest UDL variant, vs LightGBM's~$0.968$. The $3.7$~pp gap is attributable to trees' capacity for non-linear feature interaction on raw inputs---a capability UDL trades for interpretable per-operator attribution and theoretical guarantees.}
\label{fig:udl_mauc}
\end{figure}
...
\begin{itemize}
  ...
  \item \textbf{UDL matches gradient-boosted trees on Mammography.} UDL-QDA-Magnified achieves $0.925$, within $0.2$~pp of LightGBM ($0.927$) and XGBoost ($0.926$)---see Table~\ref{tab:supervised}---despite using a closed-form QDA head with a single hyperparameter ($\gamma$) rather than 200 boosted trees, demonstrating that multi-spectrum deviation features carry comparable discriminative power to gradient-boosted ensembles on low-dimensional data.
...
Beyond standard ODDS benchmarks, we evaluate UDL's System Mode on three real-world datasets spanning finance and energy infrastructure, using protocols designed for actual deployment.
...
\textbf{Economy (FRED).} Eleven quarterly macroeconomic series projected through a $12{\times}6$-dimensional sector-weight matrix to create 12~synthetic bank-sector agents ($N{=}948$ samples). UDL achieves AUC~$= 0.9999$ and FAR~$= 0.1\%$ on this constructed case study: all nine crisis quarters are detected with one false alarm.
...
\subsection{Classification via Per-Class MDN Decomposition}
\label{ssec:classification}

The Universal Deviation Principle extends naturally from anomaly detection to multi-class classification.  In anomaly detection, UDL asks ``how does $x$ deviate from the normal reference across $K$ operator perspectives?''  For classification, we ask ``how does $x$ deviate from \emph{each class} across $K$ perspectives?''  The Magnitude-Direction-Novelty~(MDN) decomposition provides the machinery.

For each class $c \in \{1, \ldots, C\}$, a dedicated \textsc{RepresentationStack}~$\Phi^{(c)}$ is fitted on class~$c$'s training data, calibrating the $K$~operators to that class's distribution.  An \textsc{AnomalyTensor}~$\mathcal{T}^{(c)}$ is then fitted on the class-specific representations, capturing centroid~$\mu_c$ and reference directions.  At prediction time, every sample is decomposed through each class's tensor, yielding a compact feature vector:
\begin{equation}
...
\begin{table*}[t]
\centering
\caption{Classification accuracy (5-fold stratified CV, mean $\pm$ std).  Best per dataset in \textbf{bold}; best UDL variant \underline{underlined}.  UDL uses per-class MDN features ($C{\times}8$ dimensions); baselines operate on standardised raw features.}
\label{tab:classification}
\begin{tabular}{@{}l ccc ccccc@{}}
\toprule
\textbf{Dataset} ($n$, $d$, $C$)
  & \textbf{UDL-QDA} & \textbf{UDL-Log.} & \textbf{UDL-RF}
...
\textbf{(i)}~UDL achieves the highest accuracy on \textbf{Wine} ($0.989$, UDL-Logistic) and \textbf{Vehicle} ($0.794$, UDL-Logistic), demonstrating that per-class MDN features can surpass strong baselines on moderate-dimensional data.  On Wine, the $+0.6$~pp margin over the best baseline (SVM, $0.983$) is notable given the compact 24-feature MDN representation.
\textbf{(ii)}~On high-dimensional data (Digits, $d{=}64$, $C{=}10$), UDL-RF reaches $0.960$ using 80~MDN features, competitive with SVM ($0.984$) and RF-200 ($0.980$).  No single head dominates: QDA excels on low-dimensional problems (Iris, BreastCancer), Logistic on moderate-dimensional (Wine, Vehicle), and RF on high-dimensional or many-class data (Digits, Segment).
\textbf{(iii)}~Selecting the best head per dataset, UDL achieves mean accuracy \textbf{0.936}---competitive with LightGBM ($0.934$), RF-200 ($0.933$), and Logistic ($0.933$), and above SVM ($0.932$) and $k$NN ($0.923$)---using only $C \times 8$ MDN features with no gradient optimisation.  This dual-use capability---competitive anomaly detection \emph{and} classification from the same multi-spectrum operators---validates the representation quality predicted by Theorem~1.
...
\begin{enumerate}
  ...
  \item \textbf{Gate constants}: The sigmoid gate threshold ($0.4$) and steepness ($20$) in Eq.~\eqref{eq:gate} are motivated by Morse-topological structural stability---the gate acts as an analogue of persistence-diagram filtration, preserving topologically robust discriminative signals while stretching ephemeral ones.  The specific numerical values were calibrated at the medium-discriminability boundary and validated by stability analysis across seven ODDS benchmarks (Section~\ref{ssec:subspacescan}).  While empirically robust across $[0.3, 0.5]$ and $[15, 30]$ respectively, deriving them directly from a topological invariant (e.g., optimal persistence threshold) remains an open problem; Bayesian optimisation and formal topological calibration are left for future work.
  \item \textbf{Hyperparameter sensitivity}: We fix $\gamma{=}5.0$ (magnifier, selected by inner CV), $\gamma_s{=}3.0$ (gate stretch, selected by A/B benchmark), $\alpha{=}1.0$ (exponential), and $\lambda{=}10^{-4}$ (QDA regularisation) globally.  These values operate within a Morse-theoretic scoring framework whose architecture ensures structural stability; the numerical constants are then empirically calibrated on this foundation.  Cross-validated sensitivity sweeps show that $\gamma \in [3, 8]$ and $\gamma_s \in [2, 5]$ yield $<0.01$ AUC variation on Mammography. Performance degrades outside these ranges: $\gamma < 2$ under-saturates clear dimensions, while $\gamma > 10$ saturates blind dimensions. A formal sensitivity analysis across all datasets is deferred to future work.
