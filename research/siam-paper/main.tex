...
\emph{current} quarter into the particle simulation---this is the
correct in-sample monitoring protocol, since the detection signal
arises from the interaction between the new observation and the
historical configuration.  Removing the current quarter (out-of-sample
scoring) eliminates the physical detection mechanism.  The evaluation
follows five rules:
\begin{enumerate}
...
(Assumption~A3 in \citet{UDL2025}) is not merely a theoretical
