...
A \emph{critical manifold} $\Cman$, characterised by the
Morse-index jump $\mathrm{ind}=0\to\mathrm{ind}\ge 1$, separates
a stable from an unstable regime; adaptive friction
...
GFC lead, AUROC $\ge 0.70$), the Terra/Luna collapse ($r=0.996$,
$+30$\,h lead), the Texas ERCOT grid ($+72$\,h lead), and three
new sensitive-sector benchmarks: FRED macroeconomic indicators
(AUC $= 0.9999$), ERCOT energy-grid dispatch (AUC $= 0.9940$),
and a prospective G-SIB bank-level panel where the multi-factor
latent score exceeds $z > 2$ six quarters before the Lehman
collapse.
...
are geometrically aligned.  The Morse-index jump from $0$ to $\ge 1$
provides a topological prediction signal that is decoupled from the
...
\subsection{Hindsight G-SIB Engine Comparison}\label{sec:hindsight}

To compare the three System Mode engines on a common benchmark,
we run a hindsight analysis on 25~G-SIBs over 76~quarters
(2005:Q1--2023:Q4), using labels from the FSB systemic-event
register.  All engines use identical calibration (reference
period 1994:Q1--2003:Q4) and the FARTargetCalibrator with target
FAR $= 5\%$.

\begin{table}[htbp]
\centering
\caption{Hindsight G-SIB engine comparison ($n = 25$ banks,
$T = 76$ quarters, 2005--2023). FAR computed on non-crisis
quarters only.}
\label{tab:hindsight}
\begin{tabular}{@{}lcccc@{}}
\toprule
...
\subsection{Strict No-Data-Leakage Evaluation}\label{sec:noleakage}
...
a \emph{strictly prospective} evaluation protocol that eliminates
every channel of data leakage.

\paragraph{Protocol.}
We apply the Molecular Engine (Section~\ref{sec:molecular}) to the
G-SIB quarterly panel ($T{=}76$ quarters, $N{=}25$ banks, $d{=}5$
features, 2005\,Q1--2023\,Q4).  The evaluation follows five rules:
\begin{enumerate}
\item \textbf{Expanding-window training.}  For each quarter
  $t \in \{5, \ldots, T{-}1\}$, the engine is trained on quarters
  $1, \ldots, t$ and scores quarter $t{+}1$.  No future data is
...
\item \textbf{Preprocessing fit on past only.}
  ...
  $[1, t]$ and applied (transform only) to the test quarter
  $t{+}1$.  No global statistics are used.
\item \textbf{Frozen hyperparameters.}  All engine parameters
  (iterations${}=80$, $k$-neighbors${}=10$,
  $n_{\mathrm{max}}=2000$) are fixed from a pre-crisis validation
  period (2005\,Q1--2007\,Q3) and never updated.
\item \textbf{Fixed alert threshold.}  The threshold
  $\tau = \mu + 3\sigma$ is computed \emph{once} from the
  pre-crisis score distribution and frozen for the entire test
  horizon.
\item \textbf{No label access at test time.}  Crisis labels are
  used only for \emph{ex-post} evaluation; the alarm decision at
  each quarter depends solely on the score and the frozen
  threshold.
\end{enumerate}

\paragraph{Results.}
Under this strict regime the system achieves AUROC~$= 0.607$ with
recall~$38.5\%$ at the frozen threshold.  Critically, the anomaly
score begins rising in 2008\,Q2 (score~$0.295$) and peaks in
2008\,Q4 (score~$0.496$)---\emph{before} the Lehman Brothers
collapse (September~2008).  Five of seven GFC quarters trigger the
alarm.  The European sovereign-debt and COVID crises are not
detected, consistent with the limited temporal modelling discussed
in Section~\ref{sec:discussion}.

Elevated scores in 2022--2023 (scores $0.46$--$0.56$) coincide
with the SVB collapse and aggressive Federal Reserve rate hikes,
periods widely regarded as genuine banking stress.  These are
labelled ``false positives'' only because the NBER crisis labels
used here do not cover post-2020 events.

\paragraph{Timeline.}
Figure~\ref{fig:no-leakage-timeline} shows the prospective anomaly
score with the frozen threshold, crisis shading, and the Lehman
Brothers date.  The GFC rise pattern---visible \emph{before} the
collapse---rules out hindsight leakage.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{molecular_no_leakage_timeline.pdf}
  \caption{Molecular Engine anomaly score under the strict
  no-data-leakage protocol (expanding window, frozen threshold
  from pre-crisis baseline).  The score rises prospectively before
  the Lehman Brothers collapse (purple dashed line), ruling out
  information leakage.  Shaded regions mark NBER-dated crises.
  Triangles: true positives (green~$\blacktriangle$) and false
  negatives (yellow~$\blacktriangledown$); crosses: false positives
  (red~$\times$).}
...
The paper establishes fifteen findings within a unified
...
\item Hindsight G-SIB analysis on 25~banks over 76~quarters
...
\item \textbf{False alarm rate.} The 35\% FAR reflects persistent
  super-criticality rather than model noise; the FARTargetCalibrator
  reduces this to 5\% but requires labelled calibration data.
  Note, however, that the BSDT channel weighting itself is now
  fully unsupervised via the Fisher variance ratio
  (Eq.~\ref{eq:fisher_vr}): label leakage has been eliminated
  from all scoring variants.
...
\item \textbf{Prospective AUROC.} The G-SIB bank-level prospective
  protocol produces AUROC $\ge 0.80$ but the strictly forward-looking
  test window contains only two major crises (GFC, COVID).
\end{enumerate}
