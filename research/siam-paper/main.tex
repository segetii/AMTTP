...
\item \textbf{Equivalence Theorem (Theorem~C) and Entropy Structure
  (Theorem~\ref{thm:entropy}).} Two-sided gradient bounds establish
...
\begin{proof}
(E1) follows directly from Part~(C3) of Theorem~\ref{thm:equiv}:
$\dot{W} \le -\kappa\norm{\grad\Ebs}^2 \le 0$.
...
\]
Since $\inner{\grad\Ebs}{F} \le 0$ (by~(C3)) and
$\gamma(\Ebs)\norm{\grad\Ebs}^2 \ge 0$, we have
$\sigma(X) = -\dot{W} = \gamma(\Ebs)\norm{\grad\Ebs}^2
- \inner{\grad\Ebs}{F} \ge 0$.

(E3) $\sigma(X) = 0$ iff $\norm{\grad\Ebs} = 0$ (since
$\gamma(E) \ge \kappa > 0$ for $E > \theta_0$ and $F = 0$ at
equilibrium). By~(C1), $\norm{\grad\Ebs} = 0$ iff
$\norm{\grad\Phi} = 0$, which holds iff $X = \Xst$.
...
(E1): From Part~(C3) of Theorem~\ref{thm:equiv}, along
\eqref{eq:bsdamped}:
$\dot{W} \le -\kappa\norm{\grad\Ebs}^2 \le 0$,
so $\ddt\Ebs(X(t)) \le 0$.
...
follows by differentiating $W$ along \eqref{eq:bsdamped}. Both
terms are non-negative: the first by $\gamma \ge 0$; the second
because $\inner{\grad\Ebs}{F} = \inner{\grad\Ebs}{-\grad\Phi} \le 0$
by the gradient alignment bound~(C1)
($\costheta \ge \nu/c_2 > 0$ implies the inner product is
non-positive).
