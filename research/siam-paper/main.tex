...
Navier--Stokes\thanks{Submitted to the editors \today.}}
...
Navier--Stokes, where adaptive viscosity $\nu(E_\mathrm{BS})$
suppresses peak enstrophy by $1.77\times$ at $\mathrm{Re}\approx
1257$ on a mesh-converged $128^3$ pseudo-spectral grid (Taylor--Green
vortex; resolution-convergent to within $<1\%$).  A
15-point Reynolds sweep ($\mathrm{Re}=314$--$62832$) reveals a
non-monotone profile: suppression peaks at $S_{\max}=1.43\times$
in the transitional regime ($\mathrm{Re}\approx 628$) and decays
as $S(\mathrm{Re})\approx 1.66\cdot\mathrm{Re}^{-0.047}$
($\alpha=-0.047\pm 0.012$, $p=0.0016$, $R^2=0.55$).  All 15 runs
complete without blow-up, the BKM integral is finite throughout,
and $\gamma^*_{\max}=0.9959$ uniformly, confirming that the BSDT
feedback is active across the full laminar-to-turbulent range.
\end{abstract}
