%% ====================================================================
%%  APPENDIX: Computational Analysis of BSDT-Modified Navier–Stokes
%%  ────────────────────────────────────────────────────────────────
%%  This appendix collects results from GPU spectral simulations
%%  (CuPy, N=128, Taylor-Green IC, semi-implicit integrator) that
%%  go beyond the conditional regularity of Theorem 9.1.
%%
%%  Every statement is labelled PROVED (from definitions/axioms),
%%  VERIFIED (computational, reproducible), or CONJECTURED (open).
%% ====================================================================

\section{Computational Analysis of the Adaptive Viscosity
  Mechanism}\label{app:computational_ns}

The regularity theorems in Section~\ref{sec:ns} are conditional:
Theorem~\ref{thm:ns_regularity} assumes an \emph{a priori}
enstrophy bound, and Theorem~\ref{thm:ns_strong} requires
small data.  This appendix reports quantitative simulation
results that (i) identify the \emph{mechanism} by which adaptive
viscosity achieves stability, (ii) delineate the precise parameter
regime where a quadratic damping inequality holds, and (iii)
formulate testable conjectures toward unconditional results.

All simulations use the Taylor--Green initial condition
$\bm{u}_0 = (\sin x\cos y\cos z,\,-\cos x\sin y\cos z,\,0)$
on $\T^3$, pseudo-spectral discretisation at $N=128$ with
$2/3$-rule de-aliasing, semi-implicit integrator, $\theta=1$,
and $\mathrm{dt} = 5\times 10^{-4}$.

%% ──────────────────────────────────────────────────────────────────
\subsection{Notation and the Implemented System}
\label{app:ns:notation}
%% ──────────────────────────────────────────────────────────────────

The adaptive-viscosity system solved in the simulations is
\begin{equation}\label{eq:app_ns}
  \partial_t\bm{u}+(\bm{u}\cdot\grad)\bm{u}
  = -\grad p + \nu_{\mathrm{eff}}(t)\,\Delta\bm{u},\quad
  \nu_{\mathrm{eff}}(t) = \nu_0\bigl(1+\gamma^*(E_{\mathrm{BS}}(t))\bigr),\quad
  \gamma^*(E) = \frac{E}{E+\theta},
\end{equation}
where $\theta > 0$ is a sensitivity parameter.  The constant-viscosity
case sets $\gamma^* \equiv 0$, giving $\nu_{\mathrm{eff}} = \nu_0$.

\paragraph{Implemented BSDT channels.}
The simulation computes three of the four channels from
Section~\ref{sec:ns:channels}:
\begin{align}
  \delta_C(t) &= \frac{|\mathcal{E}(t) - \bar{\mathcal{E}}_t|}
  {\max(\sigma_{\mathcal{E},t},\,10^{-10})},
  &\text{(enstrophy z-score)}\label{eq:app_dC}\\
  \delta_G(t) &= \Bigl[\frac{1}{|\mathcal{K}|}
  \sum_{k\in\mathcal{K}}\bigl(\log E(k,t)
  - (-\tfrac{5}{3}\log k + b_t)\bigr)^2\Bigr]^{1/2},
  &\text{(spectral departure from $k^{-5/3}$)}\label{eq:app_dG}\\
  \delta_T(t) &= \frac{\norm{\hat{\bm{u}}(t)-\hat{\bm{u}}(t-\Delta t)}_{L^2}}
  {\norm{\hat{\bm{u}}(t)}_{L^2} + 10^{-20}},
  &\text{(relative spectral change rate)}\label{eq:app_dT}
\end{align}
where $\bar{\mathcal{E}}_t$ and $\sigma_{\mathcal{E},t}$ are running
mean and standard deviation of enstrophy over all previous snapshots.
The vorticity-strain alignment channel $\delta_A$ is \textbf{not
computed} by the solver ($\delta_A \equiv 0$).  The blind-spot energy
is
\begin{equation}\label{eq:app_ebs}
  E_{\mathrm{BS}}(t) = \delta_C(t)^2 + \delta_G(t)^2 + \delta_T(t)^2.
\end{equation}
This differs from the paper's definition~\eqref{eq:ebs_ns} in three
respects: (a) $\delta_C$ is a z-score rather than raw enstrophy,
(b) the weights $w_i$ are uniform (not Fisher-optimised), and
(c) the pairwise interaction term $L_{\mathrm{pair}}$ is absent.
These differences should be borne in mind when comparing simulation
results to the theoretical framework of Section~\ref{sec:ns}.

%% ──────────────────────────────────────────────────────────────────
\subsection{Viscosity Bounds and Leray Well-Posedness}
\label{app:ns:bounds}
%% ──────────────────────────────────────────────────────────────────

\begin{proposition}[Viscosity Bounds]\label{prop:nu_bounds}
For any $\theta > 0$ and $E_{\mathrm{BS}} \ge 0$:
\[
  \nu_0 \;\le\; \nu_{\mathrm{eff}}(t) \;<\; 2\nu_0
  \qquad\text{for all } t \ge 0.
\]
The adaptive mechanism provides at most a factor-$2$ increase
in viscosity.
\end{proposition}

\begin{proof}
$\gamma^*(E) = E/(E+\theta)$ with $E \ge 0$ and $\theta > 0$.
The minimum $\gamma^*(0) = 0$ gives $\nu_{\mathrm{eff}} = \nu_0$.
As $E \to \infty$, $\gamma^* \to 1^-$, so
$\nu_{\mathrm{eff}} \to 2\nu_0^-$.
\end{proof}

\begin{corollary}[Leray Theory Applies]
Since $\nu_{\mathrm{eff}} \ge \nu_0 > 0$ is bounded and measurable,
Leray--Hopf weak solutions exist globally.  Uniqueness and smoothness
remain open (as for classical NS).
\end{corollary}

%% ──────────────────────────────────────────────────────────────────
\subsection{Rapid Saturation of Adaptive Control}
\label{app:ns:saturation}
%% ──────────────────────────────────────────────────────────────────

\begin{proposition}[Saturation Threshold]\label{prop:saturation}
For $\theta = 1$:
\begin{enumerate}
\item[\textup{(i)}]  $E_{\mathrm{BS}} \ge 1 \implies  \gamma^* \ge 1/2$
  and $\nu_{\mathrm{eff}} \ge 3\nu_0/2$.
\item[\textup{(ii)}] $E_{\mathrm{BS}} \ge 99 \implies \gamma^* \ge 0.99$
  and $\nu_{\mathrm{eff}} \ge 1.99\,\nu_0$.
\end{enumerate}
\end{proposition}

\begin{proof}
$\gamma^*(E) = E/(E+1)$ is monotone increasing.
$\gamma^*(1)=1/2$; $\gamma^*(99)=99/100$.
\end{proof}

\paragraph{Computational observation (VERIFIED).}
Across all 15 values of Re in the sweep
($314 \le \mathrm{Re} \le 62{,}832$), the maximum observed
$\gamma^*$ is $0.9959 \pm 0.001$.  Saturation occurs within
$O(1)$ non-dimensional time units.  This means the adaptive
system operates at $\nu_{\mathrm{eff}} \approx 2\nu_0$ for
the vast majority of the simulation.

%% ──────────────────────────────────────────────────────────────────
\subsection{Effective Reynolds Number Reduction}
\label{app:ns:re_reduction}
%% ──────────────────────────────────────────────────────────────────

\begin{theorem}[Reynolds Number Halving]\label{thm:re_halving}
Under the adaptive system~\eqref{eq:app_ns}, at saturation
$\gamma^* \to 1$, the effective Reynolds number is
\[
  \mathrm{Re}_{\mathrm{eff}} = \frac{UL}{\nu_{\mathrm{eff}}}
  \approx \frac{\mathrm{Re}_0}{2}.
\]
The adaptive mechanism's primary action is a
factor-$2$ reduction of Reynolds number.
\end{theorem}

\begin{proof}
At saturation, $\nu_{\mathrm{eff}} = \nu_0(1 + \gamma^*) \to 2\nu_0$.
Then $\mathrm{Re}_{\mathrm{eff}} = UL/(2\nu_0) = \mathrm{Re}_0/2$.
\end{proof}

\begin{remark}
This theorem identifies the adaptive viscosity as physically
equivalent to doubling the fluid's kinematic viscosity.  Any
stability result at the adaptive viscosity level is therefore
a stability result at approximately half the nominal Reynolds
number---expected physics, not a novel mathematical mechanism.
\end{remark}

%% ──────────────────────────────────────────────────────────────────
\subsection{The Damping Inequality and Regime Classification}
\label{app:ns:damping}
%% ──────────────────────────────────────────────────────────────────

We consider the empirical inequality
\begin{equation}\label{eq:app_ineq}
  \frac{d}{dt}\norm{\bm{\omega}}_\infty
  \le -c\,\nu\,\norm{\bm{\omega}}_\infty^2 + C^*
\end{equation}
where $c$ and $C^*$ are obtained by least-squares fit of the finite
differences $\Delta\norm{\bm{\omega}}_\infty/\Delta t$ against
$\norm{\bm{\omega}}_\infty^2$, i.e.,
\[
  c = -\frac{\text{slope}}{\nu},\qquad
  C = \text{intercept},\qquad
  C^* = \max_{t_k}\bigl[\dot{\omega}_k + c\,\nu\,\omega_k^2\bigr].
\]

\paragraph{Critical methodological note.}
The upper bound $C^*$ is defined as the \emph{maximum of the
residuals}.  By construction, $C^* = \max_k r_k$ guarantees
zero violations ($r_k \le C^*$ for all $k$).
\textbf{This is a definition, not a verification.} A genuine
verification would require $C^*$ to be determined \emph{a priori}
(e.g., from initial data alone) and then checked against the
trajectory.

\begin{theorem}[Regime Classification by Damping
  Coefficient]\label{thm:regime}
For the Taylor--Green vortex at $N=128$, $T=5.0$, the fitted
damping coefficient $c(\nu)$ in the quadratic
inequality~\eqref{eq:app_ineq} is \textbf{negative at every
constant viscosity} in the range $[\nu_0,\, 2\nu_0]$.
Representative values at $\nu_0 = 0.005$
($\mathrm{Re}_0 \approx 1257$):
\begin{center}
\begin{tabular}{@{}lcccl@{}}
\toprule
$\nu$ & $\mathrm{Re}$ & $c$ & $R^2$ & Regime \\
\midrule
$0.005$ (bare) & $1257$ & $-11.91$ & $0.652$ & Anti-damping \\
$0.007$ & $898$ & $-10.74$ & $0.678$ & Anti-damping \\
$0.009$ & $698$ & $-9.08$ & $0.559$ & Anti-damping \\
$0.00995$ ($\approx 2\nu_0$) & $631$ & $-8.40$ & $0.487$ & Anti-damping \\
\bottomrule
\end{tabular}
\end{center}
The same pattern holds at all four tested Reynolds numbers
($\mathrm{Re}_0 \in \{628, 1257, 3142, 6283\}$): $c < 0$ throughout
$[\nu_0, 2\nu_0]$.

The previously reported value $c = +2.447$ was obtained from an
\emph{adaptive} run ($\gamma^*$ varying in time, $\nu_{\mathrm{eff}}$
rising from $\nu_0$ to $\approx 0.00995$), \textbf{not} a
constant-$\nu$ run at $\nu = 0.00995$.  The same viscosity
\emph{level} gives opposite sign of~$c$ depending on whether
it is held constant ($c = -8.40$) or arrived at through
BSDT feedback ($c = +2.447$).
\end{theorem}

\begin{proof}[\textbf{VERIFIED} computationally]
Dense viscosity sweep: 11~values in $[0.005,\, 0.00995]$,
each at constant~$\nu$ (\texttt{adaptive=False}), $N=128$,
$T=5.0$.  All yield $c < 0$ (see
Section~\ref{app:ns:nu_c_experiment} for full data).
Repeated at $\nu_0 \in \{0.01, 0.005, 0.002, 0.001\}$ with
sweeps to $2\nu_0$; all yield $c < 0$.
\end{proof}

\begin{theorem}[Critical Viscosity Exceeds Adaptive
  Range]\label{thm:nu_crit}
At all four tested Reynolds numbers ($\mathrm{Re}_0 \in \{628, 1257,
3142, 6283\}$), the critical viscosity $\nu_c$ (where $c(\nu_c) = 0$)
satisfies
\[
  \nu_c > 2\nu_0.
\]
Since the adaptive mechanism~\eqref{eq:app_ns} can push
$\nu_{\mathrm{eff}}$ to at most $2\nu_0$, the quadratic damping
inequality~\eqref{eq:app_ineq} with $c > 0$ is \textbf{never achieved}
by any constant viscosity within the adaptive range.
\end{theorem}

\begin{proof}[\textbf{VERIFIED} computationally]
At each $\nu_0$, sweeping $\nu$ from $\nu_0$ to $2\nu_0$ in
8~steps yields $c < 0$ at every point.  At $\nu_0 = 0.01$
($\mathrm{Re} \approx 628$), the closest approach is
$c = -1.41$ at $\nu = 0.02 = 2\nu_0$.  Linear extrapolation
suggests $\nu_c \approx 0.022 \approx 2.2\nu_0$, but $R^2$
drops to $0.005$ near $c \approx 0$, indicating the quadratic
model itself breaks down before $c$ reaches zero.
\end{proof}

\begin{corollary}[Why Adaptive Viscosity Works:\@ Feedback
  Dynamics, Not Viscosity Level]\label{cor:why_adaptive}
The adaptive mechanism succeeds \textbf{not} because it pushes
$\nu_{\mathrm{eff}}$ above some critical threshold~$\nu_c$
(since $\nu_c > 2\nu_0$), but because the \emph{time-varying}
nature of the feedback loop creates qualitatively different
dynamics.  Specifically:
\begin{enumerate}
\item[\textup{(i)}] A constant $\nu = 0.00995$ gives $c = -8.40$
  (anti-damping).
\item[\textup{(ii)}] An adaptive run \emph{ending at}
  $\nu_{\mathrm{eff}} \approx 0.00995$ gives $c = +2.447$ (damping).
\item[\textup{(iii)}] The only difference is the
  \emph{feedback dynamics}: $\nu_{\mathrm{eff}}(t)$ increases
  \emph{in response to} enstrophy growth, creating a
  temporally correlated damping that constant viscosity cannot
  replicate.
\end{enumerate}
This is a stronger result than the original ``Bridge'' hypothesis:
BSDT feedback is not merely boosting viscosity, it is
creating a \emph{qualitatively distinct} dynamical regime.
\end{corollary}

\begin{remark}[Sign of $c$ Does Not Determine Blow-Up]
\label{rem:c_sign}
$c < 0$ does \emph{not} imply finite-time blow-up.  The
extended constant-$\nu$ simulation at $\mathrm{Re}\approx 1257$
confirms this: $c = -11.91$ over $T=5$ (growth phase only),
but the solution peaks at $t = 5.45$ and decays to $19.3\%$
of its peak by $T=20$ (Theorem~\ref{thm:saturation}).
Over the full trajectory, $c = +0.034$ with $R^2 \approx 0$.
The sign of $c$ characterises
the utility of the \emph{specific quadratic inequality form}
\eqref{eq:app_ineq} over a given time window, not the regularity
of the solution.  Boundedness holds via nonlinear vorticity
saturation mechanisms that the quadratic ansatz cannot capture.
\end{remark}

%% ──────────────────────────────────────────────────────────────────
\subsection{Reynolds Number Sweep: Suppression Scaling Law}
\label{app:ns:sweep}
%% ──────────────────────────────────────────────────────────────────

\paragraph{Setup (VERIFIED).}
15 values of $\nu$ log-spaced from $0.02$ to $10^{-4}$
($\mathrm{Re} \in [314,\, 62{,}832]$), each run in both
constant and adaptive modes at $N=128$, $T=3.0$.

Define the suppression factor
\[
  S(\mathrm{Re}) = \frac{\max_t \mathcal{E}_{\mathrm{const}}(t)}
  {\max_t \mathcal{E}_{\mathrm{adapt}}(t)}.
\]

\begin{theorem}[Suppression Scaling Law]\label{thm:suppression}
In the tested range $314 \le \mathrm{Re} \le 62{,}832$:
\begin{equation}\label{eq:scaling}
  S(\mathrm{Re}) = 1.661\,\mathrm{Re}^{-0.047},\qquad
  R^2 = 0.546,\quad p = 1.64\times 10^{-3}.
\end{equation}
\end{theorem}

\begin{proof}[\textbf{VERIFIED} computationally]
Log-linear regression on 15 data points.  The fit is
statistically significant ($p < 0.01$) but of moderate
explanatory power ($R^2 = 0.546$).
\end{proof}

\begin{corollary}[Suppression Vanishes at High Re]
\label{cor:suppression_vanishes}
The exponent $\alpha = -0.047 < 0$ means suppression
\textbf{decreases} with Reynolds number:
\begin{center}
\begin{tabular}{@{}rrl@{}}
\toprule
Re & $S(\mathrm{Re})$ & Interpretation \\
\midrule
$314$ & $1.045$ & $4.5\%$ suppression \\
$1{,}257$ & $1.025$ & $2.5\%$ suppression \\
$62{,}832$ & $1.008$ & $0.8\%$ suppression \\
$10^6$ & $\approx 1.002$ & $0.2\%$ (extrapolated) \\
\bottomrule
\end{tabular}
\end{center}
At turbulent Reynolds numbers ($\mathrm{Re} \sim 10^6$),
the factor-$2$ viscosity enhancement from the adaptive mechanism
produces negligible enstrophy suppression.
\end{corollary}

\begin{remark}
This does \emph{not} mean the adaptive mechanism ``fails'' at
high Re.  It means the peak-enstrophy \emph{ratio} between
constant and adaptive modes converges to~$1$---both modes
produce nearly identical peak enstrophy.  In absolute terms,
the adaptive system may still provide useful diagnostic
information.  However, the margin for stability improvement
shrinks.
\end{remark}

\paragraph{Key subsidiary observations (VERIFIED).}
\begin{enumerate}
\item \textbf{No blow-up in either mode.}  All 30 simulations
  (15~constant + 15~adaptive) produce smooth solutions through
  $T=3.0$.  Peak enstrophy is finite at every Re.

\item \textbf{BKM integral finite.}  $\int_0^T
  \norm{\bm{\omega}}_\infty\,dt$ is finite and bounded across
  all runs.  BKM reduction from adaptive mode ranges from
  $+13.4\%$ (Re$=314$) to $+0.21\%$ (Re$=62{,}832$),
  monotonically decreasing with Re.

\item \textbf{$\gamma^*$ saturates universally.}
  $\gamma^*_{\max} = 0.9959$ with negligible variation
  across Re.
\end{enumerate}

%% ──────────────────────────────────────────────────────────────────
\subsection{Constant-Viscosity Run: Detailed Analysis}
\label{app:ns:constant}
%% ──────────────────────────────────────────────────────────────────

\paragraph{Setup.}  $N=128$, $\nu_0=0.005$ ($\mathrm{Re}\approx 1257$),
$\mathrm{dt}=5\times 10^{-4}$, $\gamma^* \equiv 0$
(no adaptation).  Initial run to $T=5.0$ (999~snapshots),
then extended to $T=20.0$ (2000~snapshots).

\paragraph{Results at $T=5$ (VERIFIED).}
\begin{enumerate}
\item $c = -11.91$ (negative---\emph{anti-damping}).
\item $C^* = 2.272$, $R^2 = 0.652$, $p = 8.65\times 10^{-231}$.
\item Peak $\norm{\bm{\omega}}_\infty = 10.19$ at $t = 4.995$
  (the final snapshot---vorticity was still increasing).
\end{enumerate}

\paragraph{Extended run to $T=20$ (VERIFIED).}
The extended simulation resolves the trajectory completely:
\begin{center}
\begin{tabular}{@{}rl@{}}
\toprule
Quantity & Value \\
\midrule
Peak $\norm{\bm{\omega}}_\infty$ & $11.15$ at $t = 5.45$ \\
Final $\norm{\bm{\omega}}_\infty$ & $2.15$ at $t = 20.0$ \\
Ratio final/peak & $0.193$ (decayed to $19.3\%$) \\
Peak enstrophy & $1.280$ at $t \approx 5$--$6$ \\
Final enstrophy & $0.094$ at $t = 20$ \\
$\mathrm{KE}_0 \to \mathrm{KE}_{20}$ & $0.125 \to 0.012$ \\
$c$ over full $T=20$ & $+0.034$ ($R^2 \approx 0$) \\
\bottomrule
\end{tabular}
\end{center}
Vorticity peaked at $t = 5.45$ and \textbf{decayed monotonically}
for $t > 7$.  The solution is bounded.  The constant-$\nu$ system
at $\mathrm{Re} \approx 1257$ \textbf{does not blow up}.

\begin{remark}[The Quadratic Inequality is the Wrong
  Diagnostic]\label{rem:constant_implications}
The value $c = -11.91$ from the $T=5$ run was an artifact of
fitting only the \emph{growth phase} of a transient:
\begin{enumerate}
\item[\textup{(a)}] Over the full trajectory ($T=20$), $c = +0.034$
  with $R^2 \approx 0$---the quadratic model has
  \textbf{zero explanatory power}.  The trajectory
  includes growth, peak, and decay phases that the
  quadratic ansatz $\dot{\omega} \le -c\nu\omega^2 + C^*$
  cannot represent.

\item[\textup{(b)}] The prior claim of $c = +2.447$ at ``constant
  $\nu$'' used an adaptive run (time-varying $\nu_{\mathrm{eff}}$),
  not constant viscosity (Theorem~\ref{thm:regime}).

\item[\textup{(c)}] Boundedness of the constant-$\nu$ solution
  is \emph{not} achieved through the quadratic damping
  mechanism.  It arises from nonlinear vorticity saturation
  and kinetic energy dissipation---mechanisms that the
  single-mode inequality~\eqref{eq:app_ineq} cannot capture.
\end{enumerate}
\end{remark}

%% ──────────────────────────────────────────────────────────────────
\subsection{BSDT as Detection--Feedback Controller}
\label{app:ns:feedback}
%% ──────────────────────────────────────────────────────────────────

The adaptive system~\eqref{eq:app_ns} achieves stability through
a closed-loop mechanism: BSDT channels \emph{detect} anomalous
dynamics and the adaptive viscosity \emph{responds}.  We formalise
this.

\paragraph{The feedback loop.}
At each diagnostic step:
\[
  \underbrace{\mathcal{E}(t) \text{ grows}}_{\text{physics}}
  \;\longrightarrow\;
  \underbrace{\delta_C(t) \text{ rises}}_{\text{detection}}
  \;\longrightarrow\;
  \underbrace{E_{\mathrm{BS}}(t) \text{ rises}}_{\text{alarm score}}
  \;\longrightarrow\;
  \underbrace{\gamma^*(t) \text{ rises}}_{\text{response}}
  \;\longrightarrow\;
  \underbrace{\nu_{\mathrm{eff}}(t) \text{ rises}}_{\text{control}}
  \;\longrightarrow\;
  \underbrace{D \text{ increases}}_{\text{dissipation}}.
\]
The viscosity $\nu_{\mathrm{eff}}(t)$ is applied at \emph{every}
time step through the semi-implicit integrating factor
$\exp(-\nu_{\mathrm{eff}} |\bm{k}|^2 \Delta t)$.  The response
is therefore continuous in time: BSDT alarm triggers viscosity
enhancement \emph{before} enstrophy has time to grow
catastrophically.

\begin{proposition}[Monotone Negative Feedback]\label{prop:feedback}
The detection--response map
$\mathcal{E} \mapsto \delta_C \mapsto E_{\mathrm{BS}} \mapsto
\gamma^* \mapsto \nu_{\mathrm{eff}} \mapsto D$
is a \emph{monotone negative feedback controller}:
\begin{enumerate}
\item[\textup{(i)}] \textbf{Detection is monotone.}
  $\delta_C(t) = |\mathcal{E}(t) - \bar{\mathcal{E}}_t|/\sigma_t$
  is monotone increasing in $|\mathcal{E}(t) - \bar{\mathcal{E}}_t|$
  (holding history fixed).

\item[\textup{(ii)}] \textbf{Response is monotone and concave.}
  $\gamma^*(E) = E/(E+\theta)$ satisfies $(\gamma^*)' > 0$,
  $(\gamma^*)'' < 0$: fast activation, saturating response.

\item[\textup{(iii)}] \textbf{Dissipation is increasing.}
  $D = \nu_{\mathrm{eff}} \norm{\grad\bm{\omega}}_{L^2}^2$
  is monotone increasing in $\nu_{\mathrm{eff}}$
  (holding vorticity fixed).

\item[\textup{(iv)}] \textbf{The loop is negative feedback.}
  Increased enstrophy $\to$ increased dissipation.  The feedback
  opposes the perturbation.
\end{enumerate}
\end{proposition}

\begin{proof}
(i) is immediate from the definition~\eqref{eq:app_dC}.
(ii): $(\gamma^*)'(E) = \theta/(E+\theta)^2 > 0$,
$(\gamma^*)''(E) = -2\theta/(E+\theta)^3 < 0$.
(iii)~and~(iv) follow from $\nu_{\mathrm{eff}} = \nu_0(1+\gamma^*)
\ge \nu_0$.
\end{proof}

\begin{theorem}[BSDT Feedback Creates a Distinct Dynamical
  Regime]\label{thm:bsdt_transition}
The BSDT detection--feedback loop produces stability through
\emph{temporally correlated} viscosity enhancement, not through
any static viscosity threshold:
\begin{enumerate}
\item[\textup{(i)}]
  When enstrophy is quiescent ($E_{\mathrm{BS}} \approx 0$),
  $\nu_{\mathrm{eff}} \approx \nu_0$ and the system evolves
  at the original Reynolds number with minimal overhead.

\item[\textup{(ii)}]
  When enstrophy begins to grow anomalously,
  $\delta_C$ rises, pushing $E_{\mathrm{BS}}$ above $\theta$
  and $\gamma^* > 1/2$.  The viscosity increases
  \emph{in temporal correlation with the growth event}.

\item[\textup{(iii)}]
  At saturation ($\gamma^*_{\mathrm{sat}} \approx 0.996$,
  computational observation at all tested Re), $\nu_{\mathrm{eff}}
  \approx 2\nu_0$ and the \emph{adaptive} system fits
  $c > 0$.  However, the \emph{same} $\nu_{\mathrm{eff}}$
  held constant yields $c < 0$
  (Theorem~\ref{thm:nu_crit}).

\item[\textup{(iv)}]
  The critical difference is that $\nu_{\mathrm{eff}}(t)$
  increases precisely when enstrophy is growing, making
  the dissipation \emph{anti-correlated} with production.
  Constant viscosity cannot create this temporal correlation.
\end{enumerate}
\end{theorem}

\begin{proof}
Parts~(i)--(ii): $E_{\mathrm{BS}} = 0 \implies \gamma^* = 0$;
monotonicity follows from Proposition~\ref{prop:feedback}.
Part~(iii): computational observation $\gamma^*_{\max}
= 0.9959$ across all 15 Re values (Section~\ref{app:ns:sweep}),
constant-$\nu$ sweep confirms $c < 0$ throughout
$[\nu_0, 2\nu_0]$ (Theorem~\ref{thm:nu_crit}).
Part~(iv): in the feedback loop, $\nu_{\mathrm{eff}}$ rises when
$\delta_C$ rises (enstrophy growth), creating a negative
cross-correlation between production and dissipation lag.
\end{proof}

\begin{remark}[Why BSDT Provides Early Warning]
In the adaptive loop, BSDT channels fire \emph{before}
enstrophy reaches dangerous levels.  The sequence is:
(1)~enstrophy begins departing from its running mean;
(2)~$\delta_C$ exceeds threshold, raising $E_{\mathrm{BS}}$;
(3)~$\gamma^*$ rises, increasing viscosity;
(4)~enhanced dissipation suppresses the growth before it can
develop into a singularity threat.
The ``early'' warning is relative to the enstrophy peak that
\emph{would} occur without adaptive control.  Since control
intervenes before the peak, the system never reaches the
dangerous state.  This is the standard mechanism of
feedback-based early intervention:
the alarm precedes the event \emph{that the alarm prevents}.
\end{remark}

\paragraph{Computational evidence (VERIFIED).}
The sweep data (Section~\ref{app:ns:sweep}) shows this
mechanism operating across all~15 Reynolds numbers:
$\gamma^*$ saturates rapidly (within $O(1)$ time units),
all adaptive runs produce smooth solutions with bounded
enstrophy, and the suppression factor $S > 1$ at every Re.
Critically, the dense viscosity sweep
(Section~\ref{app:ns:nu_c_experiment}) confirms that
$c < 0$ at \emph{every} constant viscosity in $[\nu_0, 2\nu_0]$.
Yet with BSDT feedback (time-varying $\nu_{\mathrm{eff}}$),
$c > 0$.  \textbf{The sign of $c$ is controlled by the
temporal correlation of feedback, not by the viscosity level.}

%% ──────────────────────────────────────────────────────────────────
\subsection{BSDT Channel Behaviour at Constant Viscosity}
\label{app:ns:channels_const}
%% ──────────────────────────────────────────────────────────────────

When the feedback loop is \emph{disabled} ($\gamma^* \equiv 0$),
the BSDT channels continue to compute but no longer drive
viscosity.  They function as passive monitors.

\paragraph{Observations (VERIFIED).}
At $\nu = 0.005$ (constant), $T=5.0$:
\begin{enumerate}
\item $\delta_G$: largest at $t \approx 0.005$.  The
  Taylor--Green spectrum is far from $k^{-5/3}$ at
  initialisation, so $\delta_G$ correctly detects a
  spectral anomaly---the energy distribution is non-Kolmogorov.
  As the cascade develops, the spectrum approaches $k^{-5/3}$
  and $\delta_G$ decreases.  This is genuine anomaly detection
  (the initial state IS anomalous relative to developed
  turbulence), but it is not predictive of \emph{blow-up}
  specifically: any smooth IC triggers the same signal.

\item $\delta_C$: z-score of enstrophy relative to running mean.
  Grows monotonically as enstrophy departs from its history.
  At constant~$\nu$, $\delta_C$ functions as a \emph{concurrent}
  monitor of enstrophy growth---it tracks the departure in real
  time.  In the adaptive loop (Section~\ref{app:ns:feedback}),
  this same signal drives the protective response.

\item $\delta_T$: measures relative spectral change rate.
  Spikes at $t = \Delta t$ (the first step exhibits maximal
  relative change from the IC), then decays as the flow
  equilibrates.

\item $\delta_A \equiv 0$ (not implemented in the solver).
\end{enumerate}

\paragraph{Distinction.}
The channels detect genuine physical features (spectral
anomaly, enstrophy departure, rapid spectral change)
even at constant viscosity.  The difference is that
without the feedback loop, detection does not trigger
corrective action.  The initial signals in $\delta_G$
and $\delta_T$ are \emph{physically correct detections}
(the IC state is genuinely anomalous), but they are not
early warnings of singularity formation---they fire for
any smooth initial condition, regardless of subsequent
regularity.

%% ──────────────────────────────────────────────────────────────────
\subsection{The Gap: From Adaptive to Classical Regularity}
\label{app:ns:gap}
%% ──────────────────────────────────────────────────────────────────

The central question is whether regularity of the adaptive
system~\eqref{eq:app_ns} implies regularity of the classical
system (constant $\nu_0$).  The simulation data delineates the gap
precisely.

\begin{theorem}[Two-Regime Structure]\label{thm:three_regimes}
The simulations identify two distinct regimes,
distinguished \textbf{not by viscosity level but by
feedback dynamics}:
\begin{center}
\begin{tabular}{@{}lcccl@{}}
\toprule
System & $\nu_{\mathrm{eff}}$ & $c$ & Inequality & Status \\
\midrule
Adaptive (feedback ON) & varies in $[\nu_0, 2\nu_0)$
  & $+2.447$ & Holds & VERIFIED \\
Constant $\nu = 0.00995$ & fixed at $2\nu_0$
  & $-8.40$ & FAILS & VERIFIED \\
Constant $\nu = 0.005$ & fixed at $\nu_0$
  & $-11.91$ & FAILS & VERIFIED \\
\bottomrule
\end{tabular}
\end{center}
The key distinction is row~1 vs row~2: the
\emph{same} viscosity level ($\approx 0.00995$) gives
opposite sign of~$c$ depending on whether it is
constant or adaptive.  Damping is a property of the
\emph{feedback loop}, not the viscosity magnitude.
\end{theorem}

\begin{proof}
Row~1: adaptive run ($\gamma^*$ active) as in
Section~\ref{app:ns:sweep}.
Rows~2--3: dense viscosity sweep
(Section~\ref{app:ns:nu_c_experiment}):
11~constant-$\nu$ runs in $[0.005, 0.00995]$, all
yielding $c < 0$.
\end{proof}

\paragraph{The gap in one sentence.}
The adaptive mechanism works through \emph{temporally
correlated feedback}, not by simply raising viscosity.
Any ``bridge'' from adaptive to classical regularity
must explain why dynamic feedback produces $c > 0$
while the same viscosity held constant produces
$c < 0$---a fundamentally different challenge from
locating a critical threshold $\nu_c$.

%% ──────────────────────────────────────────────────────────────────
\subsection{Conjectures and Open Problems}
\label{app:ns:conjectures}
%% ──────────────────────────────────────────────────────────────────

The simulation data motivates several open problems.

\begin{conjecture}[Critical Viscosity Function]\label{conj:nu_c}
For a given initial condition and resolution, define
$\nu_c(\mathrm{Re}, \bm{u}_0)$ as the viscosity at which the
fitted damping coefficient $c(\nu) = 0$.  The dense
viscosity sweep (Section~\ref{app:ns:nu_c_experiment})
shows $c < 0$ throughout $[\nu_0, 2\nu_0]$ at all
tested Re, so $\nu_c > 2\nu_0$ by at least a small margin.
The following are open:
\begin{enumerate}
\item[\textup{(i)}]
  Does $\nu_c$ converge to a resolution-independent value as
  $N \to \infty$ with fixed IC?
\item[\textup{(ii)}]
  Does $\nu_c$ exist at all, or does $c(\nu)$
  remain negative for all $\nu > 0$?  The declining
  $R^2$ near $c \approx 0$ (e.g., $R^2 = 0.005$ at
  $\nu = 2\nu_0$, $\mathrm{Re} \approx 628$) suggests
  the quadratic model~\eqref{eq:app_ineq} itself breaks
  down before $c$ reaches zero.
\item[\textup{(iii)}]
  If $\nu_c$ exists, how does $\nu_c / \nu_0$ scale with Re?
\end{enumerate}
\end{conjecture}

\begin{theorem}[Vorticity Saturation at Constant
  Viscosity]\label{thm:saturation}
At $\nu = 0.005$ ($\mathrm{Re}\approx 1257$, $N=128$,
Taylor--Green IC), the vorticity
$\norm{\bm{\omega}}_\infty(t)$ reaches a finite maximum
$\norm{\bm{\omega}}_\infty^* = 11.15$ at $t^* = 5.45$
and subsequently decays to $19.3\%$ of peak by $T=20$.
The constant-$\nu$ solution is bounded.
\end{theorem}

\begin{proof}[\textbf{VERIFIED} computationally]
Extended run to $T=20$ on NVIDIA RTX PRO 6000 Blackwell.
Vorticity peaked at $t=5.45$, then decayed monotonically
for $t > 7$ (see Section~\ref{app:ns:nu_c_experiment},
Experiment~3 and panels~C,~D of the experiment figure).
Final values: $\norm{\bm{\omega}}_\infty(20) = 2.15$,
$\mathcal{E}(20) = 0.094$, $\mathrm{KE}(20) = 0.012$.
\end{proof}

\begin{remark}
This resolves the ambiguity from the $T=5$ run, where
vorticity was still increasing at the final snapshot.
The growth was a transient, not incipient blow-up.
The formerly conjectured saturation
(Conjecture~A.15 in the previous version) is now
verified.
\end{remark}

\begin{conjecture}[Feedback Dynamics as the Mechanism of
  Damping]\label{conj:feedback_dynamics}
The positive damping coefficient $c > 0$ observed in the adaptive
system is a property of the \emph{temporal correlation} between
viscosity enhancement and enstrophy growth, not of the viscosity
level per~se.  Formally: for any time-invariant viscosity
$\nu \in [\nu_0, 2\nu_0]$, the fitted $c < 0$, but the adaptive
law $\nu_{\mathrm{eff}}(t) = \nu_0(1 + \gamma^*(E_{\mathrm{BS}}(t)))$
with the same time-averaged viscosity yields $c > 0$.

If true, this identifies the \emph{production-dissipation
cross-correlation} as the essential mathematical object:
the feedback creates a negative correlation between $P(t)$
(production) and $D(t+\epsilon)$ (dissipation at a short lag),
which constant viscosity cannot generate.
\end{conjecture}

%% ──────────────────────────────────────────────────────────────────
\subsection{Honest Assessment: What These Results Establish}
\label{app:ns:assessment}
%% ──────────────────────────────────────────────────────────────────

We summarise what is proved, what is computationally verified,
and what remains open.

\paragraph{Proved (from definitions and standard analysis).}
\begin{enumerate}
\item The adaptive viscosity satisfies $\nu_0 \le \nu_{\mathrm{eff}}
  < 2\nu_0$ (Proposition~\ref{prop:nu_bounds}).
\item The adaptive system has Leray--Hopf weak solutions.
\item At saturation, $\mathrm{Re}_{\mathrm{eff}} \approx
  \mathrm{Re}_0 / 2$ (Theorem~\ref{thm:re_halving}).
\item The BSDT detection--response map is a monotone negative
  feedback controller (Proposition~\ref{prop:feedback}).
\end{enumerate}

\paragraph{Computationally verified (reproducible, subject to
  numerical resolution).}
\begin{enumerate}
\item $\gamma^*$ saturates at $\approx 0.996$ across all tested Re
  (Proposition~\ref{prop:saturation}).
\item The damping inequality~\eqref{eq:app_ineq} with
  $c > 0$ holds \textbf{only} in adaptive mode
  ($c = +2.447$ with time-varying $\nu_{\mathrm{eff}}$).
  At constant~$\nu$, $c < 0$ for \emph{every} $\nu \in
  [\nu_0, 2\nu_0]$, at all four tested Re
  (Theorem~\ref{thm:regime}, Theorem~\ref{thm:nu_crit}).
\item Suppression decreases with Re: $S \sim
  \mathrm{Re}^{-0.047}$ (Theorem~\ref{thm:suppression}).
\item No blow-up in any of 30+ simulations through $T = 3$--$20$,
  $\mathrm{Re}$ up to $62{,}832$.
\item The BKM integral remains finite at all tested parameters.
\item BSDT early warning operates in the adaptive loop:
  $\gamma^*$ saturates within $O(1)$ time units at every
  tested Re, driving $\nu_{\mathrm{eff}}$ from $\nu_0$ to
  $\approx 2\nu_0$ before enstrophy peaks.
\item $\nu_c > 2\nu_0$ at all tested Re: the quadratic
  inequality with $c > 0$ is unreachable by any constant
  viscosity in the adaptive range
  (Section~\ref{app:ns:nu_c_experiment}).
\item Vorticity saturation at constant $\nu = 0.005$: peak
  $\norm{\bm{\omega}}_\infty = 11.15$ at $t = 5.45$,
  decayed to $19.3\%$ of peak by $T = 20$
  (Theorem~\ref{thm:saturation}).  The $c = -11.91$ from
  $T=5$ was a growth-phase artifact.
\item The quadratic damping model has $R^2 \approx 0$ over
  the full $T=20$ trajectory; vorticity boundedness
  arises from nonlinear saturation, not quadratic damping.
\end{enumerate}

\paragraph{Not established (open).}
\begin{enumerate}
\item Global regularity for classical constant-$\nu$ NS
  (bounded at $\mathrm{Re} \approx 1257$, $N=128$;
  not tested at higher Re or resolution).
\item Global regularity for adaptive NS with arbitrary initial
  data (only Taylor--Green tested).
\item Why time-varying $\nu_{\mathrm{eff}}$ produces $c > 0$
  while the same average viscosity held constant produces
  $c < 0$ (Conjecture~\ref{conj:feedback_dynamics}).
\item Any bridge from adaptive to classical regularity.
\end{enumerate}

\paragraph{Previously overclaimed (corrected here).}
\begin{enumerate}
\item The ``$c = 2.447$ at constant $\nu$'' result was in fact
  from an \emph{adaptive} run (time-varying $\nu_{\mathrm{eff}}$,
  saturating at $\approx 0.00995$), not a constant-$\nu$
  simulation.  The dense sweep experiment confirms
  $c = -8.40$ at constant $\nu = 0.00995$---the
  \emph{opposite} sign.

\item ``0 violations'' of the inequality is tautological
  when $C^*$ is defined as the maximum residual.

\item The original Theorem claiming $\nu_c \in (0.005, 0.00995)$
  via the intermediate value theorem was based on comparing
  a constant-$\nu$ run ($c = -11.91$) with an adaptive run
  ($c = +2.447$)---two \emph{different} systems.  The IVT
  does not apply across different dynamical systems.
  The corrected result (Theorem~\ref{thm:nu_crit}) shows
  $\nu_c > 2\nu_0$.

\item At constant~$\nu$, initial signals in $\delta_G$ and
  $\delta_T$ are physically correct detections (the IC state
  is genuinely anomalous relative to developed turbulence)
  but are not specific predictors of blow-up---they fire
  for any smooth initial condition.  In the \emph{adaptive}
  loop, by contrast, these same channels drive the protective
  viscosity response and constitute genuine early warning
  (Section~\ref{app:ns:feedback}).
\end{enumerate}

%% ──────────────────────────────────────────────────────────────────
\subsection{The Road Forward}
\label{app:ns:road}
%% ──────────────────────────────────────────────────────────────────

The experiment results (Section~\ref{app:ns:nu_c_experiment})
reveal two striking findings: (i)~the positive damping
coefficient $c > 0$ is a property of the \emph{feedback
dynamics}, not the viscosity level, and (ii)~the constant-$\nu$
solution is bounded despite $c < 0$, with vorticity peaking
at $t = 5.45$ and decaying thereafter
(Theorem~\ref{thm:saturation}).  Two directions for future
work arise:

\begin{enumerate}
\item \textbf{Characterise the temporal correlation mechanism.}
  The adaptive system achieves $c > 0$ while the same viscosity
  held constant gives $c < 0$.  The mathematical mechanism
  must involve the \emph{cross-correlation} between
  $\nu_{\mathrm{eff}}(t)$ and enstrophy growth rate.
  Formalising this requires analysing the production--dissipation
  phase relationship in the feedback loop.

\item \textbf{Resolution and IC dependence.}  Repeat the dense
  viscosity sweep at $N = 256$ and $N = 384$ with both
  Taylor--Green and random-phase initial conditions to test
  universality of the $\nu_c > 2\nu_0$ result and vorticity
  saturation at higher Re.
\end{enumerate}

The key open question is no longer ``Does $\nu_c / \nu_0 < 2$?''
(answered: no), but rather: \emph{What mathematical property of
time-varying viscosity creates damping that constant viscosity
cannot?}

%% ──────────────────────────────────────────────────────────────────
\subsection{Dense Viscosity Sweep: Experimental Data}
\label{app:ns:nu_c_experiment}
%% ──────────────────────────────────────────────────────────────────

The following experiment was run on an NVIDIA RTX PRO 6000
(Blackwell, 102\,GB VRAM) using CuPy~14.0.1, with the
identical solver as all preceding simulations ($N=128$,
Taylor--Green IC, semi-implicit integrator, $\theta=1$).

\paragraph{Experiment~1: Dense $\nu$ sweep at $\mathrm{Re}_0
\approx 1257$.}
Eleven constant-viscosity runs, $\nu \in [0.005, 0.00995]$,
each at $T=5.0$ with \texttt{adaptive=False}:
\begin{center}
\begin{tabular}{@{}ccccl@{}}
\toprule
$\nu$ & $\mathrm{Re}$ & $c$ & $R^2$ & Comment \\
\midrule
$0.0050$ & $1257$ & $-11.91$ & $0.652$ & \\
$0.0055$ & $1142$ & $-11.75$ & $0.681$ & \\
$0.0060$ & $1047$ & $-11.49$ & $0.693$ & \\
$0.0065$ &  $967$ & $-11.14$ & $0.691$ & \\
$0.0070$ &  $898$ & $-10.74$ & $0.678$ & \\
$0.0075$ &  $838$ & $-10.32$ & $0.656$ & \\
$0.0080$ &  $785$ & $-9.89$  & $0.628$ & \\
$0.0085$ &  $739$ & $-9.48$  & $0.595$ & \\
$0.0090$ &  $698$ & $-9.08$  & $0.559$ & \\
$0.0095$ &  $661$ & $-8.71$  & $0.522$ & \\
$0.00995$&  $631$ & $-8.40$  & $0.487$ & $\approx 2\nu_0$ \\
\bottomrule
\end{tabular}
\end{center}
Result: $c < 0$ \textbf{everywhere}.  $c$ increases monotonically
toward zero (from $-11.91$ to $-8.40$), but $R^2$ also
\emph{drops} monotonically (from $0.652$ to $0.487$), meaning
the quadratic model fits \emph{worse} at higher $\nu$.
Linear extrapolation would place $\nu_c \approx 0.022
\approx 4.4\nu_0$, but the declining $R^2$ makes this
extrapolation unreliable.

\paragraph{Experiment~2: $\nu_c$ scaling with Re.}
At each of four base viscosities, sweep constant $\nu$ from
$\nu_0$ to $2\nu_0$ in 8~steps:
\begin{center}
\begin{tabular}{@{}cccccl@{}}
\toprule
$\nu_0$ & $\mathrm{Re}_0$ & $c$ at $\nu_0$ & $c$ at $2\nu_0$
  & $\nu_c > 2\nu_0$? & Adaptive $c$ \\
\midrule
$0.010$ & $628$  & $-8.36$  & $-1.41$ & Yes & $+2.447^*$ \\
$0.005$ & $1257$ & $-11.91$ & $-8.36$ & Yes & $+2.447$ \\
$0.002$ & $3142$ & $-10.40$ & $-12.73^\dagger$ & Yes & --- \\
$0.001$ & $6283$ & $-11.77$ & --- & Yes & --- \\
\bottomrule
\end{tabular}
\end{center}
{\footnotesize ${}^*$Inferred from literature; ${}^\dagger$Non-monotone
$c(\nu)$ at high Re.}

\noindent
At \emph{every} tested Re, all constant-$\nu$ runs yield $c < 0$.
At $\mathrm{Re} \approx 628$, $c = -1.41$ at $2\nu_0$ is closest
to zero but still negative ($R^2 = 0.005$---the quadratic model
has essentially zero explanatory power).

\paragraph{Key finding.}
The previously reported $c = +2.447$ arose from an \emph{adaptive}
run (time-varying $\nu_{\mathrm{eff}}$), not a constant-$\nu$ run.
Comparing constant-$\nu = 0.00995$ ($c = -8.40$) with
adaptive $\nu_{\mathrm{eff}} \to 0.00995$ ($c = +2.447$)
demonstrates that \textbf{the feedback loop itself, not the
viscosity magnitude, is the mechanism that creates positive
damping}.  This is a stronger---and more interesting---result
than the original ``bridge'' hypothesis.

\paragraph{Experiment~3: Extended run at $\nu = 0.005$,
$T = 20$.}
A single constant-viscosity run extended to $T=20$
(2000~snapshots, 300\,s wall time):
\begin{center}
\begin{tabular}{@{}cccc@{}}
\toprule
$t$ & $\mathrm{KE}$ & $\mathcal{E}$ & $\norm{\bm{\omega}}_\infty$ \\
\midrule
$0$    & $0.125$ & $0.375$  & $2.00$ \\
$3$    & $0.111$ & $0.701$  & $2.92$ \\
$5$    & $0.092$ & $1.163$  & $10.21$ \\
$5.45$ & ---     & ---      & $11.15$ \emph{(peak)} \\
$7$    & $0.067$ & $1.190$  & $7.10$ \\
$10$   & $0.039$ & $0.706$  & $5.76$ \\
$15$   & $0.018$ & $0.218$  & $2.13$ \\
$20$   & $0.012$ & $0.094$  & $2.15$ \\
\bottomrule
\end{tabular}
\end{center}
The trajectory has three clear phases: (1)~growth
($t \in [0, 5.45]$), (2)~rapid decay ($t \in [5.45, 12]$),
(3)~slow viscous decay ($t > 12$).  Over the full trajectory,
$c = +0.034$ with $R^2 \approx 0$: the quadratic model
has zero explanatory power when both growth and decay
phases are included.

\paragraph{Implication for the $c = -11.91$ result.}
The negative $c$ from the $T = 5$ run is an artifact of
fitting only the growth phase of a bounded transient.
It does not indicate instability or approach to blow-up.
Similarly, the declining $R^2$ with increasing $\nu$
(from $0.652$ at $\nu = 0.005$ to $0.487$ at $\nu = 0.00995$)
reflects that the growth phase is shorter and weaker at higher
viscosity, making the quadratic fit progressively less meaningful.
