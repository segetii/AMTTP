...
\midrule
Peak $\norm{\bm{\omega}}_\infty$ & $11.15$ & $11.19$ & $0.4\%$ \\
Peak time $t^*$ & $5.45$ & $\approx 5.45$ & $< 1\%$ \\
\bottomrule
\end{tabular}
\end{center}
The $0.4\%$ agreement in peak vorticity between $N=128$ and
...
$T=8.0$, identical solver parameters.
\end{proof}
...
\item \textbf{$c$ depends on the time window, not physics.}
  ...
  Growth + decay ($T=8$, $N=256$) & $+1.33$ & Avg of both \\
...
If true at all Re, this identifies the
\emph{production-dissipation cross-correlation}
$\mathrm{Corr}(P(t), D(t+\epsilon))$ as the essential
mathematical object distinguishing adaptive from constant
viscosity dynamics.
\end{conjecture}
...
$\mathrm{Re}$ up to $62{,}832$ (Section~\ref{app:ns:sweep})
and in detail at $\mathrm{Re} \approx 1257$
(Theorem~\ref{thm:saturation}).  The conjecture
...
\item Global regularity for classical constant-$\nu$ NS
  ...
  through $T = 3$; verified in detail at
  $\mathrm{Re} \approx 1257$ through $T = 20$).
...
\begin{enumerate}
\item \textbf{High-Re extended runs.}  Repeat the $T=20$
  extended simulation at $\mathrm{Re} = 6283$ and
  $\mathrm{Re} = 62{,}832$ (requiring $N \ge 512$ for
  resolution) to test whether vorticity saturation
  persists at turbulent Reynolds numbers.

\item \textbf{Random-phase initial conditions.}  All
  current results use the Taylor--Green vortex, which
  has special symmetry.  Random-phase IC with prescribed
  energy spectrum $E(k) \sim k^2\exp(-k^2/k_0^2)$
  would test robustness.

\item \textbf{Formal P/D attractor analysis.}  Investigate
  whether $P/D \to 1/2$ in the adaptive system is a
  consequence of the specific feedback law
  $\gamma^* = E/(E+\theta)$ or is universal across
  saturating feedback laws.

\item \textbf{3D vortex stretching diagnostics.}  Compute
  the vorticity-strain alignment channel $\delta_A$
  (currently $\equiv 0$) and assess whether it provides
  additional predictive power for singularity detection
  beyond the three active channels.

\item \textbf{Production-dissipation phase relationship.}
  Measure the cross-correlation
  $\mathrm{Corr}(P(t), D(t+\epsilon))$ as a function
  of lag~$\epsilon$ in both adaptive and constant-$\nu$
  modes to quantify the temporal correlation mechanism
  identified in Theorem~\ref{thm:bsdt_transition}.
\end{enumerate}

The key open question is: \emph{What mathematical property
of temporally correlated viscosity enhancement creates a
controlled production-dissipation balance ($P/D \to 1/2$)
that constant viscosity cannot achieve?}
