...
%%  Title & metadata
%% ====================================================================
\title{Blind-Spot Decomposition and the Geometry of System Collapse\thanks{Submitted to the editors March 14, 2026.}}
...
\thispagestyle{plain}
\markboth{O.\ ODEYEMI}{BLIND-SPOT DECOMPOSITION AND THE GEOMETRY OF SYSTEM COLLAPSE}
...
structural instability in coupled dynamical systems via
energy-topology equivalence.
A \emph{Blind-Spot Decomposition Theory} (BSDT) energy functional
\cite{odeyemi2025bsdt} $\Ebs$ is shown to be topologically
equivalent to the system potential $\Phi$ through a Morse-index
equivalence: $\mathrm{ind}_{\Ebs}(X_c) = \mathrm{ind}_\Phi(X_c)$
at every critical point, with two-sided gradient bounds and homotopy
equivalence of sublevel sets.
This recasts instability detection as a control problem on a
\emph{critical manifold} $\Cman$ where adaptive friction
$\gamma^*(X)=\alpha/\lmax(D^2\Phi_{\mathrm{pair}})$ achieves
marginal stability---formalising procyclicality in financial
regulation.
Two physics-based \emph{System Mode engines} operationalise the
theory with Lyapunov-certified convergence, fused topological
scoring, and distribution-free conformal calibration.
The framework is validated across three domains:
FDIC banking data (6-quarter GFC lead, AUROC $= 0.867$,
zero false alarms), ERCOT power-grid dispatch ($+72$\,h lead,
AUC $= 0.9997$), and three-dimensional incompressible
Navier--Stokes via a 34-run GPU stress test
($N$ up to $256^3$, $\mathrm{Re}$ up to $62{,}832$,
NVIDIA RTX PRO 6000 Blackwell, 532\,min wall time)
on the \emph{modified} (adaptive-viscosity) equation.
All runs complete without blow-up: adaptive viscosity
reduces peak vorticity by $1.77\times$ at
$\mathrm{Re}\approx 1257$; the BKM integral remains finite
at $\mathrm{Re}\approx 62{,}832$
($\|\bm{\omega}\|_\infty^{\max}=132.2$, BKM $=465.4$);
twelve random-phase initial conditions confirm IC-independence;
five distinct feedback laws produce identical dynamics
(spread $< 0.04\%$); and cross-correlations
$\mathrm{Corr}(P,D)>0.85$ demonstrate tight
production-dissipation coupling across the full
laminar-to-turbulent range.
\end{abstract}

\begin{keywords}
systemic risk, Lyapunov stability, entropy production, Onsager
gradient flow, phase transition, adaptive friction,
Navier--Stokes regularity, enstrophy, BKM criterion, blind-spot decomposition,
...
\paragraph{Scope of the geometric claim.}
A clarification is essential.  The claim is \emph{not} that banking
networks, power grids, and fluid flows share the same literal
physical energy law.  Rather, BSDT constructs a
\emph{geometry of collapse}: a detection functional $\Ebs$ that
induces an energy-topology structure in which system failure becomes
observable as a Morse-index jump.  In domains with genuine physical
energy (Navier--Stokes, where $\Phi$ is enstrophy), the
equivalence has direct physical content.  In domains without
intrinsic energy (banking, where institutions have no literal
potential), $\Phi$ is a surrogate instability landscape
constructed from agent interactions --- the energy is real within
the model, not a metaphor.  The theorems hold \emph{within the
BSDT-induced geometry}; cross-domain transfer arises because the
construction is \emph{representational}, not because distinct
physical laws happen to coincide.
...
\item \textbf{Cross-domain collapse geometry.} The same geometric
  ...
  Navier--Stokes turbulence.  The energy-topology
  structure transfers across domains not because each domain obeys
  the same physical law, but because each admits a BSDT-induced
  collapse geometry in which failure is a Morse-index jump.
  For Navier--Stokes, $\Phi$ is the physical enstrophy;
  for banking, $\Phi$ is a surrogate instability landscape with
  no literal energetic content.  The shared structure is
  \emph{geometric}---the same theorem applies to both because both
  satisfy the same regularity and non-degeneracy conditions, not
  because banking ``is'' fluid dynamics.
...
regularity backdrop.  On the computational side, high-resolution
DNS studies by Kerr~\cite{kerr1993} and Hou \&
Li~\cite{houli2006} investigated potential finite-time blow-up of
the 3D Euler equations, finding strong vortex-sheet formation but
no definitive singularity.  In the turbulence-modelling
literature, Large Eddy Simulation (LES) with the dynamic
Smagorinsky model~\cite{smagorinsky1963,germano1991} adapts an
eddy viscosity based on the \emph{resolved strain rate};
hyperviscosity~($\nu\nabla^{2p}$) and spectral vanishing
viscosity~\cite{tadmor1989} add dissipation targeted at high
wavenumbers.  Our adaptive viscosity differs from all of these:
it responds to a \emph{topological energy functional}
($E_{\mathrm{BS}}$), not to local strain rate or spectral
content, and carries a Lyapunov certificate
(Proposition~\ref{prop:ns_lyapv2}) that LES closures lack.
...
The foundational formulation of Blind-Spot Decomposition Theory
was introduced in \cite{odeyemi2025bsdt}; the present paper
extends the framework with Morse-index equivalence, adaptive
friction control, and cross-domain validation.
...
\item \textbf{Camouflage} ($\delta_C$): Mahalanobis distance from
  ...
  \[
    \delta_C(X)
    = \bigl[(X{-}\mu_0)^\top\Sigma_0^{-1}(X{-}\mu_0)\bigr]^{1/2}.
  \]
...
\subsection{Applicable System Class}

\begin{definition}[BSDT-admissible system]\label{def:admissible}
A \textbf{BSDT-admissible system} is a triple
$(\mathcal{X}, \Phi, \{\delta_k\}_{k=1}^K)$ where:
(i)~$X \in \mathcal{X}\subseteq\R^{Nd}$ evolves under a coupled
gradient flow $\dot{X} = -\grad\Phi(X) + F_{\mathrm{pair}}(X)$
with $\Phi\in C^2$ coercive;
(ii)~$\Phi$ has at least one strict local minimum and at least
one saddle point separated by an energy barrier (metastability);
(iii)~$F_{\mathrm{pair}} = -\grad\Phi_{\mathrm{pair}}$ derives from
a pairwise potential; and
(iv)~the channels $\delta_k$ are $C^1$, Lipschitz on sublevel sets.
\end{definition}

\begin{remark}[A priori applicability]\label{rem:apriori}
One can determine admissibility \emph{before} running the
framework.  The financial system of~\eqref{eq:gradflow} with
LJ~\eqref{eq:lj} or Gaussian/Coulomb~\eqref{eq:gravforce} pairwise
interactions is manifestly admissible: the potential is $C^2$ and
coercive (Assumption~\ref{ass:reg}), the equilibrium exists with
$H^*\succ 0$ (Assumption~\ref{ass:equil}), and the four channels
are Lipschitz on compact sets (Assumption~\ref{ass:bsdt}).
The Navier--Stokes formulation (Section~\ref{sec:ns}) is admissible
with $\Phi$ equal to the enstrophy functional.
\end{remark}
...
\begin{proof}
Part~(i):
$\grad_X\delta_C=\Sigma_0^{-1}(X{-}\mu_0)/\delta_C(X)$;
...
gives the bound.
Part~(ii): $(I{-}P_k)$ is an orthogonal projector, so
$\grad_X\norm{(I{-}P_k)X}$ is a unit vector.
Parts~(iii)--(iv) follow from direct gradient computation
using super-critical growth estimates
(Theorem~\ref{thm:phase}) and score-function lower bounds.
\end{proof}
...
symmetric matrices $A:=D^2\Ebs(X_c)$ and $B:=D^2\Phi(X_c)$.
Linearising the gradient bound near $X_c$ gives
$c_1\norm{Bv}\le\norm{Av}\le c_2\norm{Bv}$ for all
$v\in\R^{Nd}$.  The alignment condition (C3),
$\costheta\ge c_1/c_2>0$, linearises to
$\inner{Av}{Bv}\ge(c_1/c_2)\norm{Av}\norm{Bv}$.
Let $w$ be any eigenvector of $B$ with eigenvalue $\mu$.
Then $\mu\inner{w}{Aw}\ge(c_1/c_2)\norm{Aw}\,|\mu|\,\norm{w}$.
If $\mu>0$: $\inner{w}{Aw}>0$; if $\mu<0$: $\inner{w}{Aw}<0$.
Thus the negative eigenspaces of $A$ and $B$ have identical
dimension, i.e.\
$\mathrm{ind}_{\Ebs}(X_c) = \mathrm{ind}_\Phi(X_c)$.

\begin{remark}[Scope of (C1)]\label{rem:c1scope}
Part~(C1) holds at \emph{shared} critical points where
$\grad\Phi=\grad\Ebs=0$ simultaneously - a consequence of
the gradient equivalence~(C3).  In practice, $\Ebs$ and $\Phi$
may have geometrically distinct minima: a single-particle
numerical test finds the LJ equilibrium distance and the
$\Ebs$ centroid are separated by $\approx 0.9$--$1.9$ length
units, with gradient alignment $\costheta>0.99$ far from
equilibrium (the detection regime) and $\costheta<0$ near
minima (the anti-alignment regime of
Proposition~\ref{prop:antialign}).  The practical implication
is that the \emph{alarm equivalence}
($\mathrm{ind}_{\Ebs}\ge 1\Leftrightarrow\mathrm{ind}_\Phi
\ge 1$) is active in the instability region where it matters,
while near equilibrium the damping mechanism~(C5) governs.
This is precisely the dual-mechanism design.
\end{remark}
...
$X \neq \Xst$, and $\sigma(\Xst) = 0$ since
$\grad\Ebs(\Xst) = 0$ by~(C3).
...
\begin{proposition}[Stability under global anti-alignment]
\label{prop:antialign}
Let the empirical gradient cosine alignment satisfy
$\costheta_{\mathrm{global}} < 0$ (the expected anti-alignment
between the position sensor $\grad\Ebs$ and the force field
$-\grad\Phi$ during crisis approach).
The blind-spot-damped dynamics~\eqref{eq:bsdamped} remain
asymptotically stable on $\mathcal{L}_c$ provided
\begin{equation}\label{eq:damping_dom}
  \gamma(\Ebs(X)) \ge
  \frac{|\costheta_{\mathrm{global}}|\cdot\norm{F(X)}}{\norm{\grad\Ebs(X)}}
  =: \gamma_{\mathrm{crit}}(X).
\end{equation}
Since the adaptive law $\gamma(E) = E/(E + \theta)$ satisfies
$\gamma \to 1$ as $E \to \infty$ and $\gamma_{\mathrm{crit}} \to 0$
as $X \to \Xst$ (because $F(\Xst) = 0$), the damping dominance
condition~\eqref{eq:damping_dom} is automatically satisfied in a
neighbourhood of $\Xst$ and remains satisfied above $\Cman$ where
$\gamma$ is large.
\end{proposition}

\begin{proof}
Along the damped dynamics~\eqref{eq:bsdamped},
$\dot{W} = \inner{\grad\Ebs}{F} - \gamma\norm{\grad\Ebs}^2$.
Since $\inner{\grad\Ebs}{F} \le |\costheta_{\mathrm{global}}|
\cdot \norm{\grad\Ebs}\norm{F}$, we have
$\dot{W} \le \norm{\grad\Ebs}(
|\costheta_{\mathrm{global}}|\norm{F}
- \gamma\norm{\grad\Ebs}) \le 0$
whenever~\eqref{eq:damping_dom} holds.
\end{proof}

\begin{remark}\label{rem:antialign_meaning}
The anti-alignment phenomenon ($\costheta = -0.335$ on FDIC data)
is not a limitation but the \textbf{expected complementary geometry}
of the two-component structure of~$\Ebs$.  The blind-spot energy
is a \emph{position sensor}: $\Ebs(X)$ measures how far the
current state deviates from the historical normal.  The system
potential~$\Phi$ is a \emph{force field}: $-\grad\Phi$ drives the
dynamics.  When the system approaches crisis, the force pushes $X$
further from equilibrium, i.e., in the direction of increasing
$\Ebs$.  Consequently $\grad\Ebs \approx -\grad\Phi$, giving
$\costheta < 0$.  Anti-alignment is the signature of a system
moving toward instability --- the regime where detection matters
most.

Near critical points ($\grad\Phi \approx 0$), the shared
$\Phi_{\mathrm{pair}}$ term dominates both gradients and
alignment reverts to positive ($\costheta \ge c_1/c_2 > 0$
on $\mathcal{L}_c$, as proved in~C3).  The negative global
average reflects FDIC data spanning three major crises
(GFC, European sovereign debt, COVID), during which the
system spends significant time in the anti-aligned
crisis-approach regime.

A position sensor and a force sensor are \emph{strictly more
informative together} than either alone: the sign and magnitude
of $\costheta$ itself becomes a diagnostic signal, with
$\costheta \ll 0$ indicating active crisis approach.
The Morse-index alarm (C1) remains exact at shared critical
points regardless of gradient direction; the anti-alignment
occurs between critical points, where the complementary
information of both signals provides the operative guarantee
via norm equivalence~(C3) and damping dominance
(Proposition~\ref{prop:antialign}).
\end{remark}

\subsection{The Dual Mechanism: Geometry and Control}\label{sec:dual}

Theorem~\ref{thm:equiv} and Proposition~\ref{prop:antialign}
together establish a \emph{dual-mechanism} stability theory:

\begin{theorem}[Dual-Mechanism Stability]\label{thm:dual}
Under Assumptions~\ref{ass:reg}--\ref{ass:morse}, the BSDT
framework provides anomaly detection via two complementary mechanisms:

\textbf{D1 (Geometric regime).}
In a neighbourhood $B(x_c,\varepsilon_{x_c})$ of every
critical point $x_c\in\operatorname{Crit}(\Phi)$, the blind-spot
energy $\Ebs$ inherits the exact Morse index, topological stability,
and gradient alignment of $\Phi$
(Theorem~\ref{thm:equiv}\,(C1)--(C4)).

\textbf{D2 (Control regime).}
Globally---including regions where
$\cos\theta_{\textup{global}} < 0$---the norm equivalence
\eqref{eq:norm_equiv} combined with the adaptive friction
$\gamma(\Ebs)$ ensures monotone energy dissipation and bounded
MFLS monitoring (Proposition~\ref{prop:antialign}).
\end{theorem}

\begin{proof}
D1 follows directly from Theorem~\ref{thm:equiv}\,(C1)--(C4).
For D2, the adaptive friction condition
$\gamma(\Ebs)\ge c_\gamma\Ebs^\alpha$ with
$c_\gamma > |\cos\theta|\cdot\|\nabla\Phi\|/\|\nabla\Ebs\|$
ensures $\frac{d}{dt}\Ebs \le -\gamma(\Ebs)\|\nabla\Ebs\|^2 + \cos\theta\,\|\nabla\Ebs\|\,\|\nabla\Phi\| \le 0$.
The MFLS $= \|\nabla\Ebs\|$ remains a valid monitoring signal
because it is bounded below by $c_{-}\|\nabla\Phi\|$ near critical
points (Theorem~\ref{thm:equiv}\,(C3)) and upper-bounded by
$(1+L_{\max})\|\nabla\Phi\|$ globally.
\end{proof}

\begin{remark}
The dual-mechanism structure is a \emph{strength}, not a
concession. Detection near critical manifolds (D1) relies on exact
geometric equivalence---the strongest possible guarantee.
Detection in the transient regime (D2) relies on damping
dominance and norm monitoring---a robust control-theoretic
guarantee. Together, they cover all of state space.
\end{remark}
...
A fixed regulatory capital buffer $\bar{\kappa}$ \cite{basel2010} corresponds to
...
Lennard-Jones 6--12 pair potential \cite{jones1924lj}:
...
\subsection{Universal Granger Failure}\label{sec:granger}
...
\begin{remark}[Simulation verification of Granger failure]
\label{rem:granger_sim}
A Monte Carlo experiment (100 replications) on a synthetic
phase-transition DGP---a slow Ornstein--Uhlenbeck fragility signal
$x$ with independent shock triggers---confirms that linear Granger
\emph{systematically} fails ($78\%$ of replications, median
$p = 0.32$, matching the empirical $p>0.31$), while threshold
Granger (Hansen--Seo test, conditioning on $x > q_{80}$) detects
the causal structure ($p < 0.01$ for the seed-$42$ instance,
$26\%$ rejection rate at $\alpha = 0.10$ across seeds).  The same
paradox emerges from the actual BSDT engine on a synthetic coupled
system: linear Granger $p = 0.104$ (FAIL) but threshold Granger
$p = 0.024$ at $q_{75}$ (PASS).  Code:
\texttt{test\_granger\_nonlinear.py}.
\end{remark}
...
Hourly generation-by-fuel data \cite{ercot2023} for the Texas ERCOT interconnect,
...
To test cross-domain transfer, we apply the framework---without
...
\subsection{Cross-Domain Collapse Structure}

\begin{table}[htbp]
\centering
\caption{Cross-domain collapse structure.}
\label{tab:universal}
\begin{tabular}{@{}lccc@{}}
\toprule
...
The shared structure is not coincidence.  Any system with (i)~interacting
...
\paragraph{Physical versus non-physical energy.}
The three domains occupy different positions on the
physical-energy spectrum.
In Navier--Stokes, the energy $\Phi$ is the physical enstrophy and
the adaptive coupling is a genuine viscosity; the equivalence
$\mathrm{ind}_{\Ebs} = \mathrm{ind}_\Phi$ has direct thermodynamic
content.
In the ERCOT power grid, $\Phi$ aggregates generator dispatch
interactions that map to real energy flows, making the surrogate
interpretation semi-physical.
In banking and crypto, no institution has a literal ``potential
energy''; here $\Phi$ is a surrogate instability landscape whose
critical-point geometry controls collapse dynamics.  The energy
is real \emph{within the BSDT-induced model}, not a metaphor for
balance-sheet stress.  The theorems apply uniformly because all
three domains satisfy the same regularity and non-degeneracy
conditions (Assumptions~S1--S3), not because banking ``is''
fluid dynamics.
...
demonstrates that the mathematical framework is
\emph{geometrically} domain-agnostic: any system satisfying
the regularity and non-degeneracy conditions admits the same
...
Navier--Stokes equations.  Here, unlike banking, $\Phi$ is the
\emph{physical} enstrophy, so the equivalence has direct
thermodynamic content.

\paragraph{Relation to existing turbulence models.}
Conventional adaptive-viscosity approaches modify the dissipation
operator based on \emph{local} flow quantities: the dynamic
Smagorinsky model~\cite{smagorinsky1963,germano1991} uses the
resolved strain-rate tensor $\bar{S}_{ij}$ and a test-filter
identity to determine the sub-grid eddy viscosity;
hyperviscosity replaces $\nu\Delta$ with $\nu(-1)^{p+1}\nabla^{2p}$
to selectively damp high wavenumbers; and spectral vanishing
viscosity~\cite{tadmor1989} activates dissipation only above a
cutoff mode.  In contrast, the BSDT adaptive viscosity
$\nu_{\mathrm{eff}} = \nu(1 + \gamma^*\,E_{\mathrm{BS}})$ is
driven by a \emph{global topological functional}---the
blind-spot energy $E_{\mathrm{BS}}$ defined on Fourier-mode
interactions---and carries a three-part Lyapunov certificate
(Armijo descent, ISS robustness, La~Salle convergence;
Proposition~\ref{prop:ns_lyapv2}).  This structural guarantee
is absent from LES closures, which lack formal stability
proofs for the \emph{a priori} enstrophy bound.
Computationally, prior DNS blow-up studies~\cite{kerr1993,houli2006}
focused on the \emph{inviscid} (Euler) equations and diagnosed
singularity via local vorticity growth.
Our 34-run stress test (Appendix~\ref{app:ns:stress_test})
complements these by studying the \emph{viscous} (Navier--Stokes)
equations with adaptive feedback, providing the first systematic
evidence of feedback-law independence (5~laws, spread $= 0.0004$)
and IC-independence (12~random-phase runs) for the
production/dissipation attractor.
...
viscosity: preliminary characterisation ($N=32$, $T=1.0$,
$\tfrac{2}{3}$-rule dealiasing, Taylor--Green IC).  The ratio
converges to $0.500\pm 0.005$ across two decades of Reynolds
number, motivating the \emph{P/D halving principle}.  Enstrophy
suppression and BKM results at the resolution-converged $N=128$
grid are given in Tables~\ref{tab:ns_convergence}
and~\ref{tab:ns_re_sweep}.}
\label{tab:pd}
\begin{tabular}{@{}rcccc@{}}
\toprule
$\mathrm{Re}$ & $\nu_0$ & $P/D$ (const.\ $\nu$) &
...
control problem.  Crucially, the claims are \emph{model-relative}:
the theorems hold within the BSDT-induced geometry, and
cross-domain transfer is a consequence of shared geometric
structure (Assumptions~S1--S3), not a claim that every domain
obeys the same physical law.  The framework yields sixteen specific findings:
...
\item The Terra/Luna collapse ($r=0.996$) and ERCOT grid failure
  ($+72$\,h lead) validate cross-domain structural transfer.

\item The framework transfers to Navier--Stokes.  A 34-run
  GPU stress test on an NVIDIA RTX PRO 6000 Blackwell
  (102\,GB VRAM, 532\,min wall time) validates the
  computational theory across seven experiment blocks:
  \emph{(i)}~High-Re sweep: at $\mathrm{Re}\approx 62{,}832$
  on a $256^3$ grid, peak vorticity reaches
  $\|\bm{\omega}\|_\infty = 132.2$ (constant~$\nu$) and
  $105.3$ (adaptive), with BKM integrals $465.4$ and $406.1$
  respectively---both finite.  Adaptive viscosity reduces
  peak vorticity by $1.77\times$ at $\mathrm{Re}\approx 1257$.
  \emph{(ii)}~Resolution check: $N=128$ vs.\ $N=256$ at
  $\mathrm{Re}\approx 6283$ shows $11.3\%$ peak~$\omega$
  difference, with identical qualitative dynamics (both bounded).
  \emph{(iii)}~IC independence: 12 random-phase IC runs
  (3~seeds $\times$ 2~modes $\times$ 2~Re) all produce
  bounded vorticity ($\|\omega\|_\infty \in [9.5, 39.3]$).
  \emph{(iv)}~P/D cross-correlation:
  $\mathrm{Corr}(P,D) \ge 0.86$ at all tested Re, confirming
  tight production-dissipation coupling.
  \emph{(v)}~Feedback universality: five feedback laws
  (standard, tanh, exponential, linear, sqrt) produce
  $P/D$ values identical within spread $= 0.0004$---the
  attractor is structural, not an artefact of the specific
  $\gamma^*$ form.
  \emph{(vi)}~At $\mathrm{Re}\approx 62{,}832$, the turbulent
  regime shows bounded vorticity at $N=256$.
  \emph{(vii)}~Vortex stretching diagnostics at two Re values
  confirm bounded alignment and finite $P/D$ ratios.
  A 15-point Reynolds sweep
  ($\mathrm{Re}=314$--$62{,}832$, $N=128$) reveals the
  non-monotone suppression profile
  $S(\mathrm{Re})\approx 1.66\cdot\mathrm{Re}^{-0.047}$
  ($p=0.0016$, $R^2=0.55$).
  The Lyapunov~v2 three-part certificate
...
\item All five BSDT scoring variants---Full-BSDT, QuadSurf,
  ...
  within-normal variability---yet $\delta_G$ is uninformative for
...
\begin{enumerate}
\item \textbf{Pseudo-spectral resolution.} The resolution
  convergence study establishes grid-independent results at
  $N=128$ (suppression factor converges to within $<1\%$ from
  $N=64$ to $N=128$).  At $\mathrm{Re}=1257$ the Kolmogorov length
  scale requires $N\gtrsim 235$ for full DNS resolution; the
  present $N=128$ grid is therefore a well-resolved
  under-resolved simulation rather than a fully turbulent DNS.
  The stress test extends to $N=256$ at $\mathrm{Re}\approx
  6283$ and $62{,}832$ (Appendix~\ref{app:ns:stress_test}),
  where $N=128$ and $N=256$ agree on peak vorticity within
  $11.3\%$---identical qualitative dynamics with both bounded.
  Full DNS at $N \ge 512$ for $\mathrm{Re} \ge 10^4$
  would confirm the scaling trend in the fully turbulent regime.
...
\item \textbf{Gradient anti-alignment as complementary geometry.}
  The empirical anti-alignment ($\costheta=-0.335$) on real data
  is not a deficiency but the \emph{expected} consequence of
  $\Ebs$'s two-component structure.  $\Ebs$ is a position sensor
  (how far the system deviates from normal); $-\grad\Phi$ is the
  force pushing the system.  During crisis approach, the force drives
  the system further from equilibrium, so $\grad\Ebs \approx -\grad\Phi$
  and $\costheta < 0$.  Near critical points ($\grad\Phi \approx 0$),
  the shared $\Phi_{\mathrm{pair}}$ term dominates and alignment
  reverts to positive, as proved in~(C3).  The anti-alignment
  is therefore a diagnostic signal of crisis approach in its own
  right.  Stability is maintained by damping dominance
  (Proposition~\ref{prop:antialign}), and the combination of
  position and force sensors is strictly more informative than
  either alone.
...
\item \textbf{Empirical validation of structural claims.}
  For the simulation engines (Molecular, Gravity), $\Phi$ is
  exactly known by construction.  This enables direct measurement
  of: (i)~$\|D^2\Ebs(X_c) - D^2\Phi(X_c)\|$ at numerically
  identified critical points; (ii)~gradient cosine alignment
  $\costheta$ restricted to neighbourhoods of critical points
  (where the theory predicts $\costheta > 0$); (iii)~explicit
  Morse-index comparison $\ind_{\Ebs}(X_c)$ vs.\
  $\ind_\Phi(X_c)$ at shared critical points.  These
  measurements would close the gap between the
  theoretical guarantees (local equivalence) and the empirical
  evidence (global AUC performance).
...
\item Validate the topological singularity diagnostic at
  $N \ge 512$ resolution and $\mathrm{Re} \ge 10^5$.  The
  stress test (Appendix~\ref{app:ns:stress_test}) establishes
  bounded vorticity at $N=256$, $\mathrm{Re}\approx 62{,}832$
  (peak $\|\bm{\omega}\|_\infty = 132.2$, BKM $= 465.4$),
  and IC-independence via 12~random-phase runs.
  The fully turbulent regime at higher Re and the
  Betti-number dynamics of resolved vortex tubes remain open.
\end{enumerate}
...
For the \emph{Navier--Stokes community}: the 34-run stress test
(Appendix~\ref{app:ns:stress_test}) demonstrates that modern
financial mathematics---online learning, adaptive control,
Lyapunov analysis---provides tools for classical PDE regularity
questions.  The key results---bounded BKM integral at
$\mathrm{Re}\approx 62{,}832$, IC-independent boundedness
(12 random-phase runs), feedback universality
(spread $= 0.0004$ across 5 laws), and tight
production-dissipation coupling ($\mathrm{Corr} \ge 0.85$
at all Re)---provide concrete targets for analytical proof.
To our knowledge, three findings are novel:
\emph{(i)}~the cross-domain transfer of a single energy functional
($E_{\mathrm{BS}}$) from financial networks to
Navier--Stokes---prior DNS studies~\cite{kerr1993,houli2006}
and turbulence models~\cite{smagorinsky1963,germano1991}
operate exclusively within the fluid-dynamics domain;
\emph{(ii)}~feedback universality, i.e., the insensitivity of
the $P/D$ attractor to the specific form of the adaptive law
(five laws, spread $= 0.0004$), which we have not found
reported in the LES or hyperviscosity literature;
and \emph{(iii)}~the Lyapunov~v2 three-part certificate
(Proposition~\ref{prop:ns_lyapv2}), which provides
Armijo--ISS--La~Salle convergence guarantees that standard
LES closures lack.
...
\[
  ...
  = \norm{\grad_X\delta_j}
    \cos\angle(\grad_X\delta_j,\,\grad\Phi)
\]
...
\[
  \inner{-F_{\mathrm{pair}}}{v}
  = \norm{\grad\Phi} - \alpha\inner{X{-}\mu}{v}.
\]
Near~$\Xst$,
...
\[
  \max_t\norm{\grad_X\delta_C} = 3.21,\qquad
  \max_t\norm{\grad_X\delta_G} = 0.98,
\]
\[
  \max_t\norm{\grad_X\delta_A} = 1.47,\qquad
...
on $[0,2\pi]^3$ with $\tfrac{2}{3}$-rule dealiasing.  Time
integration is semi-implicit: the viscous term is handled via an
exact integrating factor; the nonlinear term is treated explicitly.
The adaptive viscosity
$\nu_{\mathrm{eff}}=\nu_0(1+\gamma^*(\Ebs^{\mathrm{NS}}))$
is recomputed at each diagnostic step.  Each time step costs
$O(N^3\log N)$ for the three-dimensional FFTs.

\textit{Hardware}: NVIDIA RTX PRO 6000 Blackwell Server Edition
(102\,GB VRAM), CuPy~14.0.1,
CUDA~12.  IC: Taylor--Green vortex, $\nu_0=0.005$.
The full stress test (34~runs across 7~experiment blocks,
532\,min wall time) is documented in
Appendix~\ref{app:ns:stress_test}.

\paragraph{Study~1: Resolution convergence
  ($\mathrm{Re}\approx 1257$, $T=5$).}

\begin{table}[htbp]
\centering
\caption{Resolution convergence at fixed $\mathrm{Re}\approx 1257$
  ($\nu_0=0.005$, Taylor--Green IC, $T=5$).
  $S = \Omega_{\text{const}}/\Omega_{\text{adapt}}$.
  BKM~$\Delta\%$: percentage reduction in the
  Beale--Kato--Majda integral under adaptive~$\nu$.}
\label{tab:ns_convergence}
\begin{tabular}{@{}rccccc@{}}
\toprule
$N$ & $\Omega_{\text{const}}$ & $\Omega_{\text{adapt}}$
    & $S$ & BKM~$\Delta\%$ & Smooth\\
\midrule
 64 & 1.1547 & 0.6566 & $1.759\times$ & $+29.80\%$ & $\checkmark$\\
 96 & 1.1596 & 0.6536 & $1.774\times$ & $+28.70\%$ & $\checkmark$\\
128 & 1.1543 & 0.6530 & $1.768\times$ & $+28.48\%$ & $\checkmark$\\
\bottomrule
\end{tabular}
\end{table}

The suppression factor varies by less than $1\%$ from $N=64$ to
$N=128$, confirming that the enstrophy reduction is a physical
result and not a numerical artefact of the grid.

\paragraph{Study~2: Reynolds number sweep
  ($N=128$, 15~points, $T=3$).}

\begin{table}[htbp]
\centering
\caption{Dense Reynolds-number sweep at the converged resolution
  $N=128$ ($\nu_0=1/(\mathrm{Re}\cdot N_{\text{ref}})$,
  Taylor--Green IC, $T=3$).  All 15~runs (30~solver invocations)
  complete without blow-up; $\gamma^*_{\max}=0.9959$ at every~Re.
  BKM~$\Delta\%$: percentage reduction in the BKM integral.
  Power-law fit over the descending branch
  ($\mathrm{Re}\ge 628$):
  $S(\mathrm{Re})\approx 1.66\cdot\mathrm{Re}^{-0.047}$
  ($\alpha=-0.047\pm 0.012$, $R^2=0.55$, $p=0.0016$).}
\label{tab:ns_re_sweep}
\begin{tabular}{@{}rcc@{}}
\toprule
$\mathrm{Re}$ & $S$ & BKM~$\Delta\%$\\
\midrule
   314 & $1.045\times$ & $+13.40\%$\\
   419 & $1.235\times$ & $+12.28\%$\\
   628 & $\mathbf{1.427\times}$ & $+10.18\%$\\
   898 & $1.324\times$ & $+8.24\%$\\
...
Three features of Table~\ref{tab:ns_re_sweep} are notable.
\textit{(i)}~The suppression factor is non-monotone: it rises from
$S=1.045$ at $\mathrm{Re}=314$ (laminar, limited enstrophy
build-up) to a transitional peak of $S_{\max}=1.427$ at
$\mathrm{Re}\approx 628$, where constant-$\nu$ enstrophy
accumulation is fastest relative to the adaptive-$\nu$ run.
\textit{(ii)}~For $\mathrm{Re}>628$ the suppression decays along
the power law $S\approx 1.66\cdot\mathrm{Re}^{-0.047}$,
statistically significant at the $1\%$ level ($p=0.0016$).
\textit{(iii)}~$S>1$ throughout ($S=1.008$ even at
$\mathrm{Re}=62832$), and the BKM integral is strictly reduced at
every Re, providing computational evidence that adaptive
$\nu(E_\mathrm{BS})$ suppresses the precursors to finite-time
blow-up across the full laminar-to-turbulent range.

\section{Data Sources}\label{app:data}

All banking features are drawn from the FDIC SDI quarterly call
reports \cite{fdic2024sdi} via the public REST API at
\texttt{https://banks.data.fdic.gov/api/financials}. Feature $f_5$
(yield-curve slope) is the FRED T10Y2Y series \cite{fred2024}. The normal reference
...
\input{appendix_computational_ns}
...
\emph{A dynamic subgrid-scale eddy viscosity model},
Phys.\ Fluids A, 3 (1991), pp.~1760--1765.

\bibitem{houli2006}
T.~Y.~Hou and R.~Li,
\emph{Dynamic depletion of vortex stretching and non-blowup of
the 3-D incompressible Euler equations},
J.\ Nonlinear Sci., 16 (2006), pp.~639--664.

\bibitem{kerr1993}
R.~M.~Kerr,
\emph{Evidence for a singularity of the three-dimensional,
incompressible Euler equations},
Phys.\ Fluids A, 5 (1993), pp.~1725--1746.

\bibitem{smagorinsky1963}
J.~Smagorinsky,
\emph{General circulation experiments with the primitive equations.
I.~The basic experiment},
Mon.\ Wea.\ Rev., 91 (1963), pp.~99--164.

\bibitem{tadmor1989}
E.~Tadmor,
\emph{Convergence of spectral methods for nonlinear conservation
laws},
SIAM J.\ Numer.\ Anal., 26 (1989), pp.~30--44.
...
O.~Odeyemi,
\emph{Blind-Spot Decomposition Theory: Foundations of
Energy-Topology Equivalence for Systemic Anomaly Detection},
Zenodo, 2025.
\textsc{doi}: \href{https://doi.org/10.5281/zenodo.18870589}{10.5281/zenodo.18870589}.
