...
def compute_energy_and_friction(state: pd.DataFrame, mfls: pd.DataFrame, cfg: Config) -> Tuple[np.ndarray, np.ndarray, float]:
...
def main():
    ...
    output_df = state.assign(
        ...
        C3=mfls["C3"],
        C4=mfls["C4"],
        MFLS=mfls["MFLS"],               # windowed peak MFLS(t) = max_{[t-L,t]} ||g_t'||_2
        E=E,
        gamma=gamma,
        theta=theta,
        regime=regimes,
