...
Key quantities computed per time step t:
    ...
    gamma_star(t)   - canonical guardian  lambda / (lambda + alpha)  ∈ [0,1)
...
def pairwise_energy(X: np.ndarray, gamma: float = GAMMA,
                    ...
                    eps: float = EPS) -> float:
    """Φ_pair = Σ_{i<j} [−γσ erf(r/σ) + λ ln(r + ε)]

    Sign convention (matches collapse_geometry/potential.py and math ref §III):
      erf term: −γσ erf(r/σ) — coefficient < 0, energy decreases with r  →  repulsive force
      log term: +λ ln(r)      — coefficient > 0, energy increases with r  →  attractive force

    Equilibrium: (2γ/√π) e^{−r₀²/σ²} = λ/r₀
    """
    N = X.shape[0]
    diff = X[:, None, :] - X[None, :, :]   # (N, N, d)
    dist = np.linalg.norm(diff, axis=2)    # (N, N)
    np.fill_diagonal(dist, eps)
    # Off-diagonal pairs only (sum over all i≠j, then ÷2 for double-counting)
    mask = ~np.eye(N, dtype=bool)
    d_off = dist[mask] + eps
    E_pair = 0.5 * float(
        -gamma * sigma * _erf(d_off / sigma).sum()
        + lam * np.log(d_off).sum()
    )
    return E_pair
...
def pairwise_force(X: np.ndarray, gamma: float = GAMMA, sigma: float = SIGMA,
    ...
    From Φ_pair = −γσ erf(r/σ) + λ ln r  (matches collapse_geometry/forces.py §IV):

        scalar force magnitude (on unit vector from j to i):
            m_ij = (2γ/√π) e^{−r²/σ²} − λ/r

        positive m_ij → repulsive (erf term dominates at short/mid range)
        negative m_ij → attractive (log term dominates; zero net force at equilibrium)

    Laplacian form: K_ij = m_ij / r_{ij},  F_i = Σ_j K_ij (x_i − x_j)
    """
    N, d = X.shape
    diff = X[:, None, :] - X[None, :, :]   # (N, N, d)  diff[i,j] = x_i − x_j
    dist = np.linalg.norm(diff, axis=2)    # (N, N)
    np.fill_diagonal(dist, eps)

    # K_ij = [(2γ/√π) e^{−r²/σ²} − λ/r] / r   (= m_ij / r)
    erf_kern = (2.0 * gamma / (np.sqrt(np.pi) * sigma)) * np.exp(-(dist ** 2) / (sigma ** 2))
    log_kern  = lam / (dist + eps)
    K = (erf_kern - log_kern) / (dist + eps)   # (N, N)
    np.fill_diagonal(K, 0.0)

    # Graph Laplacian force: F_i = Σ_j K_ij (x_i − x_j)
    F = np.einsum('ij,ijd->id', K, diff)   # (N, d)

    # Radial spring contribution handled separately in total_force()
    # Force clamp
