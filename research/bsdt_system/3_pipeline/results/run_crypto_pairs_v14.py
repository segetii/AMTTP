"""
Crypto BSDT v14 — Production: Sharpe × Stability + γ leverage dial
===================================================================

v13 lesson (negative result, +info):
  Regime-splitting rolling Sharpe gave −0.363 — sample-split halved
  effective N per estimate, destroyed signal quality. Confirmed:
  strategies in this period are NOT meaningfully regime-dependent.
  v10's full-sample rolling Sharpe (+0.597) remains the alpha source.

v14 — production layer (two orthogonal upgrades on top of v10):

  (1) Sharpe × Stability weighting:
        w_raw_t  = clip(rolling_Sharpe_t, 0, ∞)            # v10
        stab_t   = 1 / (1 + rolling_std(rolling_Sharpe_t))  # NEW
        w_t      = normalize(w_raw_t × stab_t)
      Penalises strategies whose Sharpe estimate itself is unstable.
      Should reduce drawdowns without sacrificing Sharpe.

  (2) γ_rank as leverage dial (NOT as selector):
        lev_mult_t = clip(1.5 − γ_rank_t, 0.5, 1.2)
      γ_rank > 0.8 → lev_mult = 0.7  (defensive in stress)
      γ_rank < 0.2 → lev_mult = 1.2  (full size in calm)
      Applied as a global gross-exposure scalar to the v14 portfolio.
      Strategy SELECTION mechanics unchanged from v10 (Sharpe weights).

Falsifiable comparisons:
  v14_stab        = Sharpe×Stability, no leverage dial   → isolates (1)
  v14_lev         = Sharpe only, with leverage dial      → isolates (2)
  v14_full        = both                                 → production

Reports v10 / v12 / v13 / v14 (3 variants) side-by-side.
"""
...
STAB_EPS          = 1e-6
LEV_BASE          = 1.5         # lev_mult = clip(LEV_BASE - γ_rank, LEV_LO, LEV_HI)
LEV_LO            = 0.5
LEV_HI            = 1.2
...
def compute_leverage_dial(gamma_rank, base=1.5, lo=0.5, hi=1.2):
    """
    v14: γ_rank → global gross-exposure scalar.
      lev_mult = clip(base − γ_rank, lo, hi)
      γ_rank=0.0 → 1.2 (full size in calm)
      γ_rank=0.5 → 1.0 (neutral)
      γ_rank=1.0 → 0.5 (defensive in stress)
    """
...
def main():
    ...
    W = 130
    print("\n" + "─" * W)
    print("── v10 / v12 / v13 / v14 (a/b/full): dynamic portfolio comparison [test] ──")
    print("─" * W)
    print(f"  {'Strategy':<22}  {'v10 Sh':>8}  {'v12 Sh':>8}  {'v13 Sh':>8}  "
          f"{'v14a Sh':>8}  {'v14b Sh':>8}  {'v14 Sh':>8}  {'v10→v14':>8}  {'MaxDD_v14':>10}")
    print("─" * W)
    ...
    for name in indiv_labels + ['DYNAMIC_COMBINED']:
        ...
        if name == 'DYNAMIC_COMBINED':
            ...
            v14as = sims_v14a_comb.get('sharpe')
            v14bs = sims_v14b_comb.get('sharpe')
            v14s  = sims_v14_comb.get('sharpe')
            dd    = sims_v14_comb.get('max_dd')
        else:
            v10s  = v9s
            v12s  = v9s
            v13s  = v9s
        ...
        def _f(x, w=8):
            return f"{x:>+{w}.3f}" if x is not None else " " * w
        dlt = (f"{v14s - v10s:>+8.3f}"
               if (v10s is not None and v14s is not None) else "        ")
        dds = f"{dd:>10.3f}" if dd is not None else "          "
        label = name if name != 'DYNAMIC_COMBINED' else '─' * 4 + ' DYNAMIC_COMBINED'
        print(f"  {label:<22}  {_f(v10s)}  {_f(v12s)}  {_f(v13s)}  "
              f"{_f(v14as)}  {_f(v14bs)}  {_f(v14s)}  {dlt}  {dds}")
    ...
    results = {
        "model":   "Crypto BSDT v14 — Production: Sharpe×Stability + γ leverage dial",
        "version": "v14",
        "gamma_stats": {
    ...
    import os
    out_path = os.path.join(OUT_DIR, "crypto_bsdt_v14_results.json")
    ...
    # ── Summary ───────────────────────────────────────────────────────────
    print("\n" + "=" * 92)
    print("  SUMMARY: v9 → v10 → v11 → v12 → v13 → v14")
    print("=" * 92)
    print(f"  {'Version':<24}  {'Source':<48}  {'DYNAMIC Sharpe':>14}")
    print("─" * 92)
    summary_rows = [
        ...
        ('v14b (γ leverage)',    'v10 weights × γ-leverage dial',                       sims_v14b_comb['sharpe']),
        ('v14 (PROD)',           'Sharpe×Stability × γ-leverage dial',                   sims_v14_comb['sharpe']),
    ]
