...
BAR      = "=" * 120
OUT_DIR_ = Path(OUT_DIR) / "v43_regime_cb"
...
ETA_GRID          = [2e-4]       # single: proven best
LOCK_MIN_GRID     = [500]        # single: proven best
KAPPA_MAX_GRID    = [None]       # single: no G7 regularisation
OMEGA_THRESH_GRID = [0.02]      # single: proven
RHO_ALPHA_GRID    = [0.5]        # single: proven

# ── v43 regime-switching CB grids ───────────────────────────────────────────────
# Hypothesis: bull regime → loose CB (captures rallies); bear regime → tight CB
# Regime detection: btc_price vs rolling MA (200h ≈ 8d, 500h ≈ 20d)
CB_HALT_BULL_GRID   = [0.12, 0.15]   # high threshold in bull: don’t interrupt rallies
CB_HALT_BEAR_GRID   = [0.05, 0.06, 0.08]  # tight threshold in bear: limit each step
MA_LEN_GRID         = [200, 500]      # hours (8d or 20d MA)
# cb_resume = cb_halt / 2 for each regime
CB_WINDOW_DAYS      = 90             # fixed rolling-peak window (same for both regimes)

# Baseline comparison (same fixed CB as v40/v41 for reference)
CB_HALT_FIXED   = 0.08
CB_RESUME_FIXED = 0.04
...
# ── v43: Regime-switching CB simulation ──────────────────────────────────────

def simulate_regime_cb_v43(
    unit_normal:    pd.Series,
    unit_crash:     pd.Series,
    K_normal:       float,
    K_crash:        float,
    ...
    y_floor:        float,
    cb_window_days: int,
    btc_arr:        np.ndarray,   # BTC close price per bar (aligned to unit_normal)
    btc_ma_arr:     np.ndarray,   # pre-computed rolling MA of BTC
    cb_halt_bull:   float,        # halt threshold in bull regime (btc > ma)
    cb_resume_bull: float,        # resume threshold in bull regime
    cb_halt_bear:   float,        # halt threshold in bear regime (btc <= ma)
    cb_resume_bear: float,        # resume threshold in bear regime
    eta:            float,
    rank_Ix:        int,
    rho_typ:        float,
    lock_min:       int,
    ...
    """v43: CB halt/resume thresholds switch with BTC trend regime.

    Bull regime (btc > btc_ma): use cb_halt_bull / cb_resume_bull
    Bear regime (btc <= btc_ma): use cb_halt_bear / cb_resume_bear
    Window fixed = cb_window_days (same for both regimes).
    All other mechanics identical to v41.
    """
    window_bars    = cb_window_days * HOURS_PER_DAY
    mono_dq: deque = deque()
    eq              = INIT
    ...
    for bar_i in range(N):
        ...
        # Resume evaluation
            ...
            lock_bars = bar_i - halt_start_bar
            is_standard = cb_dd <= cb_resume_eff
            is_g24 = (
                not is_standard
                and eta > 0.0
                and lock_bars >= lock_min
                and cb_dd <= cb_resume_eff + c_sigma_saturating(eta, rank_Ix, rho_typ, lock_bars)
            )
            candidate = is_standard or is_g24
    ...
    return dict(
        ...
        final           = float(eqs.iloc[-1]),
        maxdd           = float(dd_s.min()),
        cagr            = float(eqs.iloc[-1] / INIT) ** (1.0 / max(years, 1e-9)) - 1.0,
        n_trips         = n_trips,
...
def main() -> None:
    ...
    n_total = len(TOP3) * len(MA_LEN_GRID) * len(CB_HALT_BULL_GRID) * len(CB_HALT_BEAR_GRID) + len(TOP3)
    print(BAR)
    print("  CRYPTO GODMODE v43 — Regime-switching CB sweep")
    print(f"  CB_HALT_BULL grid: {CB_HALT_BULL_GRID}")
    print(f"  CB_HALT_BEAR grid: {CB_HALT_BEAR_GRID}")
    print(f"  MA_LEN grid:       {MA_LEN_GRID}  (hours)")
    print(f"  CB_WINDOW:         {CB_WINDOW_DAYS}d (fixed)")
    print(f"  cb_resume = cb_halt / 2 per regime")
    print(f"  Fixed engine:    eta={ETA_GRID[0]:.0e}  lm={LOCK_MIN_GRID[0]}  om={OMEGA_THRESH_GRID[0]}  ra={RHO_ALPHA_GRID[0]}")
    ...
    print(f"\n[4] Running v43 regime-switching CB sweep ...")
    results:      list[dict]      = []
    sim_store_eq: dict[str, dict] = {}

    print(f"  {'label':<80}  {'Calmar':>7}  {'Final':>12}  {'MaxDD':>7}  "
          f"{'r2026':>7}  {'pct_h':>6}  {'n_trp':>5}")
    ...
    for d, q in TOP3:
        for kappa_max in KAPPA_MAX_GRID:
            ...
            eta       = ETA_GRID[0]
            lock_min  = LOCK_MIN_GRID[0]
            omega_thresh = OMEGA_THRESH_GRID[0]
            rho_alpha = RHO_ALPHA_GRID[0]
            km_str    = str(kappa_max) if kappa_max else "none"

            # Extract BTC close price aligned to unit index
            btc_close = df["btc"].reindex(idx).ffill().values

            # ── Baseline: fixed CB (v40 reference) ────────────────────────────
            label_base = (f"d{str(d).replace('.','p')}_q{str(q).replace('.','p')}"
                          f"_FIXED_halt8_win90_km{km_str}")
            sim_base = simulate_combined_lockmin_v41(
                ...
                y_floor=DYN_Y_FLOOR,
                cb_halt=CB_HALT_FIXED, cb_resume=CB_RESUME_FIXED,
            ...
            row_base = result_row_v41(
                label_base, sim_base, test_ts, d, q, eta, lock_min, kappa_max,
                omega_thresh, rho_alpha, rank_Ix, lam_g7, rho_typ_dq, sdiag, bdiag,
            )
            row_base.update(cb_halt=CB_HALT_FIXED, cb_resume=CB_RESUME_FIXED,
                            cb_window=CB_WINDOW_DAYS, regime="fixed",
                            cb_halt_bull=CB_HALT_FIXED, cb_halt_bear=CB_HALT_FIXED, ma_len=0)
            results.append(row_base)
            print(f"  {label_base:<80}  {row_base['calmar']:>7.2f}  "
                  f"${row_base['final']:>11,.0f}  {row_base['maxdd']:>7.1%}  "
                  f"{row_base['r2026']:>+7.3f}  {sim_base.get('pct_halted',0):>6.1%}  "
                  f"{sim_base.get('n_trips',0):>5}  [BASELINE]")

            # ── Regime-switching sweep ─────────────────────────────────────────
            for ma_len in MA_LEN_GRID:
                # Pre-compute rolling MA of BTC
                btc_series = pd.Series(btc_close, index=idx)
                btc_ma     = btc_series.rolling(ma_len, min_periods=1).mean().values

                for halt_bull in CB_HALT_BULL_GRID:
                    resume_bull = round(halt_bull / 2.0, 4)
                    for halt_bear in CB_HALT_BEAR_GRID:
                        resume_bear = round(halt_bear / 2.0, 4)
                        label = (f"d{str(d).replace('.','p')}_q{str(q).replace('.','p')}"
                                 f"_REGIME_ma{ma_len}"
                                 f"_bull{int(halt_bull*100)}_bear{int(halt_bear*100)}"
                                 f"_km{km_str}")

                        sim = simulate_regime_cb_v43(
                            unit_normal=unit,
                            unit_crash=pd.Series(0.0, index=unit.index),
                            K_normal=K_NORMAL, K_crash=0.0,
                            dd_soft=DYN_DD_SOFT, dd_stop=DYN_DD_STOP,
                            y_floor=DYN_Y_FLOOR,
                            cb_window_days=CB_WINDOW_DAYS,
                            btc_arr=btc_close, btc_ma_arr=btc_ma,
                            cb_halt_bull=halt_bull, cb_resume_bull=resume_bull,
                            cb_halt_bear=halt_bear, cb_resume_bear=resume_bear,
                            eta=eta, rank_Ix=rank_Ix, rho_typ=rho_typ_dq,
                            lock_min=lock_min,
                        ...
                        row.update(cb_halt=halt_bull, cb_resume=resume_bull,
                                   cb_window=CB_WINDOW_DAYS, regime="switching",
                                   cb_halt_bull=halt_bull, cb_halt_bear=halt_bear,
                                   ma_len=ma_len)
                        results.append(row)

                        print(f"  {label:<80}  {row['calmar']:>7.2f}  "
    ...
    out_json = OUT_DIR_ / "crypto_godmode_v43_regime_cb.json"
    rows_clean = [
