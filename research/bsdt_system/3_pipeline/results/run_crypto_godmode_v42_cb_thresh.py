# -*- coding: utf-8 -*-
"""
run_crypto_godmode_v42_cb_thresh.py
=====================================

v42 = v40 + CB Threshold Sweep

Diagnostic finding (v41 run + diag_v41_plan.py)
-------------------------------------------------
MaxDD = -34.6%. Root cause (confirmed): 14-month staircase decline caused by
the strategy spending 86.5% of bars HALTED, missing BTC bull runs while
catching each 8% equity decline via CB trips.  The ramp fix (v41) provided
no improvement — ramp had expired before the MaxDD trough.

Fix (v42): CB Threshold Sweep
-------------------------------
Test whether wider CB halt thresholds (10–15%) reduce staircase frequency,
allowing MORE time active during bull runs (captures upside) at the cost of
larger per-trip losses.  Also test shorter CB windows (45d, 60d) to reset
the peak reference more frequently.

  Hypothesis: at cb_halt=0.12, the strategy halts less often (~40% vs 86.5%),
  captures more of the BTC bull runs (2× price gains), and the MaxDD improves
  from −34.6% to ~−20% even though each individual trip loss is larger.

Grid additions vs v41
----------------------
  CB_HALT_GRID   : [0.05, 0.08, 0.10, 0.12, 0.15]
  CB_RESUME_FRAC : [0.5]  (resume = halt × resume_frac)
  CB_WINDOW_GRID : [45, 60, 90]  (days)
...
BAR      = "=" * 120
OUT_DIR_ = Path(OUT_DIR) / "v42_cb_thresh"
...
# ── CB config (v42: swept) — defaults kept for any non-swept code ─────────────
CB_HALT   = 0.08
CB_RESUME = 0.04
CB_WINDOW = 90   # days

# ── v39 ODE/gate grids (focused, same as v40/v41) ────────────────────────────
ETA_GRID          = [0.0, 2e-4, 3e-4]
LOCK_MIN_GRID     = [500, 1000]
KAPPA_MAX_GRID    = [None, 10, 5]
OMEGA_THRESH_GRID = [None, 0.02]
RHO_ALPHA_GRID    = [0.0, 0.5, 0.6]

# ── v42 CB threshold grid ─────────────────────────────────────────────────────
CB_HALT_GRID        = [0.05, 0.08, 0.10, 0.12, 0.15]   # halt threshold
CB_RESUME_FRAC_GRID = [0.5]                              # resume = halt × frac
CB_WINDOW_GRID      = [45, 60, 90]                       # window (days)
...
# ── v42 CB simulation with parametric CB thresholds ─────────────────────────

def simulate_combined_lockmin_v42(
...
def result_row_v42(
    ...
    cb_halt: float = 0.08, cb_resume: float = 0.04, cb_window: int = 90,
) -> dict:
...
def main() -> None:
    ...
    # Variant count
    n_cb_combo  = len(CB_HALT_GRID) * len(CB_RESUME_FRAC_GRID) * len(CB_WINDOW_GRID)
    n_inner = 1 + len([e for e in ETA_GRID if e > 0]) * len(LOCK_MIN_GRID) * \
              len(OMEGA_THRESH_GRID) * len(RHO_ALPHA_GRID) * n_cb_combo
    n_total = len(TOP3) * len(KAPPA_MAX_GRID) * n_inner

    print(BAR)
    print("  CRYPTO GODMODE v42 — CB Threshold Sweep")
    print(f"  CB_HALT  grid:    {CB_HALT_GRID}")
    print(f"  CB_RESUME_FRAC:   {CB_RESUME_FRAC_GRID}")
    print(f"  CB_WINDOW grid:   {CB_WINDOW_GRID}")
    ...
    print(f"\n[4] Running CB simulation sweep (v42: CB thresholds + v39 gates + G7) ...")
    results:      list[dict]      = []
                ...
                for lock_min in LOCK_MIN_GRID:
                    ...
                    for omega_thresh in omega_loop:
                        for rho_alpha in rho_loop:
                            ...
                            for cb_halt_v in CB_HALT_GRID:
                                for cb_resume_frac in CB_RESUME_FRAC_GRID:
                                    cb_resume_v = round(cb_halt_v * cb_resume_frac, 4)
                                    for cb_window_v in CB_WINDOW_GRID:
                                        ch_str = f"ch{str(cb_halt_v).replace('.','p')}"
                                        cw_str = f"cw{cb_window_v}"
    ...
    # ── [5] Summary ────────────────────────────────────────────────────────────
    print(f"\n{BAR}")
    print("  v42 RESULTS SUMMARY")
    print(f"  Total variants: {len(results)}")

    # ── CB threshold sweep summary ──
    print("\n  CB threshold sweep (d=0.25,q=0.50,km=none,eta>0,om=0.02,ra=0.5) best Calmar per (halt,window):")
    print(f"  {'halt':>6}  {'resume':>7}  {'win':>4}  {'Calmar':>7}  {'Final':>12}  {'MaxDD':>7}  {'pct_halt':>9}  {'r2026':>7}")
    cb_sub = [r for r in results
              if r["d"]==0.25 and r["q"]==0.50 and r.get("kappa_max") is None
              and r["eta"]>0 and r.get("omega_thresh")==0.02
              and abs(r.get("rho_alpha",0)-0.5)<1e-9]
    from itertools import groupby as _gb
    cb_sub.sort(key=lambda r: (r.get("cb_halt",0.08), r.get("cb_window",90)))
    for key, grp in _gb(cb_sub, key=lambda r: (r.get("cb_halt",0.08), r.get("cb_window",90))):
        g = list(grp)
        b = max(g, key=lambda r: r.get("calmar",0))
        print(f"  {key[0]:>6.2%}  {b.get('cb_resume',0):>7.2%}  {key[1]:>4}  "
              f"{b['calmar']:>7.2f}  ${b['final']:>11,.0f}  {b['maxdd']:>7.1%}  "
              f"{b.get('pct_halted',0):>9.1%}  {b.get('r2026',0):>+7.3f}")

    # ── Best MaxDD per cb_halt ──
    print("\n  Best MaxDD per cb_halt (any params):")
    all_halt = sorted(set(r.get('cb_halt',0.08) for r in results))
    for halt in all_halt:
        sub = [r for r in results if abs(r.get('cb_halt',0.08)-halt)<1e-9]
        b = max(sub, key=lambda r: r.get('maxdd',-99))
        print(f"  halt={halt:.2%}: MaxDD={b['maxdd']:.1%}  Calmar={b['calmar']:.2f}  "
              f"Final=${b['final']:,.0f}  pct_halt={b.get('pct_halted',0):.1%}  name={b['name'][:80]}")

    # ── Baseline comparison (η=0) ──
    print("\n  η=0 baseline MaxDD per (d,q,κ_max, cb_halt):")
    for d, q in TOP3:
        for kappa_max in KAPPA_MAX_GRID:
            km_str   = str(kappa_max) if kappa_max else "none"
            for cb_halt_v in CB_HALT_GRID:
                ...
                base_rows = [r for r in results
                             if r["d"]==d and r["q"]==q and r.get("kappa_max")==kappa_max
                             and r["eta"]==0 and r.get("cb_halt",0.08)==cb_halt_v]
                if base_rows:
                    r = base_rows[0]
                    print(f"    d={d:.2f} q={q:.2f} κ={km_str:4} halt={cb_halt_v:.0%}:  "
                          f"Calmar={r['calmar']:.2f}  Final=${r['final']:,.0f}  "
                          f"MaxDD={r['maxdd']:.1%}  pct_halt={r.get('pct_halted',0):.1%}")

    # ── All-fixes best: omega=0.02, rho_alpha=0.5, any CB thresh ──
    ...
    if full_fix:
        ...
        print(f"    Calmar={best['calmar']:.2f}  Final=${best['final']:,.0f}  "
              ...
              f"cb_halt={best.get('cb_halt',0.08):.0%}  cb_win={best.get('cb_window',90)}d  "
              f"pct_halt={best.get('pct_halted',0):.1%}")
    else:
    ...
    print(f"  v40/v41 best:      Calmar=16.63  Final=$617K  MaxDD=−34.6%  RhBlk=0")

    # ── Sweet-spot scan ──
    print(f"\n  Sweet-spot scan (d=0.25, q=0.50, km=None, om=0.02, ra=0.5):")
    ...
    sub.sort(key=lambda r: (r.get("cb_halt",0.08), r.get("cb_window",90),
                            r["lock_min"], r["eta"]))
    print(f"  {'halt':>6}  {'win':>4}  {'lock':>6}  {'η':>9}  "
          f"{'Calmar':>7}  {'Final':>12}  {'MaxDD':>7}  {'pct_halt':>9}  {'r2026':>7}")
    for r in sub[:40]:
        print(f"  {r.get('cb_halt',0.08):>6.2%}  {r.get('cb_window',90):>4}  "
              f"{r['lock_min']:>6}  {r['eta']:>9.1e}  "
              f"{r['calmar']:>7.2f}  ${r['final']:>11,.0f}  "
              f"{r['maxdd']:>7.1%}  {r.get('pct_halted',0):>9.1%}  "
              f"{r.get('r2026',0):>+7.3f}")
    ...
    out_json = OUT_DIR_ / "crypto_godmode_v42_cb_thresh.json"
    rows_clean = [
    ...
    with open(out_json, "w") as f:
        json.dump(
            {
                "version":    "v42",
                "sweep":      "CB threshold grid: halt, resume_frac, window",
                "cb_halt_grid":   CB_HALT_GRID,
                "cb_resume_frac": CB_RESUME_FRAC_GRID,
                ...
                "fixes": {
                    ...
                    "Fix3_rho_eff_gate_v40":   "rho_eff_at_halt<alpha*rho_typ AND current<alpha*rho_typ → stay halted",
                },
                "results": rows_clean,
            },
    ...
    print(f"\n{BAR}")
    print(f"  v42 COMPLETE  |  elapsed={elapsed:.1f}s  ({elapsed/60:.1f} min)")
    print(f"  CB threshold sweep completed: {len(CB_HALT_GRID)} halt thresholds x {len(CB_WINDOW_GRID)} windows.")
