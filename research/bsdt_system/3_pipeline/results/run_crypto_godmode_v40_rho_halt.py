# -*- coding: utf-8 -*-
"""
run_crypto_godmode_v40_rho_halt.py
====================================

v40 = v39 + Fix 3 revised: ρ_eff gate uses ρ_eff recorded AT HALT TIME

v39 post-mortem
---------------
Fix 3 in v39 checked ρ_eff at the release bar (current bar). RhBlk=0 across
all 216 variants because by the time cb_dd naturally recovers to ≤resume=4%,
the ODE's ρ_eff has ALSO recovered — even for Mode-2 crash scenarios.

Root cause: Mode-2 crashes in v38 were caused by the linear budget (980× over-
estimate) releasing the CB early while ρ_eff was still suppressed. Fix 2 in v39
corrects the budget → G2.4 barely fires → standard resumes only occur after full
price recovery → ρ_eff has meanwhile recovered → gate misses Mode 2 entirely.

Fix 3 revised (v40)
-------------------
Record ρ_eff at HALT TIME. Label the trip as an ODE-shock trip when:
    ρ_eff_at_halt < rho_alpha × ρ_typ

For ODE-shock trips: require BOTH price recovery AND ODE recovery before
releasing from CB:
    cb_dd ≤ cb_resume   (price recovered)
    AND ρ_eff_current ≥ rho_alpha × ρ_typ  (ODE recovered)

This prevents re-entry even after natural price recovery if the ODE is still in
a sluggish state at that exact moment. If ρ_eff has recovered by then, the gate
passes — no false positives from non-shock trips.

For non-ODE-shock trips (ρ_eff_at_halt already healthy): gate is not applied,
standard resume logic unchanged.

v39 = v38 + Three targeted re-entry filters derived from ODE diagnostics
...
BAR      = "=" * 120
OUT_DIR_ = Path(OUT_DIR) / "v40_rho_halt"
...
# ── v40 CB simulation with Fix 3 revised: ρ_eff gate at halt time ───────────

def simulate_combined_lockmin_v40(
...
def main() -> None:
    ...
    # Variant count: per (d,q,km): 1 baseline + 2 eta × 2 lock × 2 omega × 3 rho = 25
    n_km    = len(KAPPA_MAX_GRID)
    n_inner = 1 + len([e for e in ETA_GRID if e > 0]) * len(LOCK_MIN_GRID) * \
              len(OMEGA_THRESH_GRID) * len(RHO_ALPHA_GRID)
    n_total = len(TOP3) * n_km * n_inner

    print(BAR)
    print("  CRYPTO GODMODE v40 — Fix 3 revised: ρ_eff gate at HALT TIME (not release time)")
    ...
    print("  Fix 3 revised: ρ_eff_at_halt < α×ρ_typ AND ρ_eff_current < α×ρ_typ → extend")
    print("  (gate only activates for ODE-shock trips; non-shock trips unaffected)")
                ...
                for lock_min in LOCK_MIN_GRID:
                    ...
                    for omega_thresh in omega_loop:
                        for rho_alpha in rho_loop:
                            ...
                            sim = simulate_combined_lockmin_v40(
                                unit_normal=unit,
    ...
    print(f"  v38 best:          see v38 run output")
    ...
    for kappa_max in KAPPA_MAX_GRID:
        ...
        sub.sort(key=lambda r: (r["lock_min"], r["eta"], str(r["omega_thresh"]),
                                r["rho_alpha"]))
        print(f"\n  κ_max={km_str}:")
        print(f"  {'lock_min':>8}  {'η':>9}  {'ω':>7}  {'ρα':>5}  "
    ...
    out_json = OUT_DIR_ / "crypto_godmode_v40_rho_halt.json"
    rows_clean = [
    ...
    print(f"\n{BAR}")
    print(f"  v40 COMPLETE  |  elapsed={elapsed:.1f}s  ({elapsed/60:.1f} min)")
    ...
    print(f"    Fix 3 revised (ρ halt):  ρ_eff_at_halt < α×ρ_typ AND current ρ_eff < threshold → extend")
    print(BAR)
