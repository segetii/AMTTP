...
if __name__ == '__main__':
    ...
    _eng_out = _v63mod.calibrate_intraday_engine(X, calib_mask)
    M, net, geom, lyap, ews, stoch, e_star, theta = _eng_out[:8]
    cos_mu, cos_sig = (_eng_out[8], _eng_out[9]) if len(_eng_out) > 8 else (0.0, 1.0)
    rss_mu, rss_sig = (_eng_out[10], _eng_out[11]) if len(_eng_out) > 10 else (0.5, 0.1)
    xi_calib = _eng_out[12] if len(_eng_out) > 12 else None
