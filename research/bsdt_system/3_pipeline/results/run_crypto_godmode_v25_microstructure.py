"""
run_crypto_godmode_v25_microstructure.py
=========================================
OBJECTIVE: Improve signal quality by adding Binance microstructure entry filters
           on top of the v17 champion strategy (be50_btc_sol).

APPROACH: Keep EVERYTHING from v17 unchanged (canonical engine, CB params,
          K=6.0, breakeven stop, ZE7 gate). Add 1-2 lightweight veto conditions
          at trade entry using:
            (1) Taker Buy Ratio  — r[9]/r[5] from klines, same API call, zero cost
            (2) Funding Rate     — 8h rate ffilled to 1h
            (3) Open Interest    — from Binance Data Vision daily ZIPs
            (4) Long/Short Ratio — also from Data Vision

          A veto simply skips an entry — no sizing changes, no CB changes.
          Data is loaded once from cache then injected bar-by-bar via shifted
          arrays (shift(1) = prev bar value = no lookahead bias).

BENCHMARK: v17 (be50_btc_sol)
    CAGR=+206%  Calmar=+8.54  Sharpe=+1.849  MaxDD=-24.11%
    2023=+292%  2024=+70%  2025=+424%  2026=0%

RESULT TARGET: Same or better Calmar, and ideally some 2026 return.

SWEEP:
    tbr_long_min  in [None, 0.46, 0.47, 0.48, 0.49, 0.50]
    tbr_short_max = 1 - tbr_long_min  (symmetric)
    fr_long_max   in [None, 0.0003]
    oi_floor      in [None, -0.005, -0.010]
"""
...
from simulate_master_strategy import (
    ...
    RT_COST, FUND_HOURLY, simulate_combined, equity_metrics, period_table, INIT,
)
...
OUT_DIR_ = Path(OUT_DIR) / "v25_microstructure"

# ─── filter sweep configurations ─────────────────────────────────────────────

# Each dict defines the microstructure entry filter params:
#   tbr_long_min   : float | None  — veto long  if TBR prev-bar < this
#   tbr_short_max  : float | None  — veto short if TBR prev-bar > this
#   fr_long_max    : float | None  — veto long  if FR prev-bar > this
#   oi_long_floor  : float | None  — veto long  if OI_pct1h prev-bar < this

TBR_PARAMS = [None, 0.46, 0.47, 0.48, 0.49, 0.50]
FR_PARAMS  = [None, 0.0003]
OI_PARAMS  = [None, -0.005]

FILTER_VARIANTS = []
# TBR-only sweep
for tbr in TBR_PARAMS:
    FILTER_VARIANTS.append(dict(
        name=f"tbr{'None' if tbr is None else f'{tbr:.2f}'.replace('.','p')}",
        tbr_long_min=tbr,
        tbr_short_max=None if tbr is None else 1.0 - tbr,
        fr_long_max=None,
        oi_long_floor=None,
    ))
# FR-only
FILTER_VARIANTS.append(dict(name="fr0003",  tbr_long_min=None, tbr_short_max=None, fr_long_max=0.0003, oi_long_floor=None))
# OI-only variants (only if OI data available)
FILTER_VARIANTS.append(dict(name="oi005",   tbr_long_min=None, tbr_short_max=None, fr_long_max=None,   oi_long_floor=-0.005))
FILTER_VARIANTS.append(dict(name="oi010",   tbr_long_min=None, tbr_short_max=None, fr_long_max=None,   oi_long_floor=-0.010))
# Combinations
FILTER_VARIANTS.append(dict(name="tbr0p48+fr",    tbr_long_min=0.48, tbr_short_max=0.52, fr_long_max=0.0003, oi_long_floor=None))
FILTER_VARIANTS.append(dict(name="tbr0p48+oi005", tbr_long_min=0.48, tbr_short_max=0.52, fr_long_max=None,   oi_long_floor=-0.005))
FILTER_VARIANTS.append(dict(name="tbr0p48+all",   tbr_long_min=0.48, tbr_short_max=0.52, fr_long_max=0.0003, oi_long_floor=-0.005))
FILTER_VARIANTS.append(dict(name="tbr0p49+all",   tbr_long_min=0.49, tbr_short_max=0.51, fr_long_max=0.0003, oi_long_floor=-0.005))


# ─── v25 simulation function ──────────────────────────────────────────────────

def simulate_unit_breakeven_v25(
    op, hi, lo, cl,
    ...
    # microstructure filter arrays (shifted 1 bar before passing in = no lookahead)
    tbr_arr:      np.ndarray | None = None,
    fr_arr:       np.ndarray | None = None,
    oi_pct1h_arr: np.ndarray | None = None,
    lsr_arr:      np.ndarray | None = None,
    # filter thresholds
    ...
    """
    Identical to simulate_unit_breakeven_v22 (tracks long/short separately)
    with an additional microstructure veto at entry.

    All veto conditions apply only at entry — open positions are NOT touched.
    A bar with NaN microstructure data skips the veto (treated as neutral).
    """
    n = len(op)
    ...
    for i in range(n):
        ...
        if pos == 0 and hactive:
            # ZE7 gate (unchanged from v12)
            if abs(float(ze7_arr[i])) < ze7_min:
                counts["vetoed_ze7"] += 1
                continue

            # Microstructure veto: read prev-bar values (arrays already shifted)
...
def run_variant_v25(
    cfg:     dict,          # champion v12 cfg (be50_btc_sol)
    fcfg:    dict,          # filter config (tbr_long_min etc.)
    inputs:  dict,
    sig_map: dict[str, pd.Series],
    log_ann: pd.DataFrame,
) -> dict:
    """
    Run the champion variant with microstructure entry filters.
    Per-asset microstructure data is aligned to hourly bars with a 1-bar shift
    ...
    # Map internal asset name -> Binance symbol
    ...
    for asset, p0 in inputs.items():
        ...
        # Load microstructure features (shifted 1h = prev-bar values)
        ...
        def _align_shift(col: str, default: float) -> np.ndarray:
            ...
            s = ms[col]
            # Normalize to tz-naive to match hours_idx (OHLCV index is tz-naive)
            if s.index.tz is not None:
                s = s.tz_convert("UTC").tz_localize(None)
            # shift(1) = previous bar value
        ...
        arr, long_arr, short_arr, counts = simulate_unit_breakeven_v25(
            p["op"], p["hi"], p["lo"], p["cl"],
...
# ─── simulate combined (standard CB) ─────────────────────────────────────────

def simulate_combined_v25(unit: pd.Series) -> dict:
    """Run simulate_combined with champion CB params."""
...
def _yoy_table(eq: pd.Series) -> list[dict]:
    ...
    for row in period_table(eq, "Y"):
        row["period"] = row["period"][:4]  # "2023-12-31" -> "2023"
        rows.append(row)
    return rows
...
def main():
    ...
    print(BAR)
    print("  CRYPTO GODMODE v25 — MICROSTRUCTURE ENTRY FILTERS")
    print(BAR)
    ...
    print(f"  Sweep: {len(FILTER_VARIANTS)} filter variants")
    print()

    # ── 1. Build canonical engine ─────────────────────────────────────────────
    print("[1] Building canonical engine (shared across all variants)...")
    ...
    Sigma_f, theta, _ = calibrate(df, train_mask, w_star, b_aligned, kappa=KAPPA_A, label="v25")
    log_ann     = run_godmode_sweep(df, train_mask, test_mask, w_star, b_aligned,
                                     Sigma_f, theta, kappa=KAPPA_A, anneal=True, label="v25_anneal")
    ...
    print(f"  Engine built. Data: {df.index[0].date()} → {df.index[-1].date()}  "
          f"train={train_mask.sum():,}h  test={test_mask.sum():,}h")

    # ── 2. Load all microstructure data ───────────────────────────────────────
    print("\n[2] Loading microstructure data ...")
    _ASSET_TO_SYM = {"btc": "BTCUSDT", "sol": "SOLUSDT", "eth": "ETHUSDT"}
    symbols = list({_ASSET_TO_SYM.get(a, a.upper() + "USDT") for a in inputs.keys()})
    for sym in symbols:
        _ = _get_ms(sym)

    # ── 3. Baseline: reproduce v17 result (no microstructure filters) ─────────
    print("\n[3] Baseline (no filters = v17 equivalent)...")
    base_res = run_variant_v25(CHAMPION_CFG,
                               dict(tbr_long_min=None, tbr_short_max=None, fr_long_max=None, oi_long_floor=None),
                               inputs, sig_map, log_ann)
    base_sim = simulate_combined_v25(base_res["unit"])
    base_eq  = base_sim["eq"]
    base_oos = base_eq[base_eq.index >= test_ts]
    base_m   = _m(base_oos, "BASELINE")
    print(f"  BASELINE: final=${base_m['final']:,.0f}  CAGR={base_m['cagr']:+.2%}  "
          f"Calmar={base_m['calmar']:+.3f}  MaxDD={base_m['maxdd']:+.2%}  halted={base_sim['pct_halted']:.1%}")
    _print_yoy(base_oos, "BASELINE")

    # ── 4. Filter sweep ───────────────────────────────────────────────────────
    print(f"\n[4] Sweeping {len(FILTER_VARIANTS)} filter variants ...")
    results = []
    for fcfg in FILTER_VARIANTS:
        fname = fcfg["name"]
        res   = run_variant_v25(CHAMPION_CFG, fcfg, inputs, sig_map, log_ann)
        sim   = simulate_combined_v25(res["unit"])
        eq    = sim["eq"]
        oos   = eq[eq.index >= test_ts]
        m     = _m(oos, fname)

        # Count total vetoes
        total_vetoes   = sum(sum(pa["counts"].get(k, 0) for k in
    ...
    # ── 5. Summary ────────────────────────────────────────────────────────────
    print(f"\n{BAR}")
    print("  v25 FILTER RESULTS  (OOS only, benchmark = BASELINE)")
    print(BAR)
    print(f"  {'name':<22}  {'final':>9}  {'CAGR':>7}  {'Calmar':>8}  {'MaxDD':>7}  "
          f"{'2023':>6}  {'2024':>6}  {'2025':>6}  {'2026':>6}  {'veto':>5}")
    print("  " + "-"*97)
    ...
    for r in sorted(results, key=lambda x: -x["calmar"]):
        print(f"  {r['name']:<22}  ${r['final']:>9,.0f}  {r['cagr']:+7.2%}  "
              ...
              f"{_r(r['r2023']):>6}  {_r(r['r2024']):>6}  {_r(r['r2025']):>6}  "
              f"{_r(r['r2026']):>6}  {r['veto_pct']:>5.1%}")

    best = max(results, key=lambda x: x["calmar"])
    print(f"\n  Best Calmar: {best['name']}  Calmar={best['calmar']:+.3f}")
    if best["calmar"] > base_m["calmar"]:
        delta = best["calmar"] - base_m["calmar"]
        print(f"  IMPROVEMENT vs baseline: Calmar +{delta:.3f}  "
              f"({best['calmar']:+.3f} vs {base_m['calmar']:+.3f})")
    else:
        print(f"  No Calmar improvement over baseline ({base_m['calmar']:+.3f})")

    # Save
    out = OUT_DIR_ / "crypto_godmode_v25_microstructure.json"
    payload = dict(
        meta=dict(version="v25_microstructure",
                  cb_halt=CB_HALT, cb_resume=CB_RESUME, cb_window_days=CB_WINDOW_DAYS,
                  K_normal=K_NORMAL, rt_bps=REALISTIC_RT_BPS,
                  elapsed=time.time() - t0),
        baseline=dict(final=base_m["final"], cagr=base_m["cagr"],
                      calmar=base_m["calmar"], maxdd=base_m["maxdd"],
                      pct_halted=base_sim["pct_halted"]),
        results=results,
        best=best,
    )
