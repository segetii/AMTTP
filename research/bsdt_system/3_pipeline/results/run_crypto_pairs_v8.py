"""
Crypto BSDT v8 — Training-Anchored Z-Score (root-cause fix)
=============================================================

v7 lesson:
  Test VALID% = 13–17%, drift_med = 0.77–0.90 for ALL pairs.
  Root cause: rolling z-score normalises to a LOCAL mean, masking regime
  shifts. The 60-day window adapts to a drifting spread, so the spread
  *appears* mean-zero to the UDL and router—while drifting structurally.
  The validity drift gate in v7 correctly caught this, but at the cost of
  suppressing nearly all trades. That is treating the symptom.

v8 fix — Training-Anchored Z-Score:
  z_t = (S_t − μ_train) / σ_train
  μ_train = mean of spread over TRAIN_START–TRAIN_END
  σ_train = std  of spread over TRAIN_START–TRAIN_END

  This makes z_t a genuine out-of-sample deviation from the learned
  equilibrium. If the pair has structurally repriced, z drifts and the
  validity filter correctly fires ONCE to disable it—rather than hiding
  the drift inside an adaptive window.

Consequences:
  • If a pair is healthy (ETH/SOL): z is approximately stationary →
    VALID% high, strong positions available
  • If a pair has repriced (ETH/BNB, ETH/BTC): z trends → drift gate
    fires → pair disabled automatically — no false equilibrium signal

Validity filter thresholds revised for anchored z:
  DRIFT_THRESH  = 1.0 σ  (was 0.5 — half-sigma too tight for crypto)
  EFF_THRESH    = 0.0    (unchanged)

Full 5-layer architecture (unchanged from v7):
  Layer 1: Ω — timing gate
  Layer 2: A — intensity gate
  Layer 3: UDL MDN — geometry (EQUIL / DEGEN / STRUCT)
  Layer 4: Validity filter (drift + effectiveness, anchored z)
...
def main():
    print("=" * 80)
    print("  CRYPTO BSDT v8 — Training-Anchored Z-Score + UDL MDN + Validity")
    print("  Key fix: z = (S − μ_train) / σ_train  (replaces rolling window normalisation)")
    print("  Layer 4: Validity (|z̄_60d| < 1.0  AND  rev_eff_60d > 0)")
    print("=" * 80)
    ...
    results = {
        "model": "Crypto BSDT v8 — Training-Anchored Z-Score + UDL MDN + Validity",
        "version": "v8",
        "architecture": {
    ...
    import os
    out_path = os.path.join(OUT_DIR, "crypto_bsdt_v8_results.json")
    ...
    # ── Summary ───────────────────────────────────────────────────────────
    print("\n" + "=" * 80)
    print("  SUMMARY: v4 → v6 → v7 → v8 (training-anchored z-score)")
    print("=" * 80)
    for name in ['A_directional', 'P1_ETH_BTC', 'P2_ETH_SOL', 'P3_ETH_BNB',
                 'M1_ALT_macro']:
        v4 = v4_ref.get(name, None)
        v7 = v7_ref.get(name, None)
        v8 = sims[name]['sharpe'] if name in sims else None
        ...
        if name in sims:
            ...
            dd = ""
        print(f"  {name:<22}  v4:{v4s}  v7:{v7s}  v8:{v8s}{dd}")
    print()
