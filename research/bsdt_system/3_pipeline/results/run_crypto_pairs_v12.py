"""
Crypto BSDT v12 — Ω decomposition: price + leverage + cross-market
===================================================================

v11 root-cause diagnosis:
  γ_fast = 1/(1+Ω) still flat because Ω_price is constructed from 8 crypto
  features that collapse into effective rank-4 (btc_dom and cross_disp are
  linear combinations of the 4 returns). Row-stochastic normalisation pushes
  λ_max → 1 whenever crypto beta is high (which is: always).
  γ ≈ 0.514 ± 0.004 throughout the test period.

v12 — three targeted fixes:

  1. Ω_price: extend feature space with cross-market coupling
       8 crypto → + ret_spx_z, dvix_z, ret_dxy_z  (11 features)
       VIX/SPX/DXY add genuine off-diagonal coupling blocks.
       Crypto-idiosyncratic rallies (2023, 2024 H1): spx/vix decorrelate
       → W loses uniformity → λ_max falls → Ω_price drops.
       Correlated panics (2025 bear): everything couples up → λ_max spikes.

  2. Ω_leverage: separate BSDT on Binance funding rate features
       6 features: fr_eth_z, fr_btc_z,
                   fr_vol_eth_z, fr_vol_btc_z,
                   fr_cum5_eth_z, fr_cum5_btc_z
       Funding spike → leveraged longs accumulating → liquidation risk.
       High Ω_leverage → system near stress peak → reversion favoured.

  3. γ_rank = expanding_percentile_rank(Ω_combined)
       Ω_combined = 0.6 * Ω_price + 0.4 * Ω_leverage (z-normalised before mix)
       γ_rank ALWAYS spans [0,1] by construction (uniform distribution).
       std(γ_rank_uniform) = 0.289 vs std(γ_fast) = 0.004 in v11.
       Highest-stress day ever seen → γ_rank ≈ 1.0 → max reversion tilt.
       Lowest-stress day → γ_rank ≈ 0.0 → max directional tilt.

v12 architecture:
  Layer 1:  Ω_price — timing gate (Q50 gate, unchanged)
  Layer 2:  A — intensity gate (MFLS/γ from Ω_price, unchanged)
  Layer 3:  UDL MDN — geometry  (EQUIL / DEGEN / STRUCT, unchanged)
  Layer 4:  Drift gate  |z̄_60d| < 1.0  (anchored z, unchanged)
  Layer 5:  POA-scaled sizing  (unchanged)
  Layer 6:  w = rolling_Sharpe × γ_rank_tilt(t)  [v12 upgrade]
  New:      γ_rank = pct_rank(Ω_combined); Ω_combined uses leverage data
"""
...
def main():
    ...
    W = 96
    print("\n" + "─" * W)
    print("── v9 / v10 / v11 / v12: dynamic portfolio comparison [test] ──")
    print("─" * W)
    print(f"  {'Strategy':<22}  {'v9 Sh':>7}  {'v10 Sh':>8}  {'v11 Sh':>8}  "
          f"{'v12 Sh':>8}  {'v11→v12':>8}  {'MaxDD_v12':>10}")
    print("─" * W)

    v9_sharpes = {k: sims.get(k, {}).get('sharpe') for k in pos_base}
    # v10 individual Sharpes (positions ~0.51× scaled — use v10 run reference)
    v10_ref_ind = {
        'A_directional': 0.526, 'P1_ETH_BTC': -1.253, 'P2_ETH_SOL': 1.350,
        'P3_ETH_BNB': -0.468,   'M1_ALT_macro': 1.387, 'M1_BTC_macro': 0.156,
    }
    # v11 individual = v9 (positions unchanged)
    indiv_labels = ['A_directional', 'P1_ETH_BTC', 'P2_ETH_SOL',
                    'P3_ETH_BNB', 'M1_ALT_macro', 'M1_BTC_macro']
    ...
    for name in indiv_labels + ['DYNAMIC_COMBINED']:
        ...
        if name == 'DYNAMIC_COMBINED':
            ...
            v12s = sims_v12_comb.get('sharpe')
            dd   = sims_v12_comb.get('max_dd')
        else:
            v10s = v10_ref_ind.get(name)
            v11s = v9s   # individual positions unchanged
            v12s = v9s   # individual positions unchanged in v12 too
        ...
        v12ss = f"{v12s:>+8.3f}" if v12s is not None else "        "
        dlt   = (f"{v12s - v11s:>+8.3f}"
                 if (v11s is not None and v12s is not None) else "        ")
        dds   = f"{dd:>10.3f}" if dd is not None else "          "
        label = name if name != 'DYNAMIC_COMBINED' else '─' * 4 + ' DYNAMIC_COMBINED'
        print(f"  {label:<26}  {v9ss}  {v10ss}  {v11ss}  {v12ss}  {dlt}  {dds}")

    # ── Mean weights: v10 vs v11 vs v12 ───────────────────────────────────
    print(f"\n── Mean weights [test]: v10 / v11 / v12 ──────────────────────────────")
    print(f"  {'Strategy':<22}  {'v10 wt':>8}  {'v11 wt':>8}  {'v12 wt':>8}  "
          f"{'v11→v12':>8}  {'γ mode':>12}")
    print("─" * 74)
    for k, mode in tilt_modes.items():
        wt10 = cs_wts_v10[k][test_mask].mean()
        wt11 = cs_wts_v11[k][test_mask].mean()
        wt12 = cs_wts_v12[k][test_mask].mean()
        print(f"  {k:<22}  {wt10:>+8.3f}  {wt11:>+8.3f}  {wt12:>+8.3f}  "
              f"{wt12-wt11:>+8.3f}  {mode:>12}")
    ...
    results = {
        "model":   "Crypto BSDT v12 — Ω decomposition: price + leverage + cross-market",
        "version": "v12",
        "gamma_stats": {
    ...
    import os
    out_path = os.path.join(OUT_DIR, "crypto_bsdt_v12_results.json")
    ...
    # ── Summary ───────────────────────────────────────────────────────────
    print("\n" + "=" * 84)
    print("  SUMMARY: v9 → v10 → v11 → v12")
    print("=" * 84)
    print(f"  {'Version':<22}  {'Source':<38}  {'DYNAMIC Sharpe':>14}")
    print("─" * 84)
    summary_rows = [
        ('v9 (4-way equal)',    'drift gate, anchored z-score',                -0.020),
        ('v10 (Sharpe wts)',   'slow γ pos scaling + Sharpe weights',         sims_v10_comb['sharpe']),
        ('v11 (fast γ tilt)',  'γ_fast=1/(1+Ω_price), weight tilt',           sims_v11_comb['sharpe']),
        ('v12 (γ_rank tilt)',  'γ_rank=pct_rank(Ω_price+lev+cross)',          sims_v12_comb['sharpe']),
    ]
